...
/**
 * Envía el PDF de una devolución directamente a Telegram desde el navegador.
 * IMPORTANTE: al publicar en GitHub Pages, el token del bot será visible.
 */
export async function enviarPDFTelegram(pdfBlob, datosDevolucion) {
  ...
  const folio = datosDevolucion?.folio || "DEVOLUCION";
  const tienda = datosDevolucion?.tienda || "SIN TIENDA";
  const encargado = datosDevolucion?.encargado || "SIN ENCARGADO";
  const totalPiezas = Number(datosDevolucion?.totalPiezas || 0);
  const totalPublico = Number(datosDevolucion?.totalPublico || 0);
