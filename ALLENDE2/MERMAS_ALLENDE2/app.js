...
const LS_BORRADOR = "provsoft_merma_allende2_borrador";
const DB_NAME = "provsoft_catalogo_mermas_allende2";
const STORE_PRODUCTOS = "productos";
const LS_CATALOGO_FECHA = "provsoft_merma_allende2_catalogo_fecha";
const LS_PENDIENTES = "provsoft_merma_allende2_pendientes_firebase";
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_ALLENDE2, APP_CONFIG.SUBCOLECCION_MERMAS, documento.folio),
    docOnline,
    { merge:true }
  );
}
...
function recuperarBorrador(){
  ...
  // Solo conservar folios válidos de MERMAS ALLENDE 2.
  const folioRecuperado = String(b.folio || "").trim();
  document.getElementById("folio").value = folioRecuperado.startsWith("MER-ALLENDE2-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoMerma(){
  ...
  return {
    ...
    allende2Coleccion:APP_CONFIG.TIENDA_ALLENDE2,
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de merma ALLENDE 2 generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoMerma){
  ...
  pdf.setFontSize(15);
  pdf.text("COMPROBANTE DE SOLICITUD DE MERMA ALLENDE 2",105,y,{align:"center"});

  y += 8;
  pdf.setFontSize(10);
  pdf.setFont("helvetica","normal");
  pdf.text("Documento de levantamiento de merma ALLENDE 2 - destino MATRIZ",105,y,{align:"center"});
