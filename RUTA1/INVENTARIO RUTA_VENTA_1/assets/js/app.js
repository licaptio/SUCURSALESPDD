...
async function guardarAjuste() {
  ...
  try {
    const folioAjuste = `AJU-RUTA1-${fecha.replaceAll("-", "")}-${hora.replace(":", "")}-${Date.now().toString().slice(-4)}`;
    ...
    try {
      ...
      alert("El ajuste se guardó correctamente, pero falló la notificación de Telegram: " + telegramError.message);
    }
