...
async function cargarCatalogoBloqueadosLocal() {
  ...
  try {
    if (estado) estado.textContent = "Preparando catálogo local…";

    // 1) Siempre intentamos trabajar primero con IndexedDB.
    productosActivosBloqueo = await cargarProductosActivosLocal();

    // 2) En un celular/navegador nuevo, si IndexedDB está vacío,
    //    hacemos UNA descarga de productos activos y la guardamos localmente.
    if (!productosActivosBloqueo.length) {
      if (estado) estado.textContent = "Descargando catálogo activo por primera vez…";
      productosActivosBloqueo = await sincronizarProductosActivosLocal();
      try { localStorage.setItem(CACHE_SYNC, String(Date.now())); } catch {}
    } else {
      // 3) Si ya hay catálogo local, no castigamos Firebase. Solo se refresca
      //    si pasaron 24 horas desde la última sincronización conocida.
      let ultima = 0;
    ...
    if (estado) estado.textContent = "No se pudo preparar el catálogo";
  } finally {
    productosActivosBloqueoCargados = true;
    pintarResultadosBusquedaBloqueados();
  }
}
