...
async function cargarSalidasZapata() {
  try {
    ...
    const cacheKey = crearKeySemana({
      ...
      inicio: rango.inicio,
      fin: rango.fin,
      base: FECHA_BASE_INVENTARIO
    });

    const cache = await leerCacheSemana(cacheKey);
    ...
    if (cache) {
      ...
      setProgress(100);
      setStatus(
        `Consulta local lista. Semana: ${fechaCorta(rango.inicio)} a ${fechaCorta(rango.fin)}. Datos temporales desde IndexedDB.`
      );
      ocultarLoader();
      return;
    }
    ...
    await guardarCacheSemana(cacheKey, {
      inventarioInicialOriginal,
      proveedoresAutorizadosPivot,
      registrosDetalleSemana,
      registrosDetalleAcumuladoAnterior,
    ...
    setProgress(100);
    setStatus(
      `Consulta lista. Semana: ${fechaCorta(rango.inicio)} a ${fechaCorta(rango.fin)}. Entradas: ${registrosEntradasSemana.length}. Salidas: ${registrosDetalleSemana.length}. Ajustes: ${registrosAjustesSemana.length}. Guardado temporal local.`
    );
