...
const S={
  ...
  catalog:[],byCode:new Map(),cart:[],last:[]
};
  ...
  try{
    ...
    for(const u of users.docs){
      try{
        ...
        ps.forEach(p=>{if(addInventoryPart(map,p.data()))n++});
...
function selectProduct(p){
  open(`<h2>Producto seleccionado</h2><p><b>${esc(p.descripcion)}</b><br><small>${esc(p.codigo)} · Inventario inicial: ${p.inventarioInicial}</small></p><label>Cantidad de salida</label><input id="qty" class="field" type="number" min="0.001" step="0.001" inputmode="decimal"><div class="actions"><button id="cancel" class="secondary">Cancelar</button><button id="add" class="primary">Agregar</button></div>`);
  ...
  $('add').onclick=()=>{
    ...
    if(ex)ex.cantidad=totalPropuesto;else S.cart.push({...p,cantidad:q});
    S.last.unshift({...p,cantidad:q,ts:Date.now()});S.last=S.last.slice(0,2);
    render();close();$('scanInput').value='';clearQuickResults();$('scanInput').focus()
  };
  setTimeout(()=>$('qty').focus(),100)
}
function render(){$('cartCount').textContent=S.cart.length;$('lastItems').innerHTML=S.last.length?S.last.map(x=>`<div class="item"><b>${esc(x.descripcion)}</b><small>${esc(x.codigo)}</small><div class="qty">${x.cantidad}</div></div>`).join(''):'<div class="empty">Aún no hay partidas</div>'}
