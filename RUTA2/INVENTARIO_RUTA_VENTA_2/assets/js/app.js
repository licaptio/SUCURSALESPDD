...
async function guardarAjuste() {
  ...
  const fecha = String($("ajusteFecha")?.value || "").trim();
  const hora = String($("ajusteHora")?.value || "").trim();
...
async function enviarLoteAjustes() {
  ...
  const folioLote = `AJU-RUTA2-${fecha.replaceAll('-', '')}-${hora.replace(':','')}-${Date.now().toString().slice(-4)}`;

  try {
    const batch = writeBatch(db);
    loteAjustesPendientes.forEach((x, indice) => {
      const refNuevo = doc(REF_AJUSTES_RUTA2);
