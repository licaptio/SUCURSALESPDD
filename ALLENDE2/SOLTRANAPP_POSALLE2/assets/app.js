...
const SUCURSAL = "ALLENDE 2";
const RUTA_TRANSFERENCIAS = "transferencias2";
...
// Navegación POS para resultados: Flechas cambian selección y Enter abre Cantidad.
function instalarNavegacionResultados(inputId, resultadosId){
  const input = document.getElementById(inputId);
  const cont = document.getElementById(resultadosId);
  if(!input || !cont || input.dataset.navPos === "1") return;
  input.dataset.navPos = "1";
  let indice = 0;

  const items = ()=>[...cont.querySelectorAll(".resultado-item")];
  const marcar = (nuevo)=>{
    const lista = items();
    if(!lista.length){ indice = 0; return; }
    indice = Math.max(0, Math.min(nuevo, lista.length - 1));
    lista.forEach((el,i)=>el.classList.toggle("pos-selected", i === indice));
    lista[indice].scrollIntoView({block:"nearest"});
  };

  input.addEventListener("input", ()=>requestAnimationFrame(()=>marcar(0)));
  input.addEventListener("keydown", e=>{
    ...
    if(e.key === "ArrowDown"){ e.preventDefault(); marcar(indice + 1); }
      ...
      e.preventDefault();
      e.stopPropagation();
      const elegido = lista[Math.max(0, Math.min(indice, lista.length - 1))];
      const id = elegido?.dataset?.productoId;
      if(id) seleccionarProducto(id);
    }
  });
}

instalarNavegacionResultados("inpNombre","resNombre");
instalarNavegacionResultados("inpCodigo","resCodigo");
...
window.seleccionarProducto = function(id){
  ...
  // En tipo texto select() funciona de forma consistente en Chrome.
  // El 1 queda seleccionado: escribir 10 lo reemplaza directamente.
  ...
  requestAnimationFrame(enfocarCantidad);
  setTimeout(enfocarCantidad, 30);
...
// Flujo POS del carrito por teclado:
// Cantidad + Enter => Comentarios.
// Comentarios + Enter => agrega artículo y regresa INMEDIATAMENTE a captura.
const inpCantidadPOS = document.getElementById("inpCantidad");
if(inpCantidadPOS && !inpCantidadPOS.dataset.posEnterConfigurado){
  inpCantidadPOS.dataset.posEnterConfigurado = "1";
  ...
  inpCantidadPOS.addEventListener("focus", ()=>{
    requestAnimationFrame(()=>{
      try{ inpCantidadPOS.select(); }catch(_){ }
    });
  });
...
window.confirmarCantidad = function(){
  ...
  let cant = parseFloat(String(document.getElementById("inpCantidad").value).replace(",","."));
  ...
  // El focus se aplica al siguiente frame para que el modal ya esté visible.
  requestAnimationFrame(()=>{
    inpComentario.focus({preventScroll:true});
    try{ inpComentario.setSelectionRange(inpComentario.value.length, inpComentario.value.length); }catch(_){ }
  });
}

window.confirmarComentario = function(){
  if(!productoTemporal) return;

  // Congelar los datos ANTES de liberar el flujo. Así la lectura asíncrona de
  // fotos no puede mezclarse con el siguiente producto capturado.
  const producto = productoTemporal;
  const cantidad = cantidadTemporal;
  ...
  if(existente){
    ...
    if(comentario){
      existente.comentario = existente.comentario
    ...
    carrito.unshift({
      ...
      nombre:producto.concepto || "",
      cantidad:cantidad,
      precio:Number(producto.precioPublico || 0),
  ...
  // IMPORTANTE: devolver focus de inmediato. No esperar Firebase/fotos.
  enfocarCapturaPOS(true);

  // Fotos en segundo plano; no bloquean el teclado del POS.
  leerFotosMeta(producto.codigoBarra)
    .then(fotosMeta=>{
      const urlsFotos = fotosMeta && Array.isArray(fotosMeta.urlsFotos)
        ? fotosMeta.urlsFotos
        : [];
      mostrarFotosProducto(urlsFotos);
    })
    .catch(()=>mostrarFotosProducto([]));
}

const inpComentarioPOS = document.getElementById("inpComentario");
if(inpComentarioPOS && !inpComentarioPOS.dataset.posEnterConfigurado){
  inpComentarioPOS.dataset.posEnterConfigurado = "1";

  inpComentarioPOS.addEventListener("keydown", e=>{
...
async function cargarLogoPDF(){
  try{
    const r = await fetch("assets/logo.jfif");
    if(!r.ok) return null;
...
async function generarPDF(folio){
  ...
  if(logo){
    try{ pdf.addImage(logo,"JPEG",14,10,31,18); }catch(_){ }
  }

  pdf.setFont("helvetica","bold");
  pdf.setFontSize(14);
  pdf.text("PROVEEDORA DE DULCES Y DESECHABLES",50,15);
  pdf.setFontSize(12);
  pdf.text("SUCURSAL ALLENDE 2",50,21);
  pdf.setFontSize(11);
  pdf.text("SOLICITUD DE TRANSFERENCIA",50,27);

  pdf.setFont("helvetica","normal");
  pdf.setFontSize(9);
  pdf.text(`Folio: ${folio}`,14,35);
  pdf.text(`Fecha: ${new Date().toLocaleString("es-MX")}`,105,35);
  ...
  pdf.autoTable({
    ...
    body:carrito.map(i=>[
      ...
      `$${(Number(i.precio) * Number(i.cantidad)).toFixed(2)}`,
      i.comentario || ""
    ]),
  ...
  const total = carrito.reduce((a,i)=>a + Number(i.precio) * Number(i.cantidad),0);
  const y = Math.min((pdf.lastAutoTable?.finalY || 40) + 8, 285);
  pdf.setFont("helvetica","bold");
  pdf.setFontSize(11);
  pdf.text(`TOTAL: $${total.toFixed(2)}`,196,y,{align:"right"});
