...
import { iniciarModuloEntradasZapata, iniciarModuloAjusteEntradaIndependiente } from "./entradasModulo.js";
import {
  collection,
  query,
  where,
...
const FECHA_BASE_INVENTARIO = "2026-08-17";
const FECHA_INICIO_MINIMA = "2026-08-17";
const SEMANA_MINIMA = "2026-W34";
const CONTEO_ID_INVENTARIO = "INV-ABARROTESPDD-170826";
const CONTEO_CLAVE_INVENTARIO = "ABARROTESPDD-170826";
...
const REF_AJUSTES_ENTRADA_ZAPATA = collection(
  db,
  "almacenes",
  "abarrotespdd",
  "ajustes_entrada"
...
function normalizarFecha(valor) {
  ...
  if (typeof valor === "string") {
    ...
    // Acepta D/M/YYYY, DD/M/YYYY, D/MM/YYYY y DD/MM/YYYY.
    // Las salidas móviles históricas guardan, por ejemplo, "19/8/2026".
...
function normalizarListaArticulos(data = {}) {
  ...
  const candidatos = [data.articulos, data.productos, data.detalle, data.items, data.partidas];
...
function mostrarPanelResumenListas() {
  ...
  $("panelAjusteEntrada")?.classList.add("oculto");
...
function pintarListadoResumenProveedor() {
  ...
  tbody.innerHTML = rows.map((r) => `
    <tr class="${r.tipo === "ENTRADA" ? "mov-entrada" : r.tipo === "AJUSTE_ENTRADA" ? "mov-ajuste-entrada" : r.tipo === "SALIDA" ? "mov-salida" : "mov-ajuste-inv"}">
      <td class="left codigo">${escapeHtml(r.codigo)}</td>
      <td class="left">${escapeHtml(r.nombre)}</td>
...
async function cargarInventarioInicial() {
  // Inventario físico base 17/08/2026.
  // Estrategia compatible con las reglas actuales de Firestore:
  // 1) recorremos los usuarios materializados;
  // 2) además consultamos explícitamente usuarios conocidos con PARTIDAS aunque
  //    el documento padre no esté materializado (caso KRYSTAL).
  // Esto evita collectionGroup(), que puede quedar bloqueado por reglas y dejar
  // toda la tabla en blanco.
  inventarioInicialOriginal = {};

  const procesarPartida = (partidaDoc, usuarioRuta = "") => {
    const p = partidaDoc.data() || {};
    if (String(p.almacenId || "").trim().toLowerCase() !== "abarrotespdd") return false;
    if (String(p.conteoId || "").trim() !== CONTEO_CLAVE_INVENTARIO) return false;
    if (p.eliminado === true) return false;

    const codigoOriginal = String(
      p.codigo ?? p.codigoOriginal ?? p.productoId ?? p.codigoEscaneadoUltimo ?? ""
    ).trim();
    const codigoKey = normalizarCodigo(codigoOriginal);
    const nombre = String(p.descripcion ?? p.nombre ?? "").trim();
    const cantidad = Number(p.cantidad || 0);

    if (!codigoKey || !Number.isFinite(cantidad)) return false;

    if (!inventarioInicialOriginal[codigoKey]) {
    ...
    inventarioInicialOriginal[codigoKey].inviniOriginal += cantidad;
    if (!inventarioInicialOriginal[codigoKey].nombre && nombre) {
      inventarioInicialOriginal[codigoKey].nombre = nombre;
    }
    return true;
  };
  ...
  // Usuarios que sí existen como documentos.
  const usuariosRef = collection(
    db, "almacenes", "abarrotespdd", "inventariofisico",
    CONTEO_ID_INVENTARIO, "USUARIOS"
  );
  ...
  for (const usuarioDoc of usuariosSnap.docs) {
    ...
    const partidasRef = collection(
      db, "almacenes", "abarrotespdd", "inventariofisico",
      CONTEO_ID_INVENTARIO, "USUARIOS", usuarioId, "PARTIDAS"
    );
    const partidasSnap = await getDocs(partidasRef);
    partidasSnap.forEach((partidaDoc) => {
      if (procesarPartida(partidaDoc, usuarioId)) {
        totalPartidas++;
  ...
  // Firestore permite subcolecciones debajo de un documento padre inexistente.
  // KRYSTAL tiene PARTIDAS reales en esta ruta y se consulta de forma directa.
  const usuariosPartidasSinPadre = ["krystal-156140"];

  for (const usuarioId of usuariosPartidasSinPadre) {
  ...
  console.info("[INVENTARIO ABARROTES] Usuarios padre:", usuariosSnap.size);
  console.info("[INVENTARIO ABARROTES] KRYSTAL:", partidasKrystal, "partidas incluidas");
  console.info("[INVENTARIO ABARROTES] Total partidas:", totalPartidas);

  setStatus(
    `Inventario inicial Abarrotes cargado: ${Object.keys(inventarioInicialOriginal).length} artículos, ` +
    `${totalPartidas} partidas válidas. KRYSTAL: ${partidasKrystal}.`
  );
  setProgress(12);
...
async function consultarSalidas(inicio, fin) {
  ...
  // Compatibilidad con los dos formatos de SALIDAS que existen en Abarrotes:
  // 1) histórico: fecha = "YYYY-MM-DD"
  // 2) app nueva: fechaLocal = "D/M/YYYY" + creadoEn Timestamp + partidas:[...]
  // Consultamos ambos por servidor y unimos por ID para evitar duplicados.
  const inicioDate = crearFechaLocal(inicio);
  inicioDate.setHours(0, 0, 0, 0);
  const finExclusivo = crearFechaLocal(sumarDias(fin, 1));
  ...
  // Formato nuevo: fechaCaptura = "YYYY-MM-DD".
  // Esta consulta es importante cuando una salida se registra hoy pero corresponde
  // operativamente a una fecha anterior.
  const qFechaCaptura = query(
    REF_SALIDAS_ZAPATA,
    where("fechaCaptura", ">=", inicio),
    where("fechaCaptura", "<=", fin),
    orderBy("fechaCaptura", "asc")
  );

  const qCreadoEn = query(
    REF_SALIDAS_ZAPATA,
    where("creadoEn", ">=", inicioDate),
    where("creadoEn", "<", finExclusivo),
    orderBy("creadoEn", "asc")
  );

  const resultados = await Promise.allSettled([
    getDocs(qFecha),
    getDocs(qFechaCaptura),
    getDocs(qCreadoEn)
  ]);
  ...
  resultados.forEach((r) => {
    if (r.status !== "fulfilled") {
      console.warn("[SALIDAS ABARROTES] Consulta parcial falló:", r.reason);
      return;
    }
    r.value.forEach((d) => documentos.set(d.id, d));
  });
  ...
  documentos.forEach((documento) => {
    const data = documento.data() || {};
    // Prioridad: fecha operativa capturada; después formatos históricos.
    const fecha = normalizarFecha(
      data.fechaCaptura || data.fecha || data.fechaCapturaTxt || data.fechaLocal || data.creadoEn || ""
    );
    if (!fecha || fecha < inicio || fecha > fin || fecha < FECHA_INICIO_MINIMA) return;
    ...
    articulos.forEach((art, idx) => {
      ...
      detalle.push({
        tipo: "SALIDA",
        docId: documento.id,
        partida: Number(art.renglon || idx + 1),
        folio: String(data.folio || documento.id || "").trim(),
        fecha,
        codigo: codigoOriginal,
        codigoKey,
  ...
  detalle.sort((a, b) => a.fecha.localeCompare(b.fecha) || a.folio.localeCompare(b.folio));
  return { detalle, totalDocs: documentos.size };
}
...
async function consultarAjustesEntrada(inicio, fin) {
  const qAjustesEntrada = query(
    REF_AJUSTES_ENTRADA_ZAPATA,
    where("fecha_movimiento", ">=", inicio),
    where("fecha_movimiento", "<=", fin)
  );
  const snap = await getDocs(qAjustesEntrada);
  const detalle = [];

  snap.forEach(documento => {
    const data = documento.data() || {};
    const fecha = normalizarFecha(data.fecha_movimiento || data.fecha || "");
    if (!fecha || fecha < inicio || fecha > fin) return;
    const hora = String(data.hora_movimiento || data.hora || "00:00").trim();
    const articulos = Array.isArray(data.articulos) ? data.articulos : [];

    articulos.forEach((art, idx) => {
      const codigo = String(art.codigo_interno || art.codigo || art.codigo_factura || "").trim();
      const codigoKey = normalizarCodigo(codigo);
      if (!codigoKey) return;
      const cantidad = Number(art.cantidad_ajuste ?? art.cantidad ?? 0);
      if (!Number.isFinite(cantidad) || cantidad === 0) return;
      detalle.push({
        tipo: "AJUSTE_ENTRADA",
        tipoMovimiento: "AJUSTE_ENTRADA",
        docId: documento.id,
        partidaId: `${documento.id}-${idx}`,
        fecha,
        hora,
        folio: String(data.folioAjusteEntrada || documento.id).trim(),
        folioEntrada: String(data.folioEntrada || "").trim(),
        entrada_id: String(data.entrada_id || "").trim(),
        proveedor: String(data.razon_social_emisor || data.proveedor || "").trim(),
        codigo,
        codigoKey,
        nombre: repararTexto(art.descripcion_interna || art.descripcion_factura || art.nombre || ""),
        cantidad,
        motivo: String(data.motivo || "AJUSTE A ENTRADA").trim()
      });
    });
  });
...
async function cargarMovimientosHasta(fin, forzar = false) {
  ...
  setStatus(`Descargando movimientos Abarrotes ${fechaCorta(inicioConsulta)} a ${fechaCorta(fin)}...`);

  // SALIDAS y ENTRADAS forman parte del mismo cache temporal de la tabla.
  // Se descargan una sola vez por rango; después, las entradas nuevas se
  // incorporan directamente en memoria sin volver a consultar Firestore.
  const [salidas, entradas, ajustesEntrada, ajustesInventario] = await Promise.all([
    consultarSalidas(inicioConsulta, fin),
    consultarEntradas(inicioConsulta, fin),
    consultarAjustesEntrada(inicioConsulta, fin),
  ...
  anexarSinDuplicados(cacheMovimientos.entradas, ajustesEntrada.detalle);
  anexarSinDuplicados(cacheMovimientos.ajustes, ajustesInventario.detalle);
...
async function cargarSalidasZapata(forzarRecarga = false) {
  try {
    ...
    setStatus(
      ...
      `Entradas: ${registrosEntradasSemana.length}. ` +
    ...
    console.error(error);
    setStatus("Error al cargar movimientos de Abarrotes: " + error.message);
    ocultarLoader();
  }
}
...
function aplicarMovimientoInventario(existenciaActual, mov) {
  if (mov.tipo === "ENTRADA" || mov.tipo === "AJUSTE_ENTRADA") {
...
function construirPivot(
  ...
  entradasAcumuladoAnterior.forEach((item) => {
    ...
    row.movimientosAnteriores.push({
      tipo: item.tipoMovimiento || item.tipo || "ENTRADA",
      fecha: item.fecha,
...
function pintarDetalle() {
  ...
  const totalEntradas = rows
    .filter(x => x.tipo === "ENTRADA" || x.tipo === "AJUSTE_ENTRADA")
  ...
  tfoot.innerHTML = `
    <tr>
      <td class="left" colspan="12">TOTAL ENTRADAS / AJUSTES A ENTRADA SEMANA</td>
...
function exportarExcel() {
  ...
  XLSX.writeFile(
    wb,
    `inventario_abarrotes_${vistaActual}_${rangoSemanaActual.inicio}_a_${rangoSemanaActual.fin}.xlsx`
  );
}
...
function mostrarPanelAjusteEntrada() {
  const card = document.querySelector(".card");
  card?.classList.add("modo-ajuste-entrada-independiente");
  $("panelEntradasZapata")?.classList.add("oculto");
  $("panelAjustesInventario")?.classList.add("oculto");
  $("panelAjusteEntrada")?.classList.remove("oculto");
}

function ocultarPanelAjusteEntrada() {
  document.querySelector(".card")?.classList.remove("modo-ajuste-entrada-independiente");
  $("panelAjusteEntrada")?.classList.add("oculto");
}

function mostrarPanelEntradasZapata() {
  document.body.classList.add("modo-entradas-body");
...
function ocultarPanelEntradasZapata() {
  document.body.classList.remove("modo-entradas-body");
  document.querySelector(".card")?.classList.remove("modo-entradas-zapata");
  $("panelEntradasZapata")?.classList.add("oculto");
}


const COSTEO_DB_NAME = "provsoft_inventario_costeado_abarrotespdd";
const COSTEO_DB_VERSION = 1;
const COSTEO_STORE = "costos_productos";
let filasInventarioCosteado = [];

function abrirDbCosteo() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open(COSTEO_DB_NAME, COSTEO_DB_VERSION);
    req.onupgradeneeded = () => {
      const idb = req.result;
      if (!idb.objectStoreNames.contains(COSTEO_STORE)) {
        idb.createObjectStore(COSTEO_STORE, { keyPath: "codigo" });
      }
    };
...
async function leerCostosLocales(codigos) {
  const salida = new Map();
  if (!codigos.length) return salida;
  try {
    const idb = await abrirDbCosteo();
    await new Promise((resolve, reject) => {
      const tx = idb.transaction(COSTEO_STORE, "readonly");
      const store = tx.objectStore(COSTEO_STORE);
      codigos.forEach(codigo => {
        const req = store.get(codigo);
        req.onsuccess = () => { if (req.result) salida.set(codigo, req.result); };
      });
    ...
    console.warn("No se pudo leer cache local de costos:", error);
  }
  return salida;
}

async function guardarCostosLocales(items) {
  if (!items.length) return;
  try {
    const idb = await abrirDbCosteo();
    await new Promise((resolve, reject) => {
      const tx = idb.transaction(COSTEO_STORE, "readwrite");
      const store = tx.objectStore(COSTEO_STORE);
    ...
    console.warn("No se pudo guardar cache local de costos:", error);
  }
}

async function consultarProductosPorCodigo(codigos) {
  const encontrados = [];
  const tamanoLote = 12;
  for (let i = 0; i < codigos.length; i += tamanoLote) {
    const lote = codigos.slice(i, i + tamanoLote);
    const datos = await Promise.all(lote.map(async codigo => {
      try {
        const snap = await getDoc(doc(db, "productos", codigo));
        if (!snap.exists()) return { codigo, existe: false, costoSinImpuesto: null, concepto: "", actualizadoLocalEn: Date.now() };
        const d = snap.data() || {};
        return {
          codigo,
          existe: true,
          costoSinImpuesto: Number.isFinite(Number(d.costoSinImpuesto)) ? Number(d.costoSinImpuesto) : null,
          concepto: repararTexto(d.concepto || ""),
          actualizadoEn: d.actualizadoEn || "",
          actualizadoLocalEn: Date.now()
        };
      } catch (error) {
        console.error(`Error consultando /productos/${codigo}:`, error);
        return { codigo, existe: false, error: true, costoSinImpuesto: null, concepto: "", actualizadoLocalEn: Date.now() };
      }
    }));
    encontrados.push(...datos);
    if ($("estadoCosteado")) $("estadoCosteado").textContent = `Consultando costos necesarios: ${Math.min(i + lote.length, codigos.length)} de ${codigos.length} códigos...`;
  }
  await guardarCostosLocales(encontrados);
  return encontrados;
}

function monedaCosteo(valor) {
  return Number(valor || 0).toLocaleString("es-MX", { style: "currency", currency: "MXN", minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

function pintarInventarioCosteado() {
  const tabla = $("tablaCosteado");
  if (!tabla) return;
  const tbody = tabla.querySelector("tbody");
  const tfoot = tabla.querySelector("tfoot");
  const totalCantidad = filasInventarioCosteado.reduce((a, x) => a + Number(x.cantidad || 0), 0);
  const totalCosto = filasInventarioCosteado.reduce((a, x) => a + Number(x.costoFinal || 0), 0);
  const faltantes = filasInventarioCosteado.filter(x => x.costoUnitario === null).length;

  tbody.innerHTML = filasInventarioCosteado.map(r => `
    <tr>
      <td class="codigo">${escapeHtml(r.codigo)}</td>
      <td>${fmtNum(r.cantidad)}</td>
      <td class="left">${escapeHtml(r.concepto)}</td>
      <td class="${r.costoUnitario === null ? 'costo-faltante' : ''}">${r.costoUnitario === null ? 'SIN COSTO' : monedaCosteo(r.costoUnitario)}</td>
      <td class="${r.costoUnitario === null ? 'costo-faltante' : ''}">${r.costoUnitario === null ? '—' : monedaCosteo(r.costoFinal)}</td>
    </tr>`).join("");

  tfoot.innerHTML = `<tr><td>TOTALES</td><td>${fmtNum(totalCantidad)}</td><td></td><td></td><td>${monedaCosteo(totalCosto)}</td></tr>`;
  if ($("estadoCosteado")) {
    $("estadoCosteado").textContent = `${filasInventarioCosteado.length} artículos · ${faltantes ? `${faltantes} sin costo localizado` : 'todos con costo'} · Los costos consultados quedan guardados en IndexedDB.`;
  }
}

async function cargarInventarioCosteado(forzarActualizacion = false) {
  const base = registrosPivot.map(r => ({
    codigo: String(r.codigoKey || r.codigo || "").trim(),
    cantidad: Number(r.existenciaFinalSemana || 0),
    conceptoTabla: repararTexto(r.nombre || "")
  })).filter(r => r.codigo);

  if (!base.length) {
    filasInventarioCosteado = [];
    pintarInventarioCosteado();
    if ($("estadoCosteado")) $("estadoCosteado").textContent = "No hay artículos calculados en la tabla actual.";
    return;
  }

  const codigos = [...new Set(base.map(x => x.codigo))];
  if ($("estadoCosteado")) $("estadoCosteado").textContent = "Leyendo costos guardados localmente...";
  const locales = forzarActualizacion ? new Map() : await leerCostosLocales(codigos);
  const faltantes = forzarActualizacion ? codigos : codigos.filter(c => !locales.has(c));

  if (faltantes.length) {
    const nuevos = await consultarProductosPorCodigo(faltantes);
    nuevos.forEach(x => locales.set(x.codigo, x));
  }

  filasInventarioCosteado = base.map(r => {
    const p = locales.get(r.codigo) || null;
    const costo = p && p.costoSinImpuesto !== null && Number.isFinite(Number(p.costoSinImpuesto)) ? Number(p.costoSinImpuesto) : null;
    return {
      codigo: r.codigo,
      cantidad: r.cantidad,
      concepto: repararTexto(p?.concepto || r.conceptoTabla || ""),
      costoUnitario: costo,
      costoFinal: costo === null ? 0 : r.cantidad * costo
    };
  });
  pintarInventarioCosteado();
}

async function abrirInventarioCosteado() {
  cerrarMenuMas();
  $("panelInventarioCosteado")?.classList.remove("oculto");
  await cargarInventarioCosteado(false);
}

function cerrarInventarioCosteado() {
  $("panelInventarioCosteado")?.classList.add("oculto");
}

function exportarInventarioCosteado() {
  if (!filasInventarioCosteado.length) {
    alert("No hay inventario costeado para descargar.");
    return;
  }
  const totalCantidad = filasInventarioCosteado.reduce((a, x) => a + Number(x.cantidad || 0), 0);
  const totalCosto = filasInventarioCosteado.reduce((a, x) => a + Number(x.costoFinal || 0), 0);
  const aoa = [
    ["CODIGO", "CANTIDAD", "CONCEPTO", "COSTO UNITARIO", "COSTO FINAL"],
    ...filasInventarioCosteado.map(r => [r.codigo, r.cantidad, r.concepto, r.costoUnitario ?? "SIN COSTO", r.costoUnitario === null ? "" : r.costoFinal]),
    ["TOTALES", totalCantidad, "", "", totalCosto]
  ];
  const ws = XLSX.utils.aoa_to_sheet(aoa);
  ws["!cols"] = [{wch:18},{wch:14},{wch:48},{wch:18},{wch:18}];
  for (let row = 2; row <= filasInventarioCosteado.length + 1; row++) {
    if (ws[`D${row}`] && typeof ws[`D${row}`].v === "number") ws[`D${row}`].z = '$#,##0.00';
    if (ws[`E${row}`] && typeof ws[`E${row}`].v === "number") ws[`E${row}`].z = '$#,##0.00';
  }
  const totalRow = filasInventarioCosteado.length + 2;
  if (ws[`E${totalRow}`]) ws[`E${totalRow}`].z = '$#,##0.00';
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Inventario costeado");
  XLSX.writeFile(wb, `inventario_costeado_abarrotes_${rangoSemanaActual.inicio}_a_${rangoSemanaActual.fin}.xlsx`);
}
...
function inicializarEventos() {
  ...
  if ($("selectorSemana")) {
    ...
    $("selectorSemana").addEventListener("change", async (event) => {
      if (!event.target.value || event.target.value < SEMANA_MINIMA) {
        ...
        alert("El inventario inicial es del 17/08/2026. No se permiten semanas anteriores.");
      }
      await cargarSalidasZapata();
      iniciarSincronizacionLigeraAjustes();
    });
  }
  ...
  $("btnInventarioCosteado")?.addEventListener("click", abrirInventarioCosteado);
  $("btnCerrarCosteado")?.addEventListener("click", cerrarInventarioCosteado);
  $("btnExportarCosteado")?.addEventListener("click", exportarInventarioCosteado);
  $("btnActualizarCostos")?.addEventListener("click", async () => {
    $("btnActualizarCostos").disabled = true;
    try { await cargarInventarioCosteado(true); }
    finally { $("btnActualizarCostos").disabled = false; }
  });
      ...
      try {
        await iniciarModuloEntradasZapata({
          ...
          getCatalogoTabla: () => {
            ...
            return registrosPivot
              .map(r => {
                const codigo = String(r.codigo || r.codigoKey || "").trim();
                const id = normalizarCodigo(r.codigoKey || codigo);
                return {
                  id,
                  codigoBarra: codigo || id,
                  concepto: String(r.nombre || r.descripcion || "").trim(),
                  marca: "",
                  departamento: "Tabla de movimientos",
                  unidadMedidaSat: ""
                };
              })
              .filter(p => p.id && !vistos.has(p.id) && vistos.add(p.id));
          }
        });
      } catch (error) {
        console.error("Error al abrir Entradas de Abarrotes:", error);
        setStatus(`Error al abrir Entradas: ${error?.message || error}`);
      }
    });
  }

  $("btnVerAjusteEntrada")?.addEventListener("click", async () => {
    cerrarMenuMas();
    mostrarPanelAjusteEntrada();
    await iniciarModuloAjusteEntradaIndependiente({ onAjusteGenerado: aplicarEntradaGeneradaLocal });
  });

  $("btnCerrarAjusteEntradaPanel")?.addEventListener("click", async () => {
    ocultarPanelAjusteEntrada();
    await cargarSalidasZapata(true);
  });

  $("btnRecargarAjusteEntrada")?.addEventListener("click", () => iniciarModuloAjusteEntradaIndependiente({ onAjusteGenerado: aplicarEntradaGeneradaLocal }));
  $("buscarAjusteEntrada")?.addEventListener("input", () => iniciarModuloAjusteEntradaIndependiente({ onAjusteGenerado: aplicarEntradaGeneradaLocal }));
...
function entradaGeneradaADetalleMovimientos(entrada) {
  ...
  const esAjusteEntrada = String(entrada.tipo_movimiento || "").toUpperCase() === "AJUSTE_ENTRADA";
  const fecha = normalizarFecha(
  ...
  const docId = esAjusteEntrada
    ? String(entrada.folioAjusteEntrada || entrada.id || "").trim()
    : String(entrada.uuid_cfdi || entrada.id || entrada.folioEntrada || "").trim();

  return articulos.map((art, idx) => {
    const codigoOriginal = String(art.codigo_interno || art.codigo || art.codigo_factura || "").trim();
    const codigoKey = normalizarCodigo(codigoOriginal);
    const nombre = String(art.descripcion_interna || art.descripcion_factura || art.nombre || "").trim();
    const cantidad = Number(esAjusteEntrada ? (art.cantidad_ajuste || 0) : (art.cantidad_entrada || 0));

    if (!codigoKey && !nombre && !cantidad) return null;

    return {
      tipo: esAjusteEntrada ? "AJUSTE_ENTRADA" : "ENTRADA",
      tipoMovimiento: esAjusteEntrada ? "AJUSTE_ENTRADA" : "ENTRADA",
      docId,
      partida: idx + 1,
      folio: String(esAjusteEntrada ? (entrada.folioAjusteEntrada || docId) : (entrada.folioEntrada || entrada.folio || docId)).trim(),
      folioEntrada: String(entrada.folioEntrada || "").trim(),
      fecha,
      hora: String(entrada.hora_movimiento || "00:00").trim(),
      destino: esAjusteEntrada ? "DEVOLUCIÓN / AJUSTE DE ENTRADA" : "ALMACÉN DE ABARROTES",
      entrega: aliasPivot,
...
function aplicarEntradaGeneradaLocal(entrada) {
  ...
  const docIdEntrada = detalleEntrada[0].docId;

  // La entrada recién creada se agrega también al cache principal.
  // Así permanece al cambiar de semana/vista y NO obliga a descargar
  // nuevamente las entradas desde Firestore.
  cacheMovimientos.entradas = cacheMovimientos.entradas.filter(
    x => String(x.docId || "") !== String(docIdEntrada || "")
  );
  cacheMovimientos.entradas.push(...detalleEntrada);

  if (fechaEntrada >= rango.inicio && fechaEntrada <= rango.fin) {
    const docId = docIdEntrada;
...
function ajusteInventarioGuardadoADetalle(payload) {
  if (!payload || !Array.isArray(payload.partidas)) return [];
  const docId = String(payload.docId || payload.folio || "").trim();
  return payload.partidas.map((p, idx) => {
    const fecha = normalizarFecha(p.fecha_movimiento || "");
    const codigo = String(p.codigo || p.codigoKey || "").trim();
    const codigoKey = normalizarCodigo(p.codigoKey || codigo);
    const cantidad = Number(p.diferencia ?? p.cantidad ?? 0);
    if (!fecha || !codigoKey || !Number.isFinite(cantidad)) return null;
    return {
      tipo: "AJUINV",
      tipoMovimiento: "AJUINV",
      docId,
      partida: p.partida || p.codigoKey || idx + 1,
      folio: String(payload.folio || docId).trim(),
      fecha,
      hora: String(p.hora_movimiento || "00:00").trim(),
      codigo,
      codigoKey,
      ...
      existencia_fisica: Number(p.existencia_fisica || 0),
      existencia_teorica: Number(p.existencia_teorica || 0),
      motivo: String(p.motivo || "AJUSTE FÍSICO DE INVENTARIO").trim()
    };
  }).filter(Boolean);
}

function reconstruirTablaDesdeCache(mensaje = "Tabla sincronizada.") {
  const rango = obtenerRangoSemana();
  const salidas = dividirMovimientosPorSemana(cacheMovimientos.salidas, rango);
  const entradasNormales = dividirMovimientosPorSemana(cacheMovimientos.entradas, rango);
  const entradasTraszap = dividirMovimientosPorSemana(cacheMovimientos.traszap, rango);
  ...
  registrosDetalleSemana = salidas.semana;
  registrosDetalleAcumuladoAnterior = salidas.anterior;
  registrosEntradasSemana = [...entradasNormales.semana, ...entradasTraszap.semana];
  registrosEntradasAcumuladoAnterior = [...entradasNormales.anterior, ...entradasTraszap.anterior];
  registrosAjustesSemana = ajustes.semana;
  registrosAjustesAcumuladoAnterior = ajustes.anterior;
  ...
  registrosDetalleMovimientoSemana = [
    ...
    if (a.fecha !== b.fecha) return String(b.fecha).localeCompare(String(a.fecha));
    if (String(a.hora || "") !== String(b.hora || "")) return String(b.hora || "").localeCompare(String(a.hora || ""));
    return String(a.tipo).localeCompare(String(b.tipo));
  });
...
function aplicarAjusteInventarioLocal(payload, mensaje = "Ajuste de inventario aplicado y tabla actualizada sin recarga completa.") {
  const detalle = ajusteInventarioGuardadoADetalle(payload);
  const docId = String(payload?.docId || payload?.folio || "").trim();
  if (!docId || !detalle.length) return;
  cacheMovimientos.ajustes = cacheMovimientos.ajustes.filter(x => String(x.docId || "") !== docId);
  cacheMovimientos.ajustes.push(...detalle);
  reconstruirTablaDesdeCache(mensaje);
}

let detenerSyncAjusteEntrada = null;
let detenerSyncAjusteInventario = null;
let claveSemanaSync = "";

function detenerSincronizacionLigera() {
  try { detenerSyncAjusteEntrada?.(); } catch (_) {}
  try { detenerSyncAjusteInventario?.(); } catch (_) {}
  detenerSyncAjusteEntrada = null;
  detenerSyncAjusteInventario = null;
  claveSemanaSync = "";
}

function iniciarSincronizacionLigeraAjustes() {
  const rango = obtenerRangoSemana();
  const clave = `${rango.inicio}|${rango.fin}`;
  if (claveSemanaSync === clave && detenerSyncAjusteEntrada && detenerSyncAjusteInventario) return;
  detenerSincronizacionLigera();
  claveSemanaSync = clave;

  // Firestore onSnapshot NO hace polling. Mantiene una sola escucha y sólo entrega cambios.
  // Limitamos ambas escuchas a la semana visible para reducir lecturas.
  const qAjusteEntrada = query(
    REF_AJUSTES_ENTRADA_ZAPATA,
    where("fecha_movimiento", ">=", rango.inicio),
    where("fecha_movimiento", "<=", rango.fin)
  );
  let primeraAjusteEntrada = true;
  detenerSyncAjusteEntrada = onSnapshot(qAjusteEntrada, snap => {
    if (primeraAjusteEntrada) { primeraAjusteEntrada = false; return; }
    for (const cambio of snap.docChanges()) {
      const id = cambio.doc.id;
      cacheMovimientos.entradas = cacheMovimientos.entradas.filter(x => String(x.docId || "") !== id);
      if (cambio.type !== "removed") {
        const data = { id, ...cambio.doc.data(), tipo_movimiento: "AJUSTE_ENTRADA" };
        cacheMovimientos.entradas.push(...entradaGeneradaADetalleMovimientos(data));
      }
    }
    reconstruirTablaDesdeCache("Ajuste de entrada sincronizado automáticamente.");
  }, error => console.warn("Sincronización ajuste de entrada:", error));

  const qAjusteInventario = query(
    REF_AJUSTES_INVENTARIO_ZAPATA,
    where("fecha_movimiento", ">=", rango.inicio),
    where("fecha_movimiento", "<=", rango.fin)
  );
  let primeraAjusteInventario = true;
  detenerSyncAjusteInventario = onSnapshot(qAjusteInventario, async snap => {
    if (primeraAjusteInventario) { primeraAjusteInventario = false; return; }
    for (const cambio of snap.docChanges()) {
      const id = cambio.doc.id;
      cacheMovimientos.ajustes = cacheMovimientos.ajustes.filter(x => String(x.docId || "") !== id);
      if (cambio.type === "removed") continue;
      const cabecera = cambio.doc.data() || {};
      const partidasSnap = await getDocs(collection(db, "almacenes", "abarrotespdd", "ajustealinventario", id, "PARTIDAS"));
      const partidas = partidasSnap.docs.map(d => ({ partida: d.id, ...d.data() })).filter(p => p.eliminado !== true);
      cacheMovimientos.ajustes.push(...ajusteInventarioGuardadoADetalle({ docId: id, folio: cabecera.folio || id, partidas }));
    }
    reconstruirTablaDesdeCache("Ajuste de inventario sincronizado automáticamente.");
  }, error => console.warn("Sincronización ajuste inventario:", error));
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  iniciarModuloAjustesInventarioZapata({
    ...
    onAjusteGuardado: async (payload) => {
      aplicarAjusteInventarioLocal(payload);
    }
  });
