...
// V22: bloqueo Grabando salida + ticket simple durante el bloqueo; PDF/Firebase/Telegram despues + catalogo automatico 24h. El arranque ya NO depende de Firebase/CDN. Todo lo remoto se carga
...
let collection,getDocs,doc,getDoc,setDoc,serverTimestamp,collectionGroup,query,where;
let storageRef,uploadBytes,getDownloadURL;
let onlineStackPromise=null;
const FIREBASE_CONFIG={apiKey:"AIzaSyCK5nb6u2CGRJ8AB1aPlRn54b97bdeAFeM",authDomain:"inventariopv-643f1.firebaseapp.com",projectId:"inventariopv-643f1",storageBucket:"inventariopv-643f1.firebasestorage.app",messagingSenderId:"96242533231",appId:"1:96242533231:web:aae75a18fbaf9840529e9a"};
...
const S={
  ...
  masterProducts:[],masterByCode:new Map(),inventoryReady:false,masterReady:false,offlineCatalogAt:null
};
...
function makePdf(salida){
  ...
  pdf.setFont('helvetica','normal');pdf.setFontSize(9);pdf.text(`Entrega: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`,left,y);y+=5;pdf.text(`Recibe: ${salida.recibe||''}`,left,y);
...
function ticketHtml(salida){
  const rows=(salida.partidas||[]).map(x=>`<div class="item"><b>${esc(x.renglon+'. '+x.descripcion)}</b><br><span>COD: ${esc(x.codigo)}</span><br><span>${x.cajasSalieron!=null&&x.cantidadPorCaja!=null?esc(x.cajasSalieron+' cj x '+x.cantidadPorCaja+' = '):''}<b>${esc(x.cantidad)} PZAS</b></span></div>`).join('');
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(salida.folio)}</title><style>
  @page{size:80mm auto;margin:2.5mm}*{box-sizing:border-box}body{width:74mm;margin:0 auto;font-family:Arial,sans-serif;color:#000;font-size:11px;line-height:1.25}.c{text-align:center}.title{font-size:16px;font-weight:800}.b{font-weight:800}.sep{border-top:1px dashed #000;margin:5px 0}.item{padding:4px 0;border-bottom:1px dashed #999}.names{font-size:12px}.tot{font-size:13px;font-weight:800}.no-print{margin:12px 0;text-align:center}@media print{.no-print{display:none}body{width:auto}}
  </style></head><body><div class="c title">SALIDA ABARROTES PDD</div><div class="c b">PROVSOFT</div><div class="sep"></div>
  <div><b>FOLIO:</b> ${esc(salida.folio)}</div><div><b>FECHA:</b> ${esc(salida.fechaCapturaTxt||salida.fechaCaptura)}</div><div><b>HORA:</b> ${esc(salida.horaLocal||'')}</div><div><b>DESTINO:</b> ${esc(salida.destino)}</div>
  <div class="sep"></div><div class="names"><b>ENTREGA:</b> ${esc(salida.entrega?.nombre||salida.entrega?.usuario||'')}</div><div class="names"><b>RECIBE:</b> ${esc(salida.recibe||'')}</div><div class="sep"></div>
  ${rows}<div class="sep"></div><div class="tot">PARTIDAS: ${esc(salida.totalPartidas)}<br>UNIDADES: ${esc(salida.totalUnidades)}</div><div class="sep"></div><div><b>ENTREGA:</b> ${esc(salida.entrega?.nombre||salida.entrega?.usuario||'')}</div><div><b>RECIBE:</b> ${esc(salida.recibe||'')}</div><br><div class="c">*** FIN DEL TICKET ***</div>
  <div class="no-print"><button onclick="window.print()" style="font-size:18px;padding:12px 24px">IMPRIMIR TICKET</button></div></body></html>`;
}
function prepareTicketWindow(){
  // Reservar la ventana desde el toque de FINALIZAR evita el bloqueo de popups.
  const w=window.open('about:blank','_blank');
  if(w){
    try{
      w.document.open();
      w.document.write('<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Preparando ticket</title><style>body{font-family:system-ui;padding:24px;text-align:center}small{color:#666}</style></head><body><h3>Preparando ticket de salida…</h3><small>La impresión se abrirá en un momento.</small></body></html>');
      w.document.close();
    }catch{}
  }
  return w;
}
function printTicketInPreparedWindow(salida,w){
  if(!w || w.closed)return false;
  try{
    w.document.open();w.document.write(ticketHtml(salida));w.document.close();w.focus();
    setTimeout(()=>{try{w.print()}catch(e){console.warn('[TICKET PRINT]',e)}},180);
    return true;
  }catch(e){console.error('[TICKET]',e);return false}
}
  ...
  try{
    ...
    for(const salidaLocal of pendingSalidasRead()){
      try{
        ...
        promoteMovedProducts(salidaLocal.partidas||[]).catch(e=>console.warn('[CATALOGO OPERATIVO]',e));
        try{
...
async function loadInventory(options={}){
  ...
  try{
    ...
    await cachePut('inventoryCatalog',{inventarioId:invId,catalog:S.catalog});
    if(!boot){
    ...
    console.warn('[CATÁLOGO] No disponible en línea; buscando catálogo offline:',e);
    const c=await cacheGet('inventoryCatalog');
    const list=Array.isArray(c?.value?.catalog)?c.value.catalog:[];
    if(list.length){
      S.catalog=list;rebuildCatalogIndex();S.inventoryReady=true;S.offlineCatalogAt=c.updatedAt;
      if(!boot){$('loadingText').textContent=`Catálogo offline cargado (${list.length.toLocaleString('es-MX')} productos).`;setTimeout(()=>{hideLoad();$('scanInput').focus()},350)}
      return true;
    }
    S.inventoryReady=false;if(!boot)hideLoad();
    if(!boot)alert('No hay internet y este equipo todavía no tiene catálogo offline. Conéctalo una vez y usa “Descargar catálogo offline”.');
    return false;
  }
}
...
async function loadActiveProducts(){
  ...
  try{
    ...
    if(S.masterReady){setBootStatus(`${list.length.toLocaleString('es-MX')} productos cargados desde respaldo offline.`);return true}
    return false;
  }
}
...
async function downloadOfflineCatalog(options={}){
  ...
  $('menu').classList.add('hidden');
  showLoad('Preparando descarga de catálogos...');
  ...
  showLoad('Descargando catálogos para uso offline...');
  $('catalogPrepareBtn')?.classList.add('hidden');$('catalogRetryBtn')?.classList.add('hidden');
  try{
    setBootStatus('1/4 Descargando productos activos...');
    const a=await loadActiveProducts();
    setBootStatus('2/4 Descargando inventario operativo...');
    const b=await loadInventory({boot:true});
    setBootStatus('3/4 Guardando empleados y configuración...');
    await Promise.allSettled([loadEmployees(),loadConfig(),loadFixedUser()]);
    const meta={updatedAt:Date.now(),masterCount:S.masterProducts.length,inventoryCount:S.catalog.length,inventarioId:S.config.inventarioId};
    if(!(a&&b&&meta.masterCount>0&&meta.inventoryCount>0))throw new Error('La descarga quedó incompleta.');
    await cachePut('offlineMeta',meta);
    setBootStatus('4/4 Catálogos listos para trabajar offline.');
    $('bootHint').textContent=`${meta.masterCount.toLocaleString('es-MX')} productos activos · ${meta.inventoryCount.toLocaleString('es-MX')} productos operativos`;
    if(startup){await new Promise(r=>setTimeout(r,500));hideLoad();showModeGate()}
    else{setTimeout(()=>hideLoad(),350);alert(`Catálogos offline actualizados correctamente.\n\nProductos activos: ${meta.masterCount.toLocaleString('es-MX')}\nCatálogo operativo: ${meta.inventoryCount.toLocaleString('es-MX')}`)}
    return true;
  }catch(e){
...
async function autoRefreshCatalogIfDue(reason='startup'){
  ...
  catalogAutoRefreshPromise=(async()=>{
    ...
    try{
      ...
      const inventoryOk=await loadInventory({boot:true});
      await Promise.allSettled([loadEmployees()]);
      const meta={updatedAt:Date.now(),masterCount:S.masterProducts.length,inventoryCount:S.catalog.length,inventarioId:S.config.inventarioId,automatico:true};
      if(!(masterOk&&inventoryOk&&meta.masterCount>0&&meta.inventoryCount>0))throw new Error('La actualización automática quedó incompleta.');
      await cachePut('offlineMeta',meta);
      S.offlineCatalogAt=meta.updatedAt;
      console.info(`[CATÁLOGO AUTO] Actualizado: ${meta.masterCount} activos / ${meta.inventoryCount} operativos.`);
...
async function loadRequiredCatalogsFromCache(){
  const [master,inv,meta]=await Promise.all([cacheGet('masterProducts'),cacheGet('inventoryCatalog'),cacheGet('offlineMeta')]);
  const masterList=Array.isArray(master?.value)?master.value:[];
  const invList=Array.isArray(inv?.value?.catalog)?inv.value.catalog:[];
  if(!masterList.length||!invList.length)return {ready:false,masterCount:masterList.length,inventoryCount:invList.length};
  ...
  S.masterReady=true;
  S.catalog=invList;rebuildCatalogIndex();S.inventoryReady=true;
  S.offlineCatalogAt=meta?.value?.updatedAt||master?.updatedAt||inv?.updatedAt||null;
  return {ready:true,masterCount:masterList.length,inventoryCount:invList.length,updatedAt:S.offlineCatalogAt};
}

async function enforceStartupCatalogProtection(){
  const state=await loadRequiredCatalogsFromCache();
  if(state.ready){
    setBootStatus('Catálogos offline verificados.');
    $('bootHint').textContent=`${state.masterCount.toLocaleString('es-MX')} productos activos · ${state.inventoryCount.toLocaleString('es-MX')} productos operativos · ${navigator.onLine?'RED DETECTADA':'ESTADO DE RED NO CONFIRMADO'}`;
    await new Promise(r=>setTimeout(r,350));hideLoad();showModeGate();return true;
  }
  setBootStatus('CATÁLOGOS REQUERIDOS');
  $('bootHint').textContent='Este equipo aún no tiene los catálogos completos. Con Wi‑Fi o datos móviles, pulsa DESCARGAR para preparar la aplicación.';
...
async function showOfflineInfo(){
  const m=await cacheGet('offlineMeta'),master=await cacheGet('masterProducts'),inv=await cacheGet('inventoryCatalog');
  const mc=Array.isArray(master?.value)?master.value.length:0,ic=Array.isArray(inv?.value?.catalog)?inv.value.catalog.length:0;
  alert(`CATÁLOGO OFFLINE\n\nProductos activos: ${mc.toLocaleString('es-MX')}\nCatálogo operativo: ${ic.toLocaleString('es-MX')}\nÚltima actualización: ${cacheAgeText(m?.value?.updatedAt||m?.updatedAt)}\nEstado del navegador: ${navigator.onLine?'EN LÍNEA':'NO CONFIRMADO'}`);
}
...
function reviewBeforeFinish(){
  ...
  open(`<h2>Revisar salida</h2><p class="review-note">Confirma los datos y las partidas antes de finalizar e imprimir el ticket.</p>${cartSummaryHtml()}${cartRowsHtml(true)}<div class="final-actions"><button id="back" class="secondary">SEGUIR CAPTURANDO</button><button id="tosign" class="primary">FINALIZAR E IMPRIMIR</button></div>`);
  $('back').onclick=close;
...
function signAndSave(){
  ...
  if(!confirm(`FINALIZAR SALIDA

Entrega: ${S.user?.nombre||S.user?.usuario||''}
Recibe: ${S.recibe}
Destino: ${S.destino}

Se grabará la salida y se abrirá el ticket simple para imprimir.`))return;

  // Reservamos la ventana de impresión mientras seguimos dentro del toque del usuario.
  const ticketWindow=prepareTicketWindow();
  showLoad('Grabando salida…');
  if($('bootHint'))$('bootHint').textContent='Protegiendo la salida e imprimiendo el ticket';

  // Dejamos un frame para que Android pinte el bloqueo antes del trabajo de cierre.
  requestAnimationFrame(()=>setTimeout(()=>{
    try{
      const now=new Date(),usuarioFolio=String(S.user?.id||S.user?.usuario||'USR').replace(/[^a-zA-Z0-9]/g,'').slice(-6).toUpperCase()||'USR',folio=`SABPDD-${now.toISOString().slice(0,10).replaceAll('-','')}-${String(now.getHours()).padStart(2,'0')}${String(now.getMinutes()).padStart(2,'0')}${String(now.getSeconds()).padStart(2,'0')}${String(now.getMilliseconds()).padStart(3,'0')}-${usuarioFolio}-${crypto.randomUUID().slice(0,4).toUpperCase()}`;
      const fechaCapturaTxt=(()=>{const [y,m,d]=S.fechaCaptura.split('-');return y&&m&&d?`${d}/${m}/${y}`:S.fechaCaptura})();
      const salida={folio,almacenId:'abarrotespdd',inventarioId:S.config.inventarioId,fechaCaptura:S.fechaCaptura,fechaCapturaTxt,entrega:{usuarioId:S.user.id,usuario:S.user.usuario||'',nombre:S.user.nombre||'',rol:S.user.rol||'',rutaId:S.user.rutaId||''},recibe:S.recibe,recibeEmpleado:S.recibeEmpleado?{...S.recibeEmpleado}:null,destino:S.destino,partidas:S.cart.map((x,i)=>({renglon:i+1,codigo:x.codigo,descripcion:x.descripcion,cantidad:x.cantidad,inventarioInicial:x.inventarioInicial,cantidadPorCaja:x.cantidadPorCaja??null,cajasSalieron:x.cajasSalieron??(Number(x.cantidadPorCaja)>0?Math.round((Number(x.cantidad)/Number(x.cantidadPorCaja)+Number.EPSILON)*1000)/1000:null),precioPublico:x.precioPublico??null,fueraInventario:x.fueraInventario===true})),totalPartidas:S.cart.length,totalUnidades:S.cart.reduce((a,x)=>a+Number(x.cantidad||0),0),fechaLocal:now.toLocaleDateString('es-MX'),horaLocal:now.toLocaleTimeString('es-MX'),creadoEnLocal:now.toISOString()};

      // 1) Grabación segura local: no depende de internet y evita perder la salida.
      if(!queueSalidaLocal(salida))throw new Error('No fue posible proteger la salida en almacenamiento local.');

      // 2) Dentro del MISMO bloqueo disparamos el ticket térmico simple.
      $('loadingText').textContent='Salida grabada · Imprimiendo ticket…';
      if($('bootHint'))$('bootHint').textContent=`${salida.folio} · Entrega: ${salida.entrega?.nombre||salida.entrega?.usuario||''} · Recibe: ${salida.recibe}`;
      const printed=printTicketInPreparedWindow(salida,ticketWindow);
      if(!printed && ticketWindow && !ticketWindow.closed)try{ticketWindow.close()}catch{}

      // 3) Limpiar la captura. Firebase/PDF/Telegram NO detienen al operador.
      close();S.cart=[];S.last=[];render();S.recibe='';S.recibeEmpleado=null;S.destino='';S.fechaCaptura='';

      setTimeout(()=>{
        hideLoad();
        if($('bootHint'))$('bootHint').textContent='Verificando catálogos disponibles en este equipo';
        showModeGate();
        toast(printed?'Salida grabada · ticket enviado a impresión':'Salida grabada · revisa permiso de ventanas para imprimir');
      },450);

      // 4) Firestore y auxiliares después, en segundo plano.
      setTimeout(()=>syncPendingSalidas(),700);
    }catch(e){
      console.error(e);
      hideLoad();
      if(ticketWindow && !ticketWindow.closed)try{ticketWindow.close()}catch{}
...
(async()=>{
  ...
  try{
    ...
    if(catalogsReady)setTimeout(()=>autoRefreshCatalogIfDue('startup'),900);
