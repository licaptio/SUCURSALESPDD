...
async function selectProduct(p){
  ...
  const abrirModalCantidad=()=>{
    ...
    const qtyInput=$('qty');
    qtyInput.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();$('nextBox').click();}};
    qtyInput.addEventListener('focus',()=>{
      setTimeout(()=>{try{qtyInput.select()}catch(_){ }},0);
    });
  ...
  const abrirModalCaja=(q)=>{
    ...
    const boxQtyInput=$('boxQty');
    boxQtyInput.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();$('add').click();}};
    // Siempre que este campo reciba el foco, seleccionar TODO el valor existente.
    // El setTimeout evita que Chrome vuelva a colocar el cursor dentro del número
    // después del focus/click, especialmente en inputs type=number.
    boxQtyInput.addEventListener('focus',()=>{
      setTimeout(()=>{try{boxQtyInput.select()}catch(_){ }},0);
    });
    boxQtyInput.addEventListener('mouseup',e=>{
      // Conserva la selección total que acabamos de hacer al entrar al campo.
      if(document.activeElement===boxQtyInput)e.preventDefault();
    });
    setTimeout(()=>{
      boxQtyInput.focus();
      setTimeout(()=>{try{boxQtyInput.select()}catch(_){ }},0);
  ...
  const abrirModalCantidadConValor=(q)=>{
    abrirModalCantidad();
    setTimeout(()=>{const el=$('qty');if(el){el.value=q;el.focus();el.select();}},0);
  };
