...
import {
  ...
  filtrarFacturasPendientesParaCigarro,
  marcarFacturaNoEntraCigarro,
  cargarHistorialCigarro
} from "./facturas.js";

import {
  prepararArticulosEntrada,
  generarEntradaCigarro,
...
export async function iniciarModuloEntradasCigarro(opciones = {}) {
...
export async function refrescarModuloEntradasCigarro(conModal = false) {
...
async function cargarFacturasUI(conModal = true) {
  ...
  try {
    ...
    facturasPendientes = await filtrarFacturasPendientesParaCigarro(
      facturas,
      proveedores
    );
...
async function cargarEntradasUI() {
  ...
  try {
    if (!entradasCacheCargado) {
      entradasCache = await cargarEntradasCigarro(100);
    ...
    entradas.forEach(e => {
      ...
      div.innerHTML = `
        ...
        <p><span class="badge badge-ok">${escapeHtml(e.estado_cigarro || e.estado || "")}</span></p>
      `;
...
async function cargarHistorialUI() {
  ...
  try {
    if (!historialCacheCargado) {
      historialCache = await cargarHistorialCigarro(200);
...
function pintarFacturas() {
  ...
  if (facturasPendientes.length === 0) {
    contenedor.innerHTML = `
      <p>
        No hay facturas pendientes para Cigarro.
        Revisa proveedores autorizados, fecha mínima 23/06/2026, entradas ya generadas o facturas marcadas como no entra.
      </p>
    `;
    return;
  }
...
async function generarEntradaUI() {
  ...
  const ok = await confirmarProvsoft({
    titulo: "Generar entrada Cigarro",
    mensaje: "¿Generar entrada Cigarro con esta factura? La factura saldrá de pendientes y la tabla se actualizará en memoria local.",
  ...
  mostrarCargando("Generando entrada Cigarro...");

  try {
    const entrada = await generarEntradaCigarro(
      facturaSeleccionada,
      articulosPreparados,
      "GERARDO"
    );
...
async function marcarNoEntraUI() {
  ...
  const ok = await confirmarProvsoft({
    titulo: "Factura no entra a Cigarro",
    mensaje: "¿Marcar esta factura como NO ENTRA A CIGARRO? Ya no aparecerá en pendientes, pero quedará en historial.",
  ...
  mostrarCargando("Marcando factura como no entra a Cigarro...");

  try {
    const decision = await marcarFacturaNoEntraCigarro(
      facturaSeleccionada,
      motivo,
      "GERARDO"
    );
    ...
    notificar("Factura marcada como no entra a Cigarro", "ok");
  ...
  pantalla.innerHTML = `
    ...
    <div class="revision-footer">
      <div class="bloque-decision revision-motivo">
        <label for="motivoNoEntra">Motivo si esta factura no entra a Cigarro</label>
        <textarea id="motivoNoEntraPantalla" placeholder="Ejemplo: factura no corresponde a mercancía de Cigarro, mercancía para otra ubicación, etc."></textarea>
      </div>
      <div class="revision-actions">
        ...
        <button id="btnGenerarRevision" class="principal" type="button">Generar entrada Cigarro</button>
      </div>
    </div>
  `;
