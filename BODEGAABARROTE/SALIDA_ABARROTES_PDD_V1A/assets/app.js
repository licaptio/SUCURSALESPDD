const APP_BUILD_VERSION='V32-AHORRO-FIREBASE';
...
// V26: bloqueo inmediato al CONTINUAR + PDF térmico real 80 mm + sincronización de salidas + actualización automática por documento centinela de versión. El arranque ya NO depende de Firebase/CDN. Todo lo remoto se carga
...
async function manualCatalogUpdate(){
  ...
  try{
    ...
    await Promise.allSettled([loadBlockedCodes(true),loadEmployees(true),loadConfig(true),loadFixedUser(true)]);
    hideLoad();
...
$('offlineDownloadBtn').onclick=manualCatalogUpdate;
