...
const LS_BORRADOR = "provsoft_merma_osito_borrador";
const DB_NAME = "provsoft_catalogo_mermas_osito";
const STORE_PRODUCTOS = "productos";
const LS_CATALOGO_FECHA = "provsoft_merma_osito_catalogo_fecha";
const LS_PENDIENTES = "provsoft_merma_osito_pendientes_firebase";
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_OSITO, APP_CONFIG.SUBCOLECCION_MERMAS, documento.folio),
    docOnline,
    { merge:true }
  );
}
...
function recuperarBorrador(){
  ...
  // Solo conservar folios válidos de MERMAS OSITO.
  const folioRecuperado = String(b.folio || "");
  document.getElementById("folio").value = folioRecuperado.startsWith("MER-OSITO-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoMerma(){
  ...
  return {
    ...
    ositoColeccion:APP_CONFIG.TIENDA_OSITO,
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de merma OSITO generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoMerma){
  ...
  pdf.setFontSize(15);
  pdf.text("COMPROBANTE DE SOLICITUD DE MERMA OSITO",105,y,{align:"center"});

  y += 8;
  pdf.setFontSize(10);
  pdf.setFont("helvetica","normal");
  pdf.text("Documento de levantamiento de merma OSITO - destino MATRIZ",105,y,{align:"center"});
