...
async function guardarAjuste() {
  ...
  try {
    await addDoc(REF_AJUSTES_RUTA1, {
      fecha,
      hora,
...
function pintarPivotPorSemana() {
  ...
  tbody.innerHTML = rows.map((r) => `
    <tr>
      ...
      <td class="cantidad ajuste-total ${Number(r.totalAjustesSemana) > 0 ? "positivo" : Number(r.totalAjustesSemana) < 0 ? "negativo" : ""}">${fmtNum(r.totalAjustesSemana)}</td>
  ...
  tfoot.innerHTML = `
    <tr>
      ...
      <td class="${totalAjustesSemana > 0 ? "positivo" : totalAjustesSemana < 0 ? "negativo" : ""}">${fmtNum(totalAjustesSemana)}</td>
...
function pintarDetalle() {
  ...
  tfoot.innerHTML = `
    ...
    <tr>
      <td class="left" colspan="12">TOTAL AJUSTES SEMANA</td>
...
function exportarExcel() {
  ...
  if (vistaActual === "detalle") {
    ...
    rows = registrosPivot.filter(pasaFiltroPivot).map((r) => {
      ...
      obj["TOTAL AJUSTES"] = Number(r.totalAjustesSemana || 0);
...
function obtenerFilasConteo() {
  return registrosPivot
    .filter(pasaFiltroPivot)
    .map(item => ({
      codigo: item.codigo || item.codigoKey || "",
      descripcion: item.descripcion || item.nombre || "",
      teorico: Number(item.inventarioFinal ?? item.existenciaFinal ?? item.totalFinal ?? 0)
    }));
}

function pintarConteoFisico() {
  const body = $("tbodyConteo");
  if (!body) return;
  const existentes = new Map(
    [...body.querySelectorAll("tr")].map(tr => [tr.dataset.codigo, tr.querySelector("input")?.value || ""])
  );
  body.innerHTML = "";
  const frag = document.createDocumentFragment();
  for (const item of obtenerFilasConteo()) {
    const tr = document.createElement("tr");
    tr.dataset.codigo = item.codigo;
    tr.innerHTML = `
      <td class="left">${escapeHtml(item.codigo)}</td>
      <td class="left">${escapeHtml(item.descripcion)}</td>
      <td>${fmtNum(item.teorico)}</td>
      <td><input class="input-conteo" type="number" step="0.01" min="0" value="${escapeHtml(existentes.get(item.codigo) || "")}" /></td>
      <td class="diferencia-conteo"></td>`;
    const input = tr.querySelector("input");
    const dif = tr.querySelector(".diferencia-conteo");
    const actualizar = () => {
      const fisico = input.value === "" ? null : Number(input.value);
      dif.textContent = fisico === null || !Number.isFinite(fisico) ? "" : fmtNum(fisico - item.teorico);
      dif.classList.toggle("negativo", fisico !== null && fisico - item.teorico < 0);
    };
  ...
  $("estadoConteo").textContent = `${body.children.length} productos preparados para conteo.`;
}
...
async function guardarConteoFisico() {
  const filas = [...$("tbodyConteo").querySelectorAll("tr")];
  const detalle = filas.map(tr => {
    const input = tr.querySelector("input");
    if (input.value === "") return null;
    const codigo = tr.dataset.codigo;
    const item = obtenerFilasConteo().find(x => x.codigo === codigo);
    const fisico = Number(input.value);
    return { codigo, descripcion: item?.descripcion || "", teorico: item?.teorico || 0, fisico, diferencia: fisico - (item?.teorico || 0) };
  }).filter(Boolean);
  if (!detalle.length) { alert("Captura al menos un físico."); return; }
  const btn = $("btnGuardarConteo");
  btn.disabled = true;
  try {
    const fecha = hoyISO();
    await addDoc(REF_CONTEOS_RUTA1, {
      ruta: APP_CONFIG.nombre,
      rutaId: APP_CONFIG.rutaIdVentas,
      fecha,
      semana: $("selectorSemana").value,
      detalle,
      totalProductos: detalle.length,
      creadoEn: serverTimestamp()
    });
    $("estadoConteo").textContent = `Conteo guardado: ${detalle.length} productos.`;
    alert("Conteo físico guardado correctamente.");
  } catch (error) {
    console.error(error);
