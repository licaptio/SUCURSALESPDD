...
const LS_BORRADOR = "provsoft_merma_provileon_borrador";
const DB_NAME = "provsoft_catalogo_mermas_provileon";
const STORE_PRODUCTOS = "productos";
const LS_CATALOGO_FECHA = "provsoft_merma_provileon_catalogo_fecha";
const LS_PENDIENTES = "provsoft_merma_provileon_pendientes_firebase";
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_PROVILEON, APP_CONFIG.SUBCOLECCION_MERMAS, documento.folio),
    docOnline,
    { merge:true }
  );
}
...
function recuperarBorrador(){
  ...
  // Solo conservar folios válidos de MERMAS PROVILEON.
  const folioRecuperado = String(b.folio || "");
  document.getElementById("folio").value = folioRecuperado.startsWith("MER-PROVILEON-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoMerma(){
  ...
  return {
    ...
    tiendaColeccion:APP_CONFIG.TIENDA_PROVILEON,
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de merma PROVILEON generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoMerma){
  ...
  pdf.setFontSize(15);
  pdf.text("COMPROBANTE DE SOLICITUD DE MERMA PROVILEON",105,y,{align:"center"});

  y += 8;
  pdf.setFontSize(10);
  pdf.setFont("helvetica","normal");
  pdf.text("Documento de levantamiento de merma PROVILEON - destino MATRIZ",105,y,{align:"center"});
