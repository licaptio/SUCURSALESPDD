...
// V23: impresion por Android Print Service desde la misma PWA; ticket termico primero, Firebase/PDF/Telegram despues + catalogo automatico 24h. El arranque ya NO depende de Firebase/CDN. Todo lo remoto se carga
...
function ticketPrintMarkup(salida){
  const rows=(salida.partidas||[]).map(x=>`<div class="pt-item"><b>${esc(x.renglon+'. '+x.descripcion)}</b><div>COD: ${esc(x.codigo)}</div><div>${x.cajasSalieron!=null&&x.cantidadPorCaja!=null?esc(x.cajasSalieron+' cj x '+x.cantidadPorCaja+' = '):''}<b>${esc(x.cantidad)} PZAS</b></div></div>`).join('');
  return `<div class="pt-ticket">
    <div class="pt-center pt-title">SALIDA ABARROTES PDD</div>
    <div class="pt-center"><b>PROVSOFT</b></div>
    <div class="pt-sep"></div>
    <div><b>FOLIO:</b> ${esc(salida.folio)}</div>
    <div><b>FECHA:</b> ${esc(salida.fechaCapturaTxt||salida.fechaCaptura)}</div>
    <div><b>HORA:</b> ${esc(salida.horaLocal||'')}</div>
    <div><b>DESTINO:</b> ${esc(salida.destino)}</div>
    <div class="pt-sep"></div>
    <div class="pt-name"><b>ENTREGA:</b> ${esc(salida.entrega?.nombre||salida.entrega?.usuario||'')}</div>
    <div class="pt-name"><b>RECIBE:</b> ${esc(salida.recibe||'')}</div>
    <div class="pt-sep"></div>
    ${rows}
    <div class="pt-sep"></div>
    <div class="pt-total">PARTIDAS: ${esc(salida.totalPartidas)}<br>UNIDADES: ${esc(salida.totalUnidades)}</div>
    <div class="pt-sep"></div>
    <div><b>ENTREGA:</b> ${esc(salida.entrega?.nombre||salida.entrega?.usuario||'')}</div>
    <div><b>RECIBE:</b> ${esc(salida.recibe||'')}</div>
    <div class="pt-sep"></div>
    <div class="pt-center">*** SALIDA REGISTRADA ***</div>
  </div>`;
}
function ensureAndroidPrintHost(){
  let host=document.getElementById('androidPrintHost');
  if(!host){
    host=document.createElement('div');host.id='androidPrintHost';host.setAttribute('aria-hidden','true');document.body.appendChild(host);
  }
  if(!document.getElementById('androidPrintStyles')){
    const st=document.createElement('style');st.id='androidPrintStyles';st.textContent=`
      #androidPrintHost{display:none}
      @media print{
        @page{size:80mm auto;margin:2.5mm}
        body>*:not(#androidPrintHost){display:none!important}
        #androidPrintHost{display:block!important;position:static!important;width:75mm!important;margin:0!important;padding:0!important;background:#fff!important;color:#000!important}
        #androidPrintHost,.pt-ticket{font-family:Arial,Helvetica,sans-serif!important;font-size:11px!important;line-height:1.25!important}
        .pt-ticket{width:75mm!important;margin:0!important;padding:0!important}
        .pt-center{text-align:center!important}.pt-title{font-size:16px!important;font-weight:800!important}
        .pt-sep{border-top:1px dashed #000!important;margin:4px 0!important}
        .pt-item{padding:3px 0!important;border-bottom:1px dashed #999!important;break-inside:avoid!important}
        .pt-name{font-size:12px!important}.pt-total{font-size:13px!important;font-weight:800!important}
      }`;
    document.head.appendChild(st);
  }
  return host;
}
function printTicketAndroidService(salida){
  try{
    const host=ensureAndroidPrintHost();
    host.innerHTML=ticketPrintMarkup(salida);
    // window.print() aqui significa imprimir la ventana/pagina web actual. En Android
    // abre el servicio de impresion del sistema (RawBT Print Service si esta habilitado).
    window.print();
    setTimeout(()=>{try{host.innerHTML=''}catch{}},1500);
    return true;
  }catch(e){
    console.error('[ANDROID PRINT SERVICE]',e);return false;
  }
}
...
function signAndSave(){
  ...
  if(!confirm(`FINALIZAR SALIDA\n\nEntrega: ${S.user?.nombre||S.user?.usuario||''}\nRecibe: ${S.recibe}\nDestino: ${S.destino}\n\nSe grabará la salida y enseguida Android abrirá la impresión del ticket.`))return;

  showLoad('Grabando salida…');
  if($('bootHint'))$('bootHint').textContent='Protegiendo la salida antes de imprimir';

  // Construir y proteger la salida inmediatamente. No esperamos Firebase ni PDF.
  try{
    ...
    $('loadingText').textContent='Salida grabada · Abriendo impresión…';
    if($('bootHint'))$('bootHint').textContent=`${salida.folio} · Entrega: ${salida.entrega?.nombre||salida.entrega?.usuario||''} · Recibe: ${salida.recibe}`;

    // Dar un frame al navegador para pintar el bloqueo y luego usar EL SERVICIO DE IMPRESION DE ANDROID.
    // No se abre RawBT mediante rawbt:, no hay popup y no se imprime el PDF.
    requestAnimationFrame(()=>setTimeout(()=>{
      const printed=printTicketAndroidService(salida);

      // window.print() regresa cuando el usuario cierra/confirma el panel de impresion.
      ...
      showModeGate();
      toast(printed?'Salida grabada · ticket enviado al servicio de impresión':'Salida grabada · no se pudo abrir el servicio de impresión');

      // Firebase primero; PDF/Telegram se generan después desde syncPendingSalidas().
      setTimeout(()=>syncPendingSalidas(),250);
    },80));
  }catch(e){
    console.error(e);hideLoad();alert('No se pudo finalizar la salida. '+(e?.message||''));
  }
}
