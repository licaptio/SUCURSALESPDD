...
function makePdf(salida){
  ...
  const info=[`Folio: ${salida.folio}`,`Fecha: ${salida.fechaLocal} ${salida.horaLocal}`,`Entrega: ${salida.entrega.nombre||salida.entrega.usuario}`,`Recibe: ${salida.recibe}`,`Destino: ${salida.destino}`,`Inventario: ${salida.inventarioId}`];
...
async function login(){
  ...
  $('go').onclick=async()=>{
    ...
    try{
      ...
      $('sesionTxt').textContent=S.user.nombre||S.user.usuario;close();await askReceiver();
...
async function selectProduct(p){
  ...
  const caja=Number(info.cantidadPorCaja);
  open(`<h2>Producto seleccionado</h2>
    ...
    <input id="qty" class="field" type="number" min="0.001" step="0.001" inputmode="decimal">
    ...
    <input id="boxQty" class="field" type="number" min="1" step="1" inputmode="numeric" value="${Number.isFinite(caja)&&caja>0?esc(caja):''}" placeholder="Ej. 12, 24, 36...">
    <div class="box-help">Este dato se guarda en el catálogo del producto y servirá para manejar salidas por cajas más adelante.</div>
    <div class="actions"><button id="cancel" class="secondary">Cancelar</button><button id="add" class="primary">Agregar</button></div>`);
  $('cancel').onclick=close;bindCalculator('qty');
  $('add').onclick=async()=>{
    const q=Number($('qty').value),boxQty=Number($('boxQty').value);
    if(!(q>0))return alert('Captura una cantidad válida.');
    if($('boxQty').value.trim()&&!(boxQty>0))return alert('La cantidad por caja debe ser mayor a cero.');
    ...
    $('add').disabled=true;
    try{
      if(boxQty>0 && boxQty!==caja)await saveBoxQty(p.codigo,boxQty);
      if(ex){ex.cantidad=totalPropuesto;if(boxQty>0)ex.cantidadPorCaja=boxQty;if(Number.isFinite(precio))ex.precioPublico=precio}
      else S.cart.push({...p,cantidad:q,cantidadPorCaja:boxQty>0?boxQty:(Number.isFinite(caja)?caja:null),precioPublico:Number.isFinite(precio)?precio:null});
      S.last=S.last.filter(x=>x.codigo!==p.codigo);S.last.unshift({codigo:p.codigo,ts:Date.now()});S.last=S.last.slice(0,20);
      render();close();$('scanInput').value='';clearQuickResults();$('scanInput').focus()
    }catch(e){console.error(e);alert('No se pudo guardar la cantidad por caja en el catálogo. La partida no fue agregada.');$('add').disabled=false}
  };
...
function editCartQuantity(index,returnTo='cart'){
  ...
  if(!item)return;
  open(`<h2>Modificar cantidad</h2><p><b>${esc(item.descripcion)}</b><br><small>${esc(item.codigo)} · Inventario inicial: ${item.inventarioInicial}</small></p><label>Nueva cantidad</label><input id="editQty" class="field" type="number" min="0.001" step="0.001" inputmode="decimal" value="${esc(item.cantidad)}"><button id="openCalc" class="secondary calc-open">ABRIR CALCULADORA</button><div id="calcPanel" class="calculator hidden"><input id="calcExpr" class="field calc-expression" inputmode="decimal" placeholder="Ej. 12*24"><div id="calcResult" class="calc-result">Resultado: —</div><div class="calc-grid">${['7','8','9','/','4','5','6','*','1','2','3','-','0','.','(',')'].map(k=>`<button type="button" data-k="${k}">${k==='*'?'×':k==='/'?'÷':k}</button>`).join('')}</div><div class="calc-actions"><button id="calcBack" class="secondary">BORRAR 1</button><button id="calcClear" class="secondary">LIMPIAR</button><button id="calcEqual" class="secondary">=</button></div><button id="calcRegister" class="primary">REGISTRAR CÁLCULO EN CANTIDAD</button></div><div class="actions"><button id="editCancel" class="secondary">Cancelar</button><button id="editSave" class="primary">GUARDAR</button></div>`);
  const goBack=()=>{if(returnTo==='review')reviewBeforeFinish();else cartModal()};
  $('editCancel').onclick=goBack;bindCalculator('editQty');
  $('editSave').onclick=()=>{
...
<button id="clear" class="secondary">BORRAR FIRMA</button>
  ...
  $('save').onclick=async()=>{
    ...
    try{
      ...
      S.cart=[];S.last=[];render();S.recibe='';S.destino='';await askReceiver();
    }catch(e){
...
(async()=>{await loadConfig();hideLoad();$('app').classList.remove('hidden');const cached=sessionStorage.getItem('salidaPddUser');if(cached){try{S.user=JSON.parse(cached);$('sesionTxt').textContent=S.user.nombre||S.user.usuario;await askReceiver();return}catch{}}await login()})();
