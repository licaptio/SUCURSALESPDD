...
async function downloadOfflineCatalog(options={}){
  ...
  $('menu').classList.add('hidden');
  showLoad('Comprobando conexión y preparando catálogos...');
  const reachable=await hasUsableInternet();
  if(!reachable){
    if(startup){setBootStatus('NO HAY CONEXIÓN DISPONIBLE');$('bootHint').textContent='No se pudo alcanzar internet. Puedes usar Wi‑Fi o datos móviles; no se exige Wi‑Fi.';$('catalogPrepareBtn').classList.remove('hidden');$('catalogRetryBtn').classList.remove('hidden');return false}
    hideLoad();alert('No se pudo alcanzar internet en este momento. Puedes usar Wi‑Fi o datos móviles.');return false;
  }
