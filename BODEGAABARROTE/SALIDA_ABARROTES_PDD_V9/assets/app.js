...
async function askInstallBefore(action){
  if(isStandalonePwa())return action();
  ...
  $('installCancel').onclick=()=>{close();action()};
  $('installAccept').onclick=async()=>{
    const p=deferredInstallPrompt;close();
    if(p){p.prompt();try{await p.userChoice}catch{}deferredInstallPrompt=null}
    else toast('La instalación no está disponible en este momento');
    action();
  };
}
...
$('modeEntrada').onclick=()=>askInstallBefore(enterEntrada);
$('modeSalida').onclick=()=>askInstallBefore(enterSalida);
$('entryPhotoBtn').onclick=()=>{
  ...
  try{if(typeof input.showPicker==='function')input.showPicker();else input.click()}catch(e){console.warn('[FOTO] showPicker fallback',e);input.click()}
};
$('entryPhotoInput').addEventListener('change',e=>addEntryFiles(e.target.files));
$('entrySave').onclick=saveEntry;
$('entryCancel').onclick=()=>{if(ENTRY.photos.length&&!confirm('¿Cancelar esta entrada? Las fotos capturadas no se guardarán.'))return;resetEntry();showModeGate()};
...
(async()=>{
  ...
  try{
    await Promise.all([loadConfig(),loadFixedUser()]);
