...
const LS_BORRADOR = "provsoft_devolucion_allende2_borrador";
const DB_NAME = "provsoft_catalogo_devoluciones_allende2";
const STORE_PRODUCTOS = "productos";
const LS_CATALOGO_FECHA = "provsoft_devolucion_allende2_catalogo_fecha";
const LS_PENDIENTES = "provsoft_devolucion_allende2_pendientes_firebase";
...
async function iniciarApp(){
  ...
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_ALLENDE 2;
...
async function prepararCatalogoInicio(){
  actualizarLoader(
    "Preparando Devoluciones ALLENDE 2",
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_ALLENDE 2, APP_CONFIG.SUBCOLECCION_DEVOLUCIONES, documento.folio),
    docOnline,
    { merge:true }
  );
}
      ...
      try{
        ...
        try{
          ...
          await enviarPDFTelegram(pdfBlob,{
            ...
            tienda: registro.documento.tiendaOrigen || APP_CONFIG.TIENDA_ALLENDE 2,
...
function generarFolio(){
  const limpio = APP_CONFIG.TIENDA_ALLENDE 2;
  const f = new Date();
...
function recuperarBorrador(){
  ...
  // ALLENDE 2 es fija: nunca recuperar una tienda heredada de otra app.
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_ALLENDE 2;
  document.getElementById("encargado").value = b.encargado || "";

  // Solo conservar folios válidos de DEVOLUCIONES ALLENDE 2.
  const folioRecuperado = String(b.folio || "").trim();
  document.getElementById("folio").value = folioRecuperado.startsWith("DEV-ALLENDE2-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoDevolucion(){
  const t = calcularTotales();
  const tienda = APP_CONFIG.TIENDA_ALLENDE 2;
  const encargado = document.getElementById("encargado").value.trim();
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de devolución ALLENDE 2 generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoDevolucion){
  ...
  pdf.text("Documento de alta de devolución desde ALLENDE 2 a MATRIZ",105,y,{align:"center"});
