...
const CACHE_VERSION_INVENTARIO = "cig-v6-invini-20260623-ajustes-cigman-fix";
...
async function cargarSalidasZapata() {
  try {
    ...
    if (cache) {
      ...
      setProgress(100);
      setStatus(
        `Consulta local lista. Semana: ${fechaCorta(rango.inicio)} a ${fechaCorta(rango.fin)}. Cálculos cargados desde IndexedDB.`
      );
      ocultarLoader();
      return;
    }
    ...
    const consultaCigmanSemana = await consultarCigman(rango.inicio, rango.fin);
    ...
    let consultaCigmanAcumuladoAnterior = { detalle: [], totalDocs: 0 };
    ...
    if (rango.acumuladoAnteriorFin >= FECHA_BASE_INVENTARIO) {
      ...
      consultaCigmanAcumuladoAnterior = await consultarCigman(FECHA_BASE_INVENTARIO, rango.acumuladoAnteriorFin);
    }
    ...
    registrosCigmanSemana = consultaCigmanSemana.detalle;
    registrosCigmanAcumuladoAnterior = consultaCigmanAcumuladoAnterior.detalle;

    const registrosInviniSemana = crearMovimientosInventarioInicialSemana(rango.inicio, rango.fin);

    registrosDetalleMovimientoSemana = [
      ...registrosInviniSemana,
    ...
    setProgress(100);
    setStatus(
      `Consulta lista. Semana: ${fechaCorta(rango.inicio)} a ${fechaCorta(rango.fin)}. Entradas: ${registrosEntradasSemana.length}. Salidas: ${registrosDetalleSemana.length}. Ajustes: ${registrosAjustesSemana.length}. Cálculo guardado en IndexedDB.`
    );
...
mapa.forEach((row) => {
  row.inviniSemana =
    ...
    Number(row.cigmanAcumuladoAnterior || 0);
...
function pintarPivotPorSemana() {
  ...
  thead.innerHTML = `
    <tr>
      <th class="left">Código</th>
      <th class="left">Nombre</th>
      <th>INVINI<br>${fechaCorta(ID_INVENTARIO_INICIAL)}</th>
...
${
  ...
  rows.forEach(r => {
    fechasColumnas.forEach(f => {
      ...
      totalCigmanPorFecha[f] += Number((r.cigmanPorFecha?.[f]) || 0);
  ...
  const totalCigmanSemana = rows.reduce((sum, r) => sum + Number(r.totalCigmanSemana || 0), 0);
    ...
    <tr>
      ...
      ${fechasColumnas.map(f => {
        ...
        return `
          ...
          ${tieneCigman ? `<td class="cigman-col">${fmtNum(totalCigmanPorFecha[f])}</td>` : ""}
  ...
  if (vistaActual === "listado_proveedor") {
    ...
    rows = registrosPivot.filter(pasaFiltroPivot).map((r) => {
      const obj = {
        Codigo: r.codigo,
        Nombre: r.nombre,
        [`INVINI ${fechaCorta(ID_INVENTARIO_INICIAL)}`]: Number(r.inviniSemana || 0)
      };
...
async function aplicarCigmanGuardadoLocal(mov) {
  ...
  const registrosInviniSemana = crearMovimientosInventarioInicialSemana(rango.inicio, rango.fin);
  registrosDetalleMovimientoSemana = [...registrosInviniSemana, ...registrosEntradasSemana, ...registrosCigmanSemana, ...registrosDetalleSemana, ...registrosAjustesSemana]
