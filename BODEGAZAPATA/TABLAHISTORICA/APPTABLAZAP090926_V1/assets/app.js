...
const FECHA_BASE_INVENTARIO = "2026-09-09";
const FECHA_INICIO_MINIMA = "2026-09-09";
const CONTEO_ID_INVENTARIO = "ZAPATA090926";
const HORA_CORTE_INVENTARIO = "12:24";
const HORA_INICIO_OPERACION = "12:25";
...
let cargaInicialEnCurso = true;
let intervaloContadorCarga = null;
...
// Sincronización ligera: la app actualiza en memoria sus propios cambios y
// solo relee Firestore cuando detecta un cambio externo en la semana visible.
let desuscribirEscuchasSemana = [];
let temporizadorSyncRemota = null;
let ultimaActualizacionLocal = 0;
...
function extraerHoraMovimiento(...valores) {
  for (const valor of valores) {
    if (!valor) continue;

    if (typeof valor === "string") {
      const m = valor.match(/(?:T|\s)(\d{2}):(\d{2})/);
      if (m) return `${m[1]}:${m[2]}`;
      const h = valor.match(/^(\d{1,2}):(\d{2})/);
      if (h) return `${String(h[1]).padStart(2, "0")}:${h[2]}`;
    }
...
function movimientoPerteneceNuevaBase(fecha, hora = "") {
  if (!fecha || fecha < FECHA_BASE_INVENTARIO) return false;
  if (fecha > FECHA_BASE_INVENTARIO) return true;
  // 09/09/2026: la nueva operación inicia exactamente a las 12:25 PM.
  if (!hora) return false;
  return String(hora).substring(0, 5) >= HORA_INICIO_OPERACION;
}
...
function ocultarLoader() {
  if (cargaInicialEnCurso) return;
  const loader = $("loader");
  if (loader) loader.classList.add("hide");
}

function segundosDesdeApertura() {
  const inicio = Number(window.__provsoftInicioCarga || 0);
  return Math.max(0, (performance.now() - inicio) / 1000);
}

function actualizarContadorCarga(textoFinal = "") {
  const timer = $("loaderTimer");
  if (!timer) return;
  const segundos = segundosDesdeApertura().toFixed(1);
  timer.textContent = textoFinal || `Tiempo transcurrido: ${segundos} segundos`;
}

function iniciarContadorCarga() {
  actualizarContadorCarga();
  intervaloContadorCarga = window.setInterval(actualizarContadorCarga, 100);
}

function detenerContadorCarga() {
  if (intervaloContadorCarga) window.clearInterval(intervaloContadorCarga);
  intervaloContadorCarga = null;
  actualizarContadorCarga(`Aplicación lista en ${segundosDesdeApertura().toFixed(1)} segundos`);
}
...
function obtenerRangoSemana() {
  ...
  let rango = obtenerRangoDomingoSabadoDesdeWeek(valorWeek);

  // Esta versión NO navega antes del nuevo inventario inicial.
  if (rango.fin < FECHA_BASE_INVENTARIO) {
    alert("Esta versión inicia en INVENTARIO INICIAL 09/09/2026 12:25 PM. Para consultar fechas anteriores usa la versión histórica.");
    if (selectorSemana) selectorSemana.value = "2026-W37";
    rango = obtenerRangoDomingoSabadoDesdeWeek("2026-W37");
  }
...
function pintarListadoResumenProveedor() {
  ...
  tbody.innerHTML = rows.map((r) => `
    <tr>
      ...
      <td class="cantidad teorico-celda ${Number(r.existenciaFinalSemana || 0) === 0 ? "teorico-cero-finalizado" : ""}" title="Teórico final calculado">${fmtNum(r.existenciaFinalSemana)}</td>
    </tr>
  `).join("");
...
async function consultarSalidas(inicio, fin) {
  ...
  snap.forEach((documento) => {
    ...
    const hora = extraerHoraMovimiento(data.hora, data.hora_movimiento, data.creado_en, data.timestamp);
    if (!movimientoPerteneceNuevaBase(fecha, hora)) return;
...
async function consultarAjustesInventario(inicio, fin) {
  ...
  // CORTE 09/09/2026: no recorrer la colección histórica completa.
  // Solo se consultan encabezados de ajustes dentro del rango requerido.
  ...
  for (const ajusteDoc of snapAjustes.docs) {
    ...
    partidasSnap.forEach((partidaDoc) => {
      ...
      const horaMovimiento = String(p.hora_movimiento || ajusteData.hora_movimiento || "").trim();
      if (!movimientoPerteneceNuevaBase(fecha, horaMovimiento)) return;
...
? `<th class="ajuste-head">
  ...
  tbody.innerHTML = rows.map((r) => `
    <tr>
      <td class="left codigo">${escapeHtml(r.codigo)}</td>
      <td class="left">${escapeHtml(r.nombre)}</td>
      <td class="cantidad teorico-celda ${Number(r.inviniSemana || 0) === 0 ? "teorico-cero-finalizado" : ""}" title="Teórico inicial calculado">${fmtNum(r.inviniSemana)}</td>
...
function cambiarVista(vista) {
  if (vista === "detalle") vista = "resumen";
  vistaActual = vista;
...
function inicializarEventos() {
  ...
  $("btnMenuSuperior")?.addEventListener("click", (ev) => {
    ev.stopPropagation();
    $("menuSuperiorDesplegable")?.classList.toggle("oculto");
  });

  $("menuSuperiorDesplegable")?.addEventListener("click", (ev) => ev.stopPropagation());

  document.addEventListener("click", () => {
    $("menuSuperiorDesplegable")?.classList.add("oculto");
  });

  if ($("selectorSemana")) {
    $("selectorSemana").value = obtenerSemanaActualInput();
    $("selectorSemana").addEventListener("change", async () => {
      await cargarSalidasZapata();
      iniciarEscuchaLigeraSemanaVisible();
    });
  }
  ...
  $("btnConfiguracionGeneral")?.addEventListener("click", () => {
    $("menuSuperiorDesplegable")?.classList.add("oculto");
    $("modalConfiguracionGeneral")?.classList.remove("oculto");
  });
  $("btnCerrarConfiguracionGeneral")?.addEventListener("click", () => {
    $("modalConfiguracionGeneral")?.classList.add("oculto");
  });
  $("modalConfiguracionGeneral")?.addEventListener("click", (ev) => {
    if (ev.target?.id === "modalConfiguracionGeneral") {
      $("modalConfiguracionGeneral")?.classList.add("oculto");
    }
  });

  if ($("tabResumen")) $("tabResumen").textContent = "Pivot semanal";
  ...
  $("btnCerrarAjustesInventario")?.addEventListener("click", () => {
    ocultarPanelAjustesInventario();
  });

  $("btnCerrarEntradasZapata")?.addEventListener("click", () => {
...
function entradaGeneradaADetalleMovimientos(entrada) {
  ...
  const hora = extraerHoraMovimiento(entrada.hora, entrada.hora_movimiento, entrada.creado_en, entrada.timestamp);
  if (!movimientoPerteneceNuevaBase(fecha, hora)) return [];
...
function aplicarAjusteGuardadoLocal(ajuste) {
  if (!ajuste) return;

  const fecha = normalizarFecha(ajuste.fecha_movimiento || ajuste.fecha || "");
  if (!fecha || fecha < FECHA_INICIO_MINIMA) return;

  // En el día del corte no aceptar movimientos anteriores a las 12:25 PM.
  const hora = String(ajuste.hora_movimiento || "").substring(0, 5);
  if (!movimientoPerteneceNuevaBase(fecha, hora)) return;

  const codigoOriginal = String(ajuste.codigo || ajuste.codigoKey || "").trim();
  const item = {
    tipo: "AJUINV",
    docId: String(ajuste.docId || ajuste.folio || "").trim(),
    partida: ajuste.partida || "1",
    folio: String(ajuste.folio || ajuste.docId || "").trim(),
    fecha,
    hora,
    destino: "AJUSTE INVENTARIO",
    entrega: "AJUINV",
    recibe: String(ajuste.usuario || "OPERADOR").trim(),
    folioCincho: "",
    proveedor: "",
    alias_pivot: "",
    rfc_emisor: "",
    razon_social_emisor: "",
    codigo: codigoOriginal,
    codigoKey: normalizarCodigo(ajuste.codigoKey || codigoOriginal),
    ...
    existencia_teorica: Number(ajuste.existencia_teorica || 0)
  };

  const rango = obtenerRangoSemana();
  const mismaPartida = x => !(x.docId === item.docId && String(x.partida) === String(item.partida));

  if (fecha >= rango.inicio && fecha <= rango.fin) {
    registrosAjustesSemana = registrosAjustesSemana.filter(mismaPartida);
    registrosAjustesSemana.push(item);
  } else if (fecha >= FECHA_BASE_INVENTARIO && fecha <= rango.acumuladoAnteriorFin) {
    registrosAjustesAcumuladoAnterior = registrosAjustesAcumuladoAnterior.filter(mismaPartida);
  ...
  setStatus("Ajuste aplicado inmediatamente en memoria, sin recargar Firestore completo.");
}
...
function detenerEscuchaLigeraSemanaVisible() {
  desuscribirEscuchasSemana.forEach(fn => {
    try { fn(); } catch (_) {}
  });
  desuscribirEscuchasSemana = [];
  if (temporizadorSyncRemota) clearTimeout(temporizadorSyncRemota);
  temporizadorSyncRemota = null;
}

function programarSincronizacionRemota(origen) {
  // Si el cambio acaba de hacerlo esta misma app, ya fue aplicado en memoria.
  if (Date.now() - ultimaActualizacionLocal < 4000) return;
  if (temporizadorSyncRemota || sincronizacionRemotaEnCurso) return;

  temporizadorSyncRemota = setTimeout(async () => {
    temporizadorSyncRemota = null;
    if (document.hidden || sincronizacionRemotaEnCurso) return;

    sincronizacionRemotaEnCurso = true;
    try {
      setStatus(`Cambio externo detectado (${origen}). Sincronizando...`);
      await cargarSalidasZapata();
    } catch (error) {
      console.warn("Sincronización remota omitida:", error);
    } finally {
...
function iniciarEscuchaLigeraSemanaVisible() {
  detenerEscuchaLigeraSemanaVisible();
  ...
  escuchas.forEach(({ nombre, consulta }) => {
      ...
      const unsub = onSnapshot(
        ...
        snapshot => {
          if (primeraLectura) {
            ...
            return; // La carga inicial ya trajo estos datos.
          }
          if (!snapshot.docChanges().length) return;
          programarSincronizacionRemota(nombre);
        },
        error => console.warn(`Escucha ${nombre} desactivada:`, error.message || error)
      );
      desuscribirEscuchasSemana.push(unsub);
    } catch (error) {
      console.warn(`No se pudo activar escucha ${nombre}:`, error);
    }
  });
}

window.addEventListener("beforeunload", detenerEscuchaLigeraSemanaVisible);

document.addEventListener("DOMContentLoaded", async () => {
  iniciarContadorCarga();

  try {
    setProgress(2);
    setStatus("Preparando controles de la aplicación...");
    inicializarEventos();

    setStatus("Descargando inventario y movimientos...");
    await cargarSalidasZapata();

    // Los listados de inventario son secundarios: se cargan bajo demanda
    // al abrir "Ver inventarios" y no bloquean el arranque principal.
    setProgress(86);
    setStatus("Configurando ajustes de inventario...");
    iniciarModuloAjustesInventarioZapata({
      ...
      onAjusteGuardado: async (ajuste) => {
        aplicarAjusteGuardadoLocal(ajuste);
      }
    });

    setProgress(92);
    setStatus("Configurando proveedores y facturas pendientes...");
    await iniciarModuloEntradasZapata({
      onEntradaGenerada: async (entrada) => {
        aplicarEntradaGeneradaLocal(entrada);
      }
    });

    setProgress(100);
    iniciarEscuchaLigeraSemanaVisible();
    setStatus("Aplicación lista. Actualización local inmediata + escucha ligera de cambios externos.");
    detenerContadorCarga();
    await new Promise(resolve => setTimeout(resolve, 450));
    cargaInicialEnCurso = false;
    ocultarLoader();
  } catch (error) {
    console.error("Error durante el arranque completo:", error);
    if (intervaloContadorCarga) window.clearInterval(intervaloContadorCarga);
    intervaloContadorCarga = null;
    $("loader")?.classList.add("error-carga");
    setStatus(`No se pudo completar la carga: ${error.message || error}. Recarga la página para intentar nuevamente.`);
    actualizarContadorCarga(`Carga detenida después de ${segundosDesdeApertura().toFixed(1)} segundos`);
  }
});
