...
window.addEventListener('online',()=>{setConnectionBadge();toast('Conexión recuperada')});
...
async function loadFixedUser(){
  if(!navigator.onLine){const c=await cacheGet('fixedUser');S.user=c?.value||{id:FIXED_USER_DOC,usuario:'JUAN',nombre:'JUAN PEREZ',rol:'OPERADOR'};const name=S.user.nombre||S.user.usuario||'JUAN';$('sesionTxt').textContent=name;$('entryUserTxt').textContent=name;$('modeUser').textContent=`Usuario: ${name}`;return}
  try{
...
async function saveEntry(){
  ...
  if(!navigator.onLine){$('entryStatus').textContent='Sin internet. Las fotos siguen aquí; conecta el teléfono y vuelve a tocar GUARDAR ENTRADA.';$('entryStatus').classList.add('error');return}
...
async function loadConfig(){
  if(!navigator.onLine){const c=await cacheGet('config');if(c?.value)S.config={...S.config,...c.value};return}
...
async function loadEmployees(){
  if(!navigator.onLine){const c=await cacheGet('employees');S.empleados=Array.isArray(c?.value)?c.value:[];if(S.empleados.length)return S.empleados;throw new Error('Sin internet y sin catálogo offline de empleados')}
  try{
...
async function loadInventory(options={}){
  ...
  if(!navigator.onLine){
    const c=await cacheGet('inventoryCatalog'),list=Array.isArray(c?.value?.catalog)?c.value.catalog:[];
    if(list.length){S.catalog=list;rebuildCatalogIndex();S.inventoryReady=true;S.offlineCatalogAt=c.updatedAt;if(!boot){$('loadingText').textContent=`Catálogo offline cargado (${list.length.toLocaleString('es-MX')} productos).`;setTimeout(()=>{hideLoad();$('scanInput').focus()},250)}return true}
    S.inventoryReady=false;if(!boot){hideLoad();alert('Este equipo todavía no tiene catálogo offline. Conéctalo una vez y usa “Descargar catálogo offline”.')}return false;
  }
  try{
...
async function loadActiveProducts(){
  setBootStatus(navigator.onLine?'Descargando catálogo de productos activos...':'Cargando catálogo offline...');
  if(!navigator.onLine){
    const c=await cacheGet('masterProducts'),list=Array.isArray(c?.value)?c.value:[];
    S.masterProducts=list;S.masterByCode.clear();for(const p of list){S.masterByCode.set(p.codigo,p);(p.codigosEquivalentes||[]).forEach(code=>S.masterByCode.set(code,p));S.productInfo.set(p.codigo,p._raw||{})}
    S.masterReady=list.length>0;if(c?.updatedAt)S.offlineCatalogAt=c.updatedAt;if(S.masterReady)setBootStatus(`${list.length.toLocaleString('es-MX')} productos cargados desde respaldo offline.`);return S.masterReady;
  }
  try{
...
async function downloadOfflineCatalog(options={}){
  ...
  $('menu').classList.add('hidden');
  if(!navigator.onLine){
    if(startup){setBootStatus('SIN INTERNET: este equipo aún no tiene los catálogos requeridos.');$('bootHint').textContent='Conecta el teléfono a internet y pulsa VOLVER A REVISAR.';return false}
    alert('Para descargar/actualizar los catálogos necesitas internet. Los catálogos ya guardados seguirán funcionando sin conexión.');return false;
  }
...
async function enforceStartupCatalogProtection(){
  ...
  if(state.ready){
    ...
    $('bootHint').textContent=`${state.masterCount.toLocaleString('es-MX')} productos activos · ${state.inventoryCount.toLocaleString('es-MX')} productos operativos · ${navigator.onLine?'EN LÍNEA':'MODO OFFLINE'}`;
    await new Promise(r=>setTimeout(r,350));hideLoad();showModeGate();return true;
  }
  setBootStatus('CATÁLOGOS REQUERIDOS');
  $('bootHint').textContent=navigator.onLine?'Este equipo aún no tiene los catálogos completos. Descárgalos una vez antes de entrar a la aplicación.':'Sin internet y sin catálogos completos. Conecta el teléfono a internet para preparar la aplicación.';
  $('catalogPrepareBtn').classList.toggle('hidden',!navigator.onLine);
...
<button id="clear" class="secondary">BORRAR FIRMA</button>
  ...
  $('save').onclick=async()=>{
    ...
    if(!navigator.onLine)return alert('Sin internet. La salida y la firma permanecen en pantalla. Conecta el equipo y vuelve a tocar GUARDAR.');
    ...
    try{
      ...
      S.cart=[];S.last=[];render();S.recibe='';S.recibeEmpleado=null;S.destino='';S.fechaCaptura='';showModeGate();
    }catch(e){
...
window.addEventListener('online',()=>{if(!$('loading').classList.contains('hidden'))enforceStartupCatalogProtection()});
...
(async()=>{
  ...
  try{
    ...
    diag('BOOT','cargando config+usuario');await Promise.all([loadConfig(),loadFixedUser()]);diag('BOOT','config+usuario OK');
    await enforceStartupCatalogProtection();
