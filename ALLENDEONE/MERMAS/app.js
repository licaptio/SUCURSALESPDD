...
const LS_BORRADOR = "provsoft_merma_allende1_borrador";
const DB_NAME = "provsoft_catalogo_mermas_allende1";
const STORE_PRODUCTOS = "productos";
const LS_CATALOGO_FECHA = "provsoft_merma_allende1_catalogo_fecha";
const LS_PENDIENTES = "provsoft_merma_allende1_pendientes_firebase";
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_ALLENDE1, APP_CONFIG.SUBCOLECCION_MERMAS, documento.folio),
    docOnline,
    { merge:true }
  );
}
...
function construirDocumentoMerma(){
  ...
  return {
    ...
    tiendaColeccion:APP_CONFIG.TIENDA_ALLENDE1,
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de merma ALLENDE 1 generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoMerma){
  ...
  pdf.setFontSize(15);
  pdf.text("COMPROBANTE DE SOLICITUD DE MERMA ALLENDE 1",105,y,{align:"center"});

  y += 8;
  pdf.setFontSize(10);
  pdf.setFont("helvetica","normal");
  pdf.text("Documento de levantamiento de merma ALLENDE 1 - destino MATRIZ",105,y,{align:"center"});
