...
export function iniciarModuloAjustesInventarioZapata(opciones = {}) {
  ...
  $("btnLimpiarAjusteInventario")?.addEventListener("click", limpiarLoteCompleto);
...
function validarFechaHora() {
  ...
  if (fechaInput?.max && fecha > fechaInput.max) {
    alert(`Selecciona una fecha dentro de la semana cargada (hasta ${fechaInput.max}).`);
    return false;
  }
  return true;
}
...
function agregarPartidaAlLote() {
  ...
  if (!articuloSeleccionado) {
    alert("Selecciona un código de la tabla calculada.");
    return;
  }
  if (!articuloEstaEnTablaCalculada(articuloSeleccionado)) {
    alert("Ese código ya no pertenece a la tabla calculada actual.");
    articuloSeleccionado = null;
    return;
  }
  if (partidasLote.length >= MAX_PARTIDAS_LOTE) {
    alert(`El lote llegó al máximo de ${MAX_PARTIDAS_LOTE} artículos. Guarda este lote y comienza otro.`);
    return;
  }
  ...
  if (partidasLote.some(p => p.codigoKey === key)) {
    alert("Ese código ya está agregado al lote.");
    return;
  }
...
function pintarLote() {
  ...
  if (!partidasLote.length) {
    ...
    body.querySelectorAll(".aju-fisico-inline").forEach(input => {
  ...
  const totalAjuste = partidasLote.reduce((s, p) => s + Number(p.diferencia || 0), 0);
...
async function guardarLoteAjustes() {
  if (!validarFechaHora()) return;
  if (!partidasLote.length) {
    alert("Agrega por lo menos un artículo al lote.");
    return;
  }
  ...
  try {
    ...
    alert(`Lote guardado correctamente.\nFolio: ${folio}\nArtículos: ${partidasGuardadas.length}` +
      (telegramOk ? "\nNotificación enviada a Telegram." : "\nATENCIÓN: no se pudo enviar Telegram; el lote sí quedó guardado."));
    limpiarLoteCompleto();
  } catch (error) {
    console.error(error);
    alert("Error al guardar el lote de ajustes: " + error.message);
  } finally {
