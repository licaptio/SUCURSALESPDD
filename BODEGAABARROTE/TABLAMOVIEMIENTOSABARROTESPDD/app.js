...
async function consultarSalidas(inicio, fin) {
  ...
  // SALIDAS ABARROTES: lectura robusta de toda la colección.
  // No dependemos de que todos los documentos tengan el mismo campo de fecha.
  // La app actual puede guardar fechaCaptura, fecha, fechaLocal y creadoEn.
  // La fecha operativa del movimiento es fechaCaptura cuando exista.
  const snap = await getDocs(REF_SALIDAS_ZAPATA);
  const detalle = [];
  let docsEnRango = 0;
  ...
  snap.forEach((documento) => {
    ...
    if (!articulos.length) {
      console.warn("[SALIDAS ABARROTES] Documento sin partidas reconocibles:", documento.id, data);
      return;
    }
    ...
    articulos.forEach((art, idx) => {
      const codigoOriginal = String(
        art.codigoBarra ?? art.codigo ?? art.codigo_interno ?? art.productoId ?? ""
      ).trim();
      const codigoKey = normalizarCodigo(codigoOriginal);
      const nombre = String(
      ...
      const cantidad = Number(
        art.cantidad ?? art.cantidad_salida ?? art.unidades ?? 0
      );
      ...
      detalle.push({
        tipo: "SALIDA",
        docId: documento.id,
        partida: Number(art.renglon ?? art.renglon1 ?? art.renglon2 ?? art.renglon3 ?? art.renglon4 ?? art.renglon5 ?? art.renglon6 ?? art.renglon7 ?? art.renglon8 ?? idx + 1),
        folio: String(data.folio || documento.id || "").trim(),
        fecha,
        hora: String(data.horaCaptura || data.horaLocal || data.hora || "").trim(),
        codigo: codigoOriginal,
        codigoKey,
        nombre,
        cantidad
  ...
  console.info(
    `[SALIDAS ABARROTES] Colección: ${snap.size} documento(s). ` +
    `En rango ${inicio} a ${fin}: ${docsEnRango} documento(s), ${detalle.length} partida(s).`
  );

  return { detalle, totalDocs: docsEnRango };
}
