...
function pintarFacturas() {
  ...
  if (facturasPendientes.length === 0) {
    contenedor.innerHTML = `
      <p>
        No hay facturas pendientes para ABARROTES PDD.
        Revisa proveedores autorizados, fecha mínima 17/08/2026, entradas ya generadas o facturas marcadas como no entra.
      </p>
    `;
    return;
  }
  ...
  facturasPendientes.forEach((f, index) => {
    ...
    div.innerHTML = `
      <div class="factura-pendiente-head">
        ...
        <div class="factura-avance ${completa ? "completo" : ""}">
          <small>AVANCE ENLACE</small>
          <b>${avance}%</b>
          <span>${enlazados} de ${totalConceptos}</span>
        </div>
      </div>
      ...
      <div class="acciones-card">
        <button data-index="${index}" class="btnAutorizar">${completa ? "Revisar y autorizar" : "Revisar / autorizar"}</button>
      </div>
    `;
...
async function abrirModalProducto(index) {
  ...
  try {
    ...
    // Primera opción: catálogo construido por los artículos que ya forman parte de la tabla.
    const propios = filtrarProductosCatalogo(catalogoTabla, "");
    if (propios.length) {
      pintarProductos(propios, "tabla");
    } else {
      // Segunda opción: si la tabla aún no tiene catálogo, usamos el catálogo general.
      await asegurarProductosActivos();
      pintarProductos(filtrarProductosCatalogo(productosActivos, ""), "general");
    }
    setTimeout(() => {
...
async function buscarProductoUI() {
  ...
  // Siempre mostramos las dos fuentes en la misma búsqueda:
  // 1) catálogo construido por la tabla y 2) catálogo general.
  const propios = filtrarProductosCatalogo(catalogoTabla, texto);
  contenedor.innerHTML = "<p>Buscando también en catálogo general...</p>";

  try {
    await asegurarProductosActivos();
    const generales = filtrarProductosCatalogo(productosActivos, texto);
    pintarProductosCombinados(propios, generales);
  } catch (error) {
    // Si falla el catálogo general, al menos conservamos los resultados de la tabla.
    if (propios.length) {
      pintarProductos(propios, "tabla");
...
function pintarProductosCombinados(productosTabla = [], productosGenerales = []) {
  const contenedor = document.getElementById("listaProductos");
  contenedor.innerHTML = "";

  if (!productosTabla.length && !productosGenerales.length) {
    contenedor.innerHTML = "<p>No se encontraron productos activos.</p>";
    return;
  }

  pintarGrupoProductos(contenedor, productosTabla, "tabla");
  pintarGrupoProductos(contenedor, productosGenerales, "general");
}
