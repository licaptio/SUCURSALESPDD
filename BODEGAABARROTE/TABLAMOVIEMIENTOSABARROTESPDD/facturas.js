...
const FECHA_MINIMA_ZAPATA = "2026-08-17";

export async function cargarFacturasOrigen(max = 1000) {
  const col = collection(db, RUTA_FACTURAS_ORIGEN);

  // Leer directamente desde Punto Cero por la fecha real de la factura.
  // Así el módulo no depende de que la factura esté entre los últimos N documentos
  // por timestamp de carga.
  const q = query(
    col,
    where("fecha", ">=", `${FECHA_MINIMA_ZAPATA}T00:00:00`),
    orderBy("fecha", "desc"),
    limit(max)
  );
...
export async function filtrarFacturasPendientesParaZapata(
  ...
  const rfcsPermitidos = new Set(
    proveedoresAutorizados
      // Un proveedor se considera autorizado salvo que esté explícitamente inactivo.
      // Esto mantiene compatibilidad con documentos cargados como { autorizado: true }
      // y con los creados por la interfaz usando { activo: true }.
      .filter(p => p.activo !== false && p.autorizado !== false)
      .map(p => String(p.rfc_emisor || p.rfc || "").trim().toUpperCase())
      .filter(Boolean)
  );
