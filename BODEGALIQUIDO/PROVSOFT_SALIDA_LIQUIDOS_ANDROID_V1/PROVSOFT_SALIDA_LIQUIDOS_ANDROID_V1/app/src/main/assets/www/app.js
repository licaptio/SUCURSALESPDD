import { printSalida58, printerConfigRead, openPrinterConfigModal } from "./printer58.js";
...
const estado = {
  ...
  // Códigos que NO deben aparecer en el buscador manual.
  // Se cargan primero desde IndexedDB y luego se mantienen
  // sincronizados en tiempo real con Firestore.
  codigosBloqueados: new Set(),
  cancelarEscuchaCodigosBloqueados: null,
...
async function iniciarApp(){
  ...
  try{
    ...
    // Arranque inmediato con la última lista conocida.
    // No bloquea la app si no hay internet.
    await cargarCodigosBloqueadosLocal();

    // Un único listener en tiempo real. Firestore avisa solamente
    // cuando cambia el documento; no hacemos consultas por búsqueda.
...
/* =====================================================
   CÓDIGOS BLOQUEADOS - FIRESTORE + CACHE LOCAL

   Firestore:
   /almacenes/Almacen_Liquidos/configuracion/codigosbloqueados

   Documento:
   codigos: ["7501048100057", ...]
===================================================== */

const CLAVE_CACHE_CODIGOS_BLOQUEADOS = "codigos_bloqueados_salida";

function normalizarListaCodigosBloqueados(lista){
  ...
  return [...new Set(
    lista
      .map(codigo => limpiarCodigo(codigo))
      .filter(Boolean)
  )];

}

async function cargarCodigosBloqueadosLocal(){

  try{

    const local = await leerMetadata(
      CLAVE_CACHE_CODIGOS_BLOQUEADOS
    );

    const codigos = normalizarListaCodigosBloqueados(
      local?.codigos
    );

    estado.codigosBloqueados = new Set(codigos);

    if(codigos.length){
      console.log(
        `🔒 ${codigos.length} códigos bloqueados cargados desde local`
      );
    }
    ...
    console.warn(
      "No se pudo leer el cache local de códigos bloqueados:",
      error
    );
...
function iniciarEscuchaCodigosBloqueados(){
  ...
  if(estado.cancelarEscuchaCodigosBloqueados){
    return;
  }
  ...
  estado.cancelarEscuchaCodigosBloqueados = onSnapshot(
    referencia,
    async snap => {
      ...
      try{
        ...
        console.warn(
          "No se pudo guardar localmente la lista de bloqueados:",
          error
        );
      ...
      console.log(
        `🔒 Lista de bloqueados sincronizada: ${codigos.length}`
      );

      // Si el usuario tiene abierto el buscador, refrescamos
      // inmediatamente los resultados visibles.
      if(
        el.modalBusqueda &&
        !el.modalBusqueda.classList.contains("oculto") &&
        el.inputBusqueda
      ){
    ...
    error => {
      console.warn(
        "Sin conexión para actualizar códigos bloqueados; se conserva la lista local.",
        error
      );
    }
  );
...
function configurarEventos(){
  ...
  if(el.btnImpresora58){ el.btnImpresora58.addEventListener("click", openPrinterConfigModal); }
  ...
  const resultados =
    ...
    estado.catalogo
      ...
      .filter(p=>{
        ...
        // El bloqueo aplica SOLO al buscador manual.
        // El escáner continúa funcionando normalmente.
        if(
          estado.codigosBloqueados.has(
            limpiarCodigo(p.codigoBarra)
          )
        ){
          return false;
        }
...
el.txtDestino.textContent = estado.destino;
  ...
  const salida = {
    ...
    const cfgImpresora = printerConfigRead();
    let resultadoImpresion = { ok:false, native:false, reason:"AUTO_PRINT_DISABLED" };

    if(cfgImpresora.autoPrint !== false){
      mostrarCarga("Enviando ticket a impresora 58 mm...");
      resultadoImpresion = await printSalida58(salida);

      const ahoraImpresion = new Date().toISOString();
      salida.impresion = resultadoImpresion.ok
        ? { estado:"IMPRESO", intentos:1, ultimaIntentoLocal:ahoraImpresion, ultimaImpresionLocal:ahoraImpresion, errorCodigo:null, errorMensaje:null, dispositivo:cfgImpresora.device||null }
        : resultadoImpresion.native
          ? { estado:"ERROR", intentos:1, ultimaIntentoLocal:ahoraImpresion, ultimaImpresionLocal:null, errorCodigo:resultadoImpresion.reason||"PRINT_ERROR", errorMensaje:resultadoImpresion.error||resultadoImpresion.reason||"Error de impresión", dispositivo:cfgImpresora.device||null }
          : { estado:"PENDIENTE", intentos:0, ultimaIntentoLocal:null, ultimaImpresionLocal:null, errorCodigo:"NO_NATIVE_BRIDGE", errorMensaje:"Pendiente de imprimir desde Android", dispositivo:cfgImpresora.device||null };

      await setDoc(doc(db,"almacenes","Almacen_Dulces","salidas",folio), { impresion: salida.impresion }, { merge:true });
    }else{
      salida.impresion = { estado:"NO_SOLICITADA", intentos:0, errorCodigo:null, errorMensaje:null, dispositivo:cfgImpresora.device||null };
      await setDoc(doc(db,"almacenes","Almacen_Dulces","salidas",folio), { impresion: salida.impresion }, { merge:true });
    }

    const mensajeImpresion = resultadoImpresion.ok
      ? "\nTicket 58 mm IMPRESO"
      : resultadoImpresion.native
        ? `\nImpresión 58 mm FALLÓ: ${resultadoImpresion.error||resultadoImpresion.reason||"revisar impresora"}`
        : "\nTicket 58 mm PENDIENTE: se imprimirá desde la APK Android";

    alert(`Salida ${folio} guardada${mensajeImpresion}`);
    location.reload();
...
/* =====================================================
   IMPRESION: reemplazada por printer58.js (ESC/POS 58 mm)
===================================================== */
