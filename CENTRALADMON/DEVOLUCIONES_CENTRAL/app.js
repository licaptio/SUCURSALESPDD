...
const LS_BORRADOR = "provsoft_devolucion_central_borrador";
const DB_NAME = "provsoft_catalogo_productos_central";
const DB_VERSION = 2;
const STORE_PRODUCTOS = "productos";
const STORE_META = "metadata";
const META_ULTIMA_SYNC = "ultimaSincronizacionUpdatedAt";
const LS_CATALOGO_FECHA = "provsoft_catalogo_productos_central_fecha";
const LS_PENDIENTES = "provsoft_devolucion_central_pendientes_firebase";
...
async function prepararCatalogoInicio(){
  actualizarLoader(
    "Preparando Devoluciones CENTRAL",
  ...
  try{
    ...
    const checkpoint = await leerCheckpointCatalogo();
    ...
    if(!navigator.onLine){
      if(catalogo.length > 0){
        actualizarLoader(
          ...
          "Se usará la última copia guardada. Los cambios del catálogo se sincronizarán cuando vuelva internet."
        );
        finalizarInicio();
        return;
      }
    ...
    const esCargaInicial = catalogo.length === 0 || !checkpoint;
    actualizarLoader(
      esCargaInicial ? "Descargando catálogo inicial" : "Sincronizando catálogo",
      esCargaInicial ? "Obteniendo productos activos desde Firebase..." : "Buscando únicamente productos modificados...",
      esCargaInicial ? "Esta descarga completa solo se necesita para crear o reconstruir el catálogo local." : "PROVSOFT usará el catálogo CENTRAL compartido y aplicará solo los cambios posteriores al último checkpoint."
    );

    const resultado = await sincronizarCatalogoIncremental({forzarCompleto:esCargaInicial});

    if(!resultado.ok || catalogo.length === 0){
      actualizarLoader(
        "No se pudo preparar la aplicación",
        "La sincronización del catálogo no pudo completarse.",
    ...
    actualizarLoader(
      "Catálogo listo",
      `${catalogo.length.toLocaleString("es-MX")} productos activos disponibles.`,
      resultado.completo
        ? "Catálogo inicial guardado. Iniciando aplicación..."
        : `${resultado.cambios} cambio(s) recibido(s) desde Firebase. Iniciando aplicación...`
    );
    finalizarInicio();
  }catch(err){
    ...
    actualizarLoader(
      ...
      "No fue posible leer o sincronizar el catálogo local.",
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_CENTRAL, APP_CONFIG.SUBCOLECCION_DEVOLUCIONES, documento.folio),
    docOnline,
    { merge:true }
  );
}
      ...
      try{
        ...
        try{
          ...
          await enviarPDFTelegram(pdfBlob,{
            ...
            tienda: registro.documento.tiendaOrigen || APP_CONFIG.TIENDA_CENTRAL,
...
async function actualizarCatalogoPendienteEnFondo(){
  ...
  try{
    await sincronizarCatalogoIncremental({forzarCompleto:false});
  }catch(err){
    console.warn("No se pudo sincronizar catálogo en segundo plano:",err);
  }finally{
  ...
  return new Promise((resolve,reject)=>{
    ...
    req.onupgradeneeded = e=>{
      ...
      if(!dbLocal.objectStoreNames.contains(STORE_META)){
        dbLocal.createObjectStore(STORE_META,{keyPath:"clave"});
      }
    };
...
async function aplicarCambiosCatalogoLocal(cambios){
  const dbLocal = await abrirDB();
  ...
  return new Promise((resolve,reject)=>{
    ...
    cambios.forEach(({id,producto,activo})=>{
      if(activo) store.put(producto);
...
async function leerCheckpointCatalogo(){
  const dbLocal = await abrirDB();
  return new Promise((resolve,reject)=>{
    const tx = dbLocal.transaction(STORE_META,"readonly");
    const req = tx.objectStore(STORE_META).get(META_ULTIMA_SYNC);
    req.onsuccess = ()=>resolve(Number(req.result?.valor || 0) || 0);
    req.onerror = ()=>reject(req.error);
  });
}

async function guardarCheckpointCatalogo(millis){
  if(!millis || !Number.isFinite(Number(millis))) return;
  const dbLocal = await abrirDB();
  return new Promise((resolve,reject)=>{
    const tx = dbLocal.transaction(STORE_META,"readwrite");
    tx.objectStore(STORE_META).put({
      clave: META_ULTIMA_SYNC,
      valor: Number(millis),
...
function convertirProductoCatalogo(documento){
...
function obtenerMillisUpdatedAt(d){
  const valor = d?.updatedAt;
  if(valor && typeof valor.toMillis === "function") return valor.toMillis();
  if(valor?.seconds != null) return Number(valor.seconds) * 1000 + Math.floor(Number(valor.nanoseconds || 0) / 1e6);
  return 0;
}

async function sincronizarCatalogoIncremental({forzarCompleto=false}={}){
  if(!navigator.onLine) return {ok:false,completo:false,cambios:0};

  const checkpoint = await leerCheckpointCatalogo();
  const hacerCompleto = forzarCompleto || catalogo.length === 0 || !checkpoint;
  ...
  if(hacerCompleto){
    ...
    const snap = await getDocs(qInicial);
    if(snap.empty) throw new Error("Firebase devolvió un catálogo activo vacío.");

    let maxUpdatedAt = 0;
    catalogo = snap.docs.map(documento=>{
      maxUpdatedAt = Math.max(maxUpdatedAt,obtenerMillisUpdatedAt(documento.data()));
      return convertirProductoCatalogo(documento);
    });

    await guardarCatalogoLocal(catalogo);
    if(maxUpdatedAt > 0) await guardarCheckpointCatalogo(maxUpdatedAt);
    localStorage.setItem(LS_CATALOGO_FECHA,new Date().toLocaleDateString("en-CA"));
    return {ok:true,completo:true,cambios:snap.size};
  }

  const qCambios = query(
    collection(db, APP_CONFIG.COLECCION_PRODUCTOS),
    where("updatedAt",">",Timestamp.fromMillis(checkpoint))
  );
  const snap = await getDocs(qCambios);

  if(snap.empty){
    localStorage.setItem(LS_CATALOGO_FECHA,new Date().toLocaleDateString("en-CA"));
    return {ok:true,completo:false,cambios:0};
  }

  let maxUpdatedAt = checkpoint;
  const cambios = snap.docs.map(documento=>{
    const d = documento.data();
    maxUpdatedAt = Math.max(maxUpdatedAt,obtenerMillisUpdatedAt(d));
    return {
      id: documento.id,
      activo: d.activo === true,
      producto: convertirProductoCatalogo(documento)
    };
  });

  await aplicarCambiosCatalogoLocal(cambios);
  await guardarCheckpointCatalogo(maxUpdatedAt);
  await cargarCatalogoLocal();
  localStorage.setItem(LS_CATALOGO_FECHA,new Date().toLocaleDateString("en-CA"));
  return {ok:true,completo:false,cambios:snap.size};
}
...
async function descargarCatalogo(manual=false,opciones={}){
  ...
  if(!navigator.onLine){
    if(manual) alert("No hay internet para sincronizar catálogo.");
    return false;
  }
  ...
  try{
    if(!silencioso){
      ...
      document.getElementById("loaderTexto").textContent = "Sincronizando cambios del catálogo...";
    }

    const resultado = await sincronizarCatalogoIncremental({forzarCompleto:false});
    if(manual){
      alert(resultado.completo
        ? `Catálogo inicial actualizado: ${catalogo.length} productos.`
        : `Sincronización lista: ${resultado.cambios} cambio(s). ${catalogo.length} productos activos.`);
    }
    return resultado.ok;
  }catch(err){
    console.error(err);
    if(manual) alert("Error al sincronizar catálogo. Revisa conexión y el campo updatedAt.");
    return false;
  }finally{
...
function abrirModalProducto(producto){
  ...
  document.getElementById("btnGuardarProducto").textContent = "Agregar a devolución";
...
function editarItem(index){
  ...
  document.getElementById("modalMotivo").value = item.motivo || "PRODUCTO_PARADO";
...
function recuperarBorrador(){
  ...
  // CENTRAL es fija: nunca recuperar una tienda heredada de otra app.
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_CENTRAL;
  document.getElementById("encargado").value = b.encargado || "";

  // Solo conservar folios válidos de DEVOLUCIONES CENTRAL.
  const folioRecuperado = String(b.folio || "").trim();
  document.getElementById("folio").value = folioRecuperado.startsWith("DEV-CENTRAL-")
    ? folioRecuperado
    : generarFolio();
...
async function generarSolicitud(){
  ...
  if(!navigator.onLine){
    try{
      encolarPendienteFirebase(documentoDevolucion);
      descargarPDF(pdfBlob,`${documentoDevolucion.folio}.pdf`);
      reiniciarFormularioDespuesDeGrabar();
      alert(`Solicitud ${documentoDevolucion.folio} guardada en este equipo.\n\nQueda PENDIENTE de subir a Firebase y se sincronizará automáticamente cuando vuelva internet.`);
    }catch(err){
      console.error("No se pudo guardar la devolución pendiente:",err);
  ...
  try{
    document.getElementById("loader").style.display = "flex";
    document.getElementById("loaderTexto").textContent = "Generando solicitud de devolución...";

    await subirDocumentoFirebase(documentoLocalSeguro(documentoDevolucion));
    ...
    try{
      ...
      await enviarPDFTelegram(pdfBlob,{
        ...
        tienda: documentoDevolucion.tiendaOrigen,
        encargado: documentoDevolucion.solicitante.nombre,
        totalPiezas: documentoDevolucion.totales.piezas,
        totalPublico: documentoDevolucion.totales.precioPublicoEstimado
      });
      telegramEnviado = true;
    }catch(errorTelegram){
      telegramError = errorTelegram;
      console.error("La devolución se guardó, pero Telegram falló:",errorTelegram);
    }
    ...
    if(telegramEnviado){
      alert("Solicitud de devolución CENTRAL generada y PDF enviado a Telegram.");
    }else{
    ...
    try{
      ...
      descargarPDF(pdfBlob,`${documentoDevolucion.folio}.pdf`);
      reiniciarFormularioDespuesDeGrabar();
      alert(`Firebase no respondió. La solicitud ${documentoDevolucion.folio} quedó guardada localmente como PENDIENTE y se reintentará automáticamente.`);
    }catch(errorLocal){
...
function generarPDF(documentoDevolucion){
  ...
  pdf.text("Documento de alta de devolución desde CENTRAL a MATRIZ",105,y,{align:"center"});
  ...
  pdf.text("Folio:",15,y);
  pdf.setFont("helvetica","normal");
  pdf.text(documentoDevolucion.folio,45,y);

  y += 7;
  pdf.setFont("helvetica","bold");
  pdf.text("Tienda origen:",15,y);
  pdf.setFont("helvetica","normal");
  pdf.text(documentoDevolucion.tiendaOrigen,45,y);
  ...
  pdf.setFont("helvetica","normal");
  pdf.text(documentoDevolucion.solicitante.nombre,45,y);
  ...
  documentoDevolucion.productos.forEach(p=>{
    if(y > 240){
      pdf.addPage();
      y = 15;
    }
  ...
  pdf.setFont("helvetica","bold");
  pdf.text(`Total piezas: ${documentoDevolucion.totales.piezas}`,15,y);

  y += 6;
  pdf.text(`Precio público total: $${Number(documentoDevolucion.totales.ventaEstimado || documentoDevolucion.totales.precioPublicoEstimado || 0).toFixed(2)}`,15,y);
  ...
  pdf.setFont("helvetica","normal");
  const notas = pdf.splitTextToSize(documentoDevolucion.notasAdicionales || "Sin notas adicionales.",180);
  pdf.text(notas,15,y);

  y += notas.length * 5 + 10;

  if(documentoDevolucion.solicitante.firmaBase64){
    pdf.setFont("helvetica","bold");
    pdf.text("Firma del solicitante:",15,y);
    y += 5;

    pdf.addImage(documentoDevolucion.solicitante.firmaBase64,"PNG",15,y,70,28);
    y += 33;
    ...
    pdf.setFont("helvetica","normal");
    pdf.text(documentoDevolucion.solicitante.nombre,15,y);
  }
  ...
  pdf.setFontSize(8);
  pdf.setTextColor(90);
  pdf.text("Este comprobante representa la solicitud de devolución. La recepción final corresponde a MATRIZ.",15,y);
