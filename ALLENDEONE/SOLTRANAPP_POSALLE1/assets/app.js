...
function nuevoFolio(){
  ...
  // Primeras 4 letras/números de la sucursal.
  // ALLENDE 1 -> ALLE
  const sucursalLimpia = String(TIENDA || "SUCURSAL")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g,"")
    .replace(/[^A-Za-z0-9]/g,"")
    .toUpperCase();

  const prefijo = (sucursalLimpia + "XXXX").slice(0,4);

  const pad2 = n => String(n).padStart(2,"0");
  const pad3 = n => String(n).padStart(3,"0");

  // Serial cronológico único:
  // AAAAMMDD-HHMMSSmmm
  const serial =
    ahora.getFullYear() +
