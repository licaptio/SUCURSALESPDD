...
async function iniciarApp(){
  ...
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_ALLENDE2;
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_ALLENDE2, APP_CONFIG.SUBCOLECCION_DEVOLUCIONES, documento.folio),
    docOnline,
    { merge:true }
  );
}
      ...
      try{
        ...
        try{
          ...
          await enviarPDFTelegram(pdfBlob,{
            ...
            tienda: registro.documento.tiendaOrigen || APP_CONFIG.TIENDA_ALLENDE2,
...
function generarFolio(){
  const limpio = APP_CONFIG.TIENDA_ALLENDE2;
  const f = new Date();
...
function construirDocumentoDevolucion(){
  const t = calcularTotales();
  const tienda = APP_CONFIG.TIENDA_ALLENDE2;
  const encargado = document.getElementById("encargado").value.trim();
