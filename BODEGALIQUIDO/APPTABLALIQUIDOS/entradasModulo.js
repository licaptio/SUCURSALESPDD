  ...
  facturasPendientes.forEach((f, index) => {
    ...
    div.innerHTML = `
      ...
      ${(() => {
        ...
        const total = preparados.length;
        const enlazados = preparados.filter(a => a.equivalencia_encontrada).length;
        const porcentaje = total > 0 ? Math.round((enlazados / total) * 100) : 100;
        return `
          <div class="factura-enlace-progreso" aria-label="${enlazados} de ${total} conceptos enlazados, ${porcentaje} por ciento">
            <div class="factura-enlace-progreso-cabecera">
              <span><b>Enlazado:</b> ${enlazados}/${total}</span>
              <strong>${porcentaje}%</strong>
            </div>
            <div class="factura-enlace-barra" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${porcentaje}">
              <span style="width:${porcentaje}%"></span>
            </div>
            <small>${total - enlazados === 0 ? "Lista para autorizar" : `${total - enlazados} concepto${total - enlazados === 1 ? "" : "s"} por enlazar`}</small>
          </div>`;
      })()}
      <div class="acciones-card">
