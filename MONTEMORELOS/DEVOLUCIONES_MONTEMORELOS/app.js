...
const APP_CONFIG = {
  ...
  TIENDA_MONTEMORELOS: "MONTEMORELOS",
  DESTINO_OPERATIVO: "MATRIZ",
  SUBCOLECCION_DEVOLUCIONES: "DEVOLUCION"
};
...
const LS_BORRADOR = "provsoft_devolucion_montemorelos_borrador";
const DB_NAME = "provsoft_catalogo_montemorelos_compartido";
const STORE_PRODUCTOS = "productos";
const STORE_META = "meta";
const STORE_FOTOS_META = "fotos_meta";
const DB_VERSION = 2;
const LS_CATALOGO_FECHA = "provsoft_montemorelos_catalogo_fecha";
const LS_PENDIENTES = "provsoft_devolucion_montemorelos_pendientes_firebase";
...
async function iniciarApp(){
  ...
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_MONTEMORELOS;
...
async function prepararCatalogoInicio(){
  actualizarLoader(
    "Preparando Devoluciones MONTEMORELOS",
    ...
    if(!navigator.onLine){
      if(catalogo.length > 0){
        actualizarLoader(
          ...
          "Se usará la última copia guardada. La actualización diaria y las devoluciones pendientes se sincronizarán cuando vuelva internet."
        );
        finalizarInicio();
        return;
      }
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_MONTEMORELOS, APP_CONFIG.SUBCOLECCION_DEVOLUCIONES, documento.folio),
    docOnline,
    { merge:true }
  );
}
      ...
      try{
        ...
        try{
          ...
          await enviarPDFTelegram(pdfBlob,{
            ...
            tienda: registro.documento.tiendaOrigen || APP_CONFIG.TIENDA_MONTEMORELOS,
...
function generarFolio(){
  const limpio = APP_CONFIG.TIENDA_MONTEMORELOS;
  const f = new Date();
...
function abrirDB(){
  return new Promise((resolve,reject)=>{
    ...
    req.onupgradeneeded = e=>{
      ...
      if(!dbLocal.objectStoreNames.contains(STORE_FOTOS_META)){
        dbLocal.createObjectStore(STORE_FOTOS_META,{keyPath:"codigoBarra"});
      }
    };
...
async function descargarCatalogo(manual=false,opciones={}){
  ...
  try{
    ...
    catalogo = snap.docs.map(documento=>{
      ...
      const codigo = String(d.codigoBarra || documento.id);
      const descripcion = String(d.concepto || "");
      const departamento = String(d.departamento || "");
      const marca = String(d.marca || "");
      const equivalentes = Array.isArray(d.codigosEquivalentes) ? d.codigosEquivalentes : [];
      const textoBusqueda = [codigo, descripcion, marca, departamento, ...equivalentes].join(" ");
      ...
      return {
        ...
        _search: normalizar(textoBusqueda)
      };
    });
...
function recuperarBorrador(){
  ...
  // MONTEMORELOS es fija: nunca recuperar una tienda heredada de otra app.
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_MONTEMORELOS;
  document.getElementById("encargado").value = b.encargado || "";

  // Solo conservar folios válidos de DEVOLUCIONES MONTEMORELOS.
  const folioRecuperado = String(b.folio || "").trim();
  document.getElementById("folio").value = folioRecuperado.startsWith("DEV-MONTEMORELOS-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoDevolucion(){
  const t = calcularTotales();
  const tienda = APP_CONFIG.TIENDA_MONTEMORELOS;
  const encargado = document.getElementById("encargado").value.trim();
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de devolución MONTEMORELOS generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoDevolucion){
  ...
  pdf.text("Documento de alta de devolución desde MONTEMORELOS a MATRIZ",105,y,{align:"center"});
