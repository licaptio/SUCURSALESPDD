...
const REF_AJUSTES = collection(db, "almacenes", "abarrotespdd", "ajustealinventario");
...
async function guardarLoteAjustes() {
  ...
  const partidasGuardadas = partidasLote.map((p, index) => {
    ...
    const refPartida = doc(db, "almacenes", "abarrotespdd", "ajustealinventario", ajusteId, "PARTIDAS", partidaId);
