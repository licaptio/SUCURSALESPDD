...
async function cargarCatalogoBloqueadosLocal() {
  ...
  const CACHE_SYNC = "provsoft_liquidos_catalogo_activos_sync_v1";
  ...
  try {
    ...
    if (!productosActivosBloqueo.length) {
      ...
      try { localStorage.setItem(CACHE_SYNC, String(Date.now())); } catch {}
    } else {
      // Hay catálogo local: habilitamos inmediatamente. La actualización de 24 h va al fondo.
      habilitarBuscador();
      let ultima = 0;
      try { ultima = Number(localStorage.getItem(CACHE_SYNC) || 0); } catch {}
      if (!ultima || (Date.now() - ultima) >= REFRESCO_MS) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (esCelular) {
    // En celular la app arranca en el menú principal, no en el pivot.
    // El fondo blanco del menú oculta por completo la tabla hasta elegir una opción.
