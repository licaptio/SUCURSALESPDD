...
// V19: RAWBT directo + sincronizacion de salidas + actualización automática de catálogos cada 24 horas. El arranque ya NO depende de Firebase/CDN. Todo lo remoto se carga
...
function printRawBtSalida(salida){
  try{
    const payload=uint8ToBase64(buildRawBtBytes(salida));
    const rawUrl='rawbt:base64,'+payload;
    // IMPORTANTE: se ejecuta sincronamente dentro del toque en FINALIZAR.
    // En Android/PWA, window.location es mas confiable que disparar un <a> oculto,
    // porque conserva la activacion del usuario necesaria para abrir RawBT.
...
<button id="clear" class="secondary">BORRAR FIRMA</button>
  ...
  $('save').onclick=()=>{
    ...
    try{
      ...
      // 1) Primero queda protegida LOCALMENTE. No depende de que Firebase responda.
      if(!queueSalidaLocal(salida))throw new Error('No fue posible proteger la salida en almacenamiento local.');

      // 2) RawBT se dispara inmediatamente desde el toque del usuario.
      const rawOk=printRawBtSalida(salida);

      // 3) La pantalla queda libre. Firebase/PDF/Telegram trabajan por separado.
      ro.disconnect();close();
      S.cart=[];S.last=[];render();S.recibe='';S.recibeEmpleado=null;S.destino='';S.fechaCaptura='';showModeGate();
      toast(rawOk?'Salida enviada a RawBT · sincronizando':'Salida guardada localmente · sincronizando');
      setTimeout(()=>syncPendingSalidas(),0);
    }catch(e){
      console.error(e);alert('No se pudo finalizar la salida. '+(e?.message||''));$('save').disabled=false;$('clear').disabled=false;
    }
  };
}
