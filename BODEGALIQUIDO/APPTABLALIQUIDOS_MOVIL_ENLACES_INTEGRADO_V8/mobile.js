...
async function guardarEnlace(){
  ...
  try{
    await guardarEquivalencia({
      ...
      texto_factura:texto,
      codigo_factura:String(a.codigo_factura||'').trim(),
      descripcion_factura:texto,
      codigo_interno:String(productoElegido.codigoBarra||productoElegido.id||'').trim(),
      descripcion_interna:String(productoElegido.concepto||a.descripcion_interna||'').trim(),
      unidad_factura:String(a.unidad_factura||'').trim(),
      unidad_inventario:String(productoElegido.unidadMedidaSat||a.unidad_inventario||'').trim(),
    ...
    console.error('Error al guardar equivalencia:',e);
    toast(e?.message||'No se pudo guardar el enlace','error');
  } finally {
    ocultarLoading();
    if(boton){ boton.disabled=false; boton.textContent='Guardar enlace'; }
  }
}
