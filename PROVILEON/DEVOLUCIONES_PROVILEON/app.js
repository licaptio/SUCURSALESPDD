...
const LS_BORRADOR = "provsoft_devolucion_provileon_borrador";
const DB_NAME = "provsoft_catalogo_devoluciones_provileon";
const STORE_PRODUCTOS = "productos";
const LS_CATALOGO_FECHA = "provsoft_devolucion_provileon_catalogo_fecha";
const LS_PENDIENTES = "provsoft_devolucion_provileon_pendientes_firebase";
...
async function iniciarApp(){
  ...
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_PROVILEON;
...
async function prepararCatalogoInicio(){
  actualizarLoader(
    "Preparando Devoluciones PROVILEON",
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_PROVILEON, APP_CONFIG.SUBCOLECCION_DEVOLUCIONES, documento.folio),
    docOnline,
    { merge:true }
  );
}
      ...
      try{
        ...
        try{
          ...
          await enviarPDFTelegram(pdfBlob,{
            ...
            tienda: registro.documento.tiendaOrigen || APP_CONFIG.TIENDA_PROVILEON,
...
function generarFolio(){
  const limpio = APP_CONFIG.TIENDA_PROVILEON;
  const f = new Date();
...
function recuperarBorrador(){
  ...
  // PROVILEON es fija: nunca recuperar una tienda heredada de otra app.
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_PROVILEON;
  document.getElementById("encargado").value = b.encargado || "";

  // Solo conservar folios válidos de DEVOLUCIONES PROVILEON.
  const folioRecuperado = String(b.folio || "").trim();
  document.getElementById("folio").value = folioRecuperado.startsWith("DEV-PROVILEON-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoDevolucion(){
  const t = calcularTotales();
  const tienda = APP_CONFIG.TIENDA_PROVILEON;
  const encargado = document.getElementById("encargado").value.trim();
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de devolución PROVILEON generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoDevolucion){
  ...
  pdf.text("Documento de alta de devolución desde PROVILEON a MATRIZ",105,y,{align:"center"});
