...
const LS_BORRADOR = "provsoft_merma_montemorelos_borrador";
...
const LS_PENDIENTES = "provsoft_merma_montemorelos_pendientes_firebase";
...
async function prepararCatalogoInicio(){
  actualizarLoader(
    "Preparando Mermas MONTEMORELOS",
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_MONTEMORELOS, APP_CONFIG.SUBCOLECCION_MERMAS, documento.folio),
    docOnline,
    { merge:true }
  );
}
...
function recuperarBorrador(){
  ...
  // Solo conservar folios válidos de MERMAS MONTEMORELOS.
  const folioRecuperado = String(b.folio || "").trim();
  document.getElementById("folio").value = folioRecuperado.startsWith("MER-MONTEMORELOS-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoMerma(){
  ...
  return {
    ...
    montemorelosColeccion:APP_CONFIG.TIENDA_MONTEMORELOS,
    ...
    origen:"APP_MERMAS_MONTEMORELOS",
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de merma MONTEMORELOS generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoMerma){
  ...
  pdf.setFontSize(15);
  pdf.text("COMPROBANTE DE SOLICITUD DE MERMA MONTEMORELOS",105,y,{align:"center"});

  y += 8;
  pdf.setFontSize(10);
  pdf.setFont("helvetica","normal");
  pdf.text("Documento de levantamiento de merma MONTEMORELOS - destino MATRIZ",105,y,{align:"center"});
