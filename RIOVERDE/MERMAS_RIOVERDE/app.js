...
const LS_BORRADOR = "provsoft_merma_rioverde_borrador";
const DB_NAME = "provsoft_catalogo_mermas_rioverde";
const STORE_PRODUCTOS = "productos";
const LS_CATALOGO_FECHA = "provsoft_merma_rioverde_catalogo_fecha";
const LS_PENDIENTES = "provsoft_merma_rioverde_pendientes_firebase";
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_RIOVERDE, APP_CONFIG.SUBCOLECCION_MERMAS, documento.folio),
    docOnline,
    { merge:true }
  );
}
...
function recuperarBorrador(){
  ...
  // Solo conservar folios válidos de MERMAS RIO VERDE.
  const folioRecuperado = String(b.folio || "");
  document.getElementById("folio").value = folioRecuperado.startsWith("MER-RIO VERDE-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoMerma(){
  ...
  return {
    ...
    rioverdeColeccion:APP_CONFIG.TIENDA_RIOVERDE,
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de merma RIO VERDE generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoMerma){
  ...
  pdf.setFontSize(15);
  pdf.text("COMPROBANTE DE SOLICITUD DE MERMA RIO VERDE",105,y,{align:"center"});

  y += 8;
  pdf.setFontSize(10);
  pdf.setFont("helvetica","normal");
  pdf.text("Documento de levantamiento de merma RIO VERDE - destino MATRIZ",105,y,{align:"center"});
