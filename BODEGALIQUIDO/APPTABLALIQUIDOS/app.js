...
import { cargarProductosActivosLocal, sincronizarProductosActivosLocal } from "./configuracion.js";
import {
  collection,
  query,
  where,
...
// Catálogo propio de esta app. La bolsa vive en un IndexedDB exclusivo de
// APPTABLALIQUIDOS (definido en configuracion.js) para no chocar con otras apps.
const CATALOGO_SYNC_KEY = "PROVSOFT_APPTABLALIQUIDOS_CATALOGO_SYNC_V1";
const CATALOGO_REFRESCO_MS = 24 * 60 * 60 * 1000;
...
let productosActivosBloqueo = [];
let productosActivosBloqueoCargados = false;
...
// Cache persistente de movimientos. Firebase sigue siendo la fuente oficial,
// pero el histórico ya descargado se conserva en IndexedDB para no volver a
// leerlo completo en cada F5/recarga del navegador.
const DB_CACHE_MOVIMIENTOS = "PROVSOFT_LIQUIDOS_MOVIMIENTOS_V1";
const STORE_CACHE_MOVIMIENTOS = "cache";
const KEY_CACHE_MOVIMIENTOS = "movimientos";
const VERSION_CACHE_MOVIMIENTOS = 1;
...
let cacheMovimientos = {
  ...
  cargadoHasta: "",
  inicializado: false,
  restauradoDeIndexedDB: false,
  ultimaSincronizacion: 0
};

function abrirDbCacheMovimientos() {
  return new Promise((resolve, reject) => {
    if (!("indexedDB" in window)) {
      reject(new Error("IndexedDB no disponible"));
      return;
    }
    const req = indexedDB.open(DB_CACHE_MOVIMIENTOS, VERSION_CACHE_MOVIMIENTOS);
    req.onupgradeneeded = () => {
      const dbLocal = req.result;
      if (!dbLocal.objectStoreNames.contains(STORE_CACHE_MOVIMIENTOS)) {
        dbLocal.createObjectStore(STORE_CACHE_MOVIMIENTOS);
      }
    };
...
async function leerCacheMovimientosIndexedDB() {
  try {
    const dbLocal = await abrirDbCacheMovimientos();
    const guardado = await new Promise((resolve, reject) => {
      const tx = dbLocal.transaction(STORE_CACHE_MOVIMIENTOS, "readonly");
      const req = tx.objectStore(STORE_CACHE_MOVIMIENTOS).get(KEY_CACHE_MOVIMIENTOS);
      req.onsuccess = () => resolve(req.result || null);
      req.onerror = () => reject(req.error);
    });
    dbLocal.close();

    if (!guardado || guardado.fechaBase !== FECHA_BASE_INVENTARIO) return false;

    cacheMovimientos = {
      salidas: Array.isArray(guardado.salidas) ? guardado.salidas : [],
      entradas: Array.isArray(guardado.entradas) ? guardado.entradas : [],
      traszap: Array.isArray(guardado.traszap) ? guardado.traszap : [],
      ajustes: Array.isArray(guardado.ajustes) ? guardado.ajustes : [],
      cargadoHasta: String(guardado.cargadoHasta || ""),
      inicializado: Boolean(guardado.inicializado && guardado.cargadoHasta),
      restauradoDeIndexedDB: Boolean(guardado.inicializado && guardado.cargadoHasta),
      ultimaSincronizacion: Number(guardado.ultimaSincronizacion || 0)
    };

    return cacheMovimientos.inicializado;
  } catch (error) {
    console.warn("No se pudo leer cache de movimientos desde IndexedDB:", error);
    return false;
  }
}

async function guardarCacheMovimientosIndexedDB() {
  try {
    const dbLocal = await abrirDbCacheMovimientos();
    const payload = {
      fechaBase: FECHA_BASE_INVENTARIO,
      salidas: cacheMovimientos.salidas,
      entradas: cacheMovimientos.entradas,
      traszap: cacheMovimientos.traszap,
      ajustes: cacheMovimientos.ajustes,
      cargadoHasta: cacheMovimientos.cargadoHasta,
      inicializado: cacheMovimientos.inicializado,
      ultimaSincronizacion: Date.now()
    };
    await new Promise((resolve, reject) => {
      const tx = dbLocal.transaction(STORE_CACHE_MOVIMIENTOS, "readwrite");
      tx.objectStore(STORE_CACHE_MOVIMIENTOS).put(payload, KEY_CACHE_MOVIMIENTOS);
      tx.oncomplete = () => resolve();
      tx.onerror = () => reject(tx.error);
      tx.onabort = () => reject(tx.error || new Error("IndexedDB abortó la escritura"));
    });
    dbLocal.close();
    cacheMovimientos.ultimaSincronizacion = payload.ultimaSincronizacion;
  } catch (error) {
    console.warn("No se pudo guardar cache de movimientos en IndexedDB:", error);
  }
}

function reemplazarRangoCache(destino, nuevos, inicio, fin) {
  const fueraDelRango = destino.filter(item => item.fecha < inicio || item.fecha > fin);
  destino.length = 0;
  destino.push(...fueraDelRango);
  anexarSinDuplicados(destino, nuevos);
}

function reemplazarDocumentoCache(destino, docId, nuevos) {
  const conservados = destino.filter(item => item.docId !== docId);
  destino.length = 0;
  destino.push(...conservados, ...nuevos);
}
...
function normalizarTextoBusqueda(valor) {
  return repararTexto(String(valor ?? ""))
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
...
function obtenerCatalogoBusquedaBloqueados() {
  const mapa = new Map();

  productosActivosBloqueo.forEach(p => {
    const codigo = String(p?.codigoBarra || p?.codigo || p?.id || "").trim();
    const codigoKey = normalizarCodigo(codigo);
    const nombre = repararTexto(String(p?.concepto || p?.descripcion || p?.nombre || "").trim());
    if (!codigo && !codigoKey) return;
    const key = codigoKey || codigo.toLowerCase();
    mapa.set(key, { codigo: codigo || codigoKey, codigoKey, nombre });
  });

  registrosPivot.forEach(row => {
    const codigo = String(row?.codigo || row?.codigoKey || "").trim();
    const codigoKey = normalizarCodigo(row?.codigoKey || codigo);
    const nombre = repararTexto(String(row?.nombre || "").trim());
    if (!codigo && !codigoKey) return;
    const key = codigoKey || codigo.toLowerCase();
    mapa.set(key, { codigo: codigo || codigoKey, codigoKey, nombre });
  });

  Object.entries(descripcionesBloqueadosCache).forEach(([codigo, info]) => {
    const codigoKey = normalizarCodigo(codigo);
    const key = codigoKey || String(codigo).toLowerCase();
    if (!mapa.has(key)) {
      mapa.set(key, {
        codigo: String(codigo).trim(),
        codigoKey,
        nombre: repararTexto(String(info?.concepto || "").trim())
      });
    }
  });
...
function puntuarCoincidenciaProductoBloqueado(producto, consulta) {
  const q = normalizarTextoBusqueda(consulta);
  if (!q) return 0;

  const codigo = normalizarTextoBusqueda(producto.codigo);
  const codigoKey = normalizarTextoBusqueda(producto.codigoKey);
  const nombre = normalizarTextoBusqueda(producto.nombre);
  const combinado = `${codigo} ${codigoKey} ${nombre}`.trim();
  const tokens = q.split(" ").filter(Boolean);

  // Multibúsqueda: todas las palabras escritas deben existir en código o descripción,
  // sin importar el orden en que fueron capturadas.
  if (!tokens.every(token => combinado.includes(token))) return 0;

  let score = 1000;
  if (codigo === q || codigoKey === q) score += 10000;
  if (nombre === q) score += 9000;
  if (nombre.startsWith(q)) score += 5000;
  if (nombre.includes(q)) score += 3500;
  if (codigo.startsWith(q) || codigoKey.startsWith(q)) score += 3000;

  // Favorece productos donde las palabras están más juntas y comienzan palabras reales.
  tokens.forEach(token => {
    if (nombre.split(" ").some(p => p === token)) score += 250;
    else if (nombre.split(" ").some(p => p.startsWith(token))) score += 150;
    if (codigo.includes(token) || codigoKey.includes(token)) score += 100;
  });

  score -= Math.min(nombre.length, 250) / 10;
  return score;
}

function buscarProductosParaBloqueo(consulta, limite = 40) {
  const q = String(consulta || "").trim();
  if (!q) return [];

  return obtenerCatalogoBusquedaBloqueados()
    .map(producto => ({ producto, score: puntuarCoincidenciaProductoBloqueado(producto, q) }))
    .filter(x => x.score > 0)
    .sort((a, b) => b.score - a.score || String(a.producto.nombre).localeCompare(String(b.producto.nombre), "es"))
    .slice(0, limite)
    .map(x => x.producto);
}

function pintarResultadosBusquedaBloqueados() {
  const input = $("nuevoCodigoBloqueado");
  const cont = $("resultadosBusquedaBloqueados");
  ...
  const resultados = buscarProductosParaBloqueo(q);
  cont.classList.remove("oculto");

  if (!resultados.length) {
    cont.innerHTML = `<div class="cb-busqueda-vacia">Sin coincidencias en el catálogo cargado. Si tienes el código exacto, puedes presionar <b>Agregar</b>.</div>`;
    return;
  }

  cont.innerHTML = resultados.map((p, i) => {
    const bloqueado = codigosBloqueadosAdmin.includes(String(p.codigo));
    return `<button type="button" class="cb-resultado${bloqueado ? " bloqueado" : ""}" data-cb-resultado="${escapeHtml(p.codigo)}" ${bloqueado ? "disabled" : ""}>
      <span class="cb-resultado-codigo">${escapeHtml(p.codigo)}</span>
      <span class="cb-resultado-nombre">${escapeHtml(p.nombre || "SIN DESCRIPCIÓN")}</span>
      <em>${bloqueado ? "Bloqueado" : (i === 0 ? "Mejor coincidencia" : "Seleccionar")}</em>
    </button>`;
  }).join("");

  cont.querySelectorAll("[data-cb-resultado]").forEach(btn => {
    btn.addEventListener("click", async () => {
      if (btn.disabled) return;
      input.value = btn.dataset.cbResultado || "";
      cont.innerHTML = "";
      cont.classList.add("oculto");
      await agregarCodigoBloqueado();
    });
  });
}

async function cargarCatalogoBloqueadosLocal() {
  if (productosActivosBloqueoCargados) return;

  const estado = $("estadoCodigoBloqueado");
  const input = $("nuevoCodigoBloqueado");
  const btnAgregar = $("btnAgregarCodigoBloqueado");
  const capaCarga = $("cargaCatalogoBloqueados");
  const textoCarga = $("textoCargaCatalogoBloqueados");

  const bloquearBuscador = (mensaje) => {
    if (textoCarga) textoCarga.textContent = mensaje || "Descargando catálogo de productos…";
    capaCarga?.classList.remove("oculto");
    if (input) input.disabled = true;
    if (btnAgregar) btnAgregar.disabled = true;
  };
  const habilitarBuscador = () => {
    capaCarga?.classList.add("oculto");
    if (input) input.disabled = false;
    if (btnAgregar) btnAgregar.disabled = false;
  };

  try {
    if (estado) estado.textContent = "Revisando catálogo local…";

    // Primero IndexedDB. Si ya existe catálogo, el usuario puede buscar de inmediato.
    productosActivosBloqueo = await cargarProductosActivosLocal();

    if (!productosActivosBloqueo.length) {
      // Navegador/celular nuevo: no dejamos usar el buscador con un catálogo vacío.
      bloquearBuscador("Descargando catálogo de productos…");
      if (estado) estado.textContent = "Descargando catálogo por primera vez…";
      productosActivosBloqueo = await sincronizarProductosActivosLocal();
      try { localStorage.setItem(CATALOGO_SYNC_KEY, String(Date.now())); } catch {}
    } else {
      // Hay catálogo local: habilitamos inmediatamente. La actualización de 24 h va al fondo.
      habilitarBuscador();
      let ultima = 0;
      try { ultima = Number(localStorage.getItem(CATALOGO_SYNC_KEY) || 0); } catch {}
      if (!ultima || (Date.now() - ultima) >= CATALOGO_REFRESCO_MS) {
        sincronizarProductosActivosLocal()
          .then(actualizados => {
            productosActivosBloqueo = actualizados;
            try { localStorage.setItem(CATALOGO_SYNC_KEY, String(Date.now())); } catch {}
            if (estado) estado.textContent = `${actualizados.length.toLocaleString("es-MX")} productos disponibles localmente`;
            pintarResultadosBusquedaBloqueados();
          })
          .catch(error => console.warn("Se conserva IndexedDB; no se pudo refrescar catálogo:", error));
      }
    }

    if (estado) estado.textContent = `${productosActivosBloqueo.length.toLocaleString("es-MX")} productos disponibles localmente`;
  } catch (error) {
    console.warn("No se pudo preparar catálogo para bloqueos:", error);
    productosActivosBloqueo = [];
    if (estado) estado.textContent = "No se pudo descargar el catálogo. Cierra y vuelve a intentar.";
  } finally {
    productosActivosBloqueoCargados = productosActivosBloqueo.length > 0;
    habilitarBuscador();
    pintarResultadosBusquedaBloqueados();
  }
}

async function prepararCatalogoDesdeInicio() {
  setStatus("Preparando catálogo de productos…");
  setProgress(6);

  let productosLocales = [];
  try {
    productosLocales = await cargarProductosActivosLocal();
  } catch (error) {
    console.warn("No se pudo leer catálogo local al iniciar:", error);
  }

  let ultimaSync = 0;
  try { ultimaSync = Number(localStorage.getItem(CATALOGO_SYNC_KEY) || 0); } catch {}

  const necesitaPrimeraDescarga = productosLocales.length === 0;
  const necesitaActualizar = !ultimaSync || (Date.now() - ultimaSync) >= CATALOGO_REFRESCO_MS;

  if (necesitaPrimeraDescarga || necesitaActualizar) {
    setProgress(10);
    setStatus(necesitaPrimeraDescarga
      ? "Descargando catálogo de productos por primera vez…"
      : `Actualizando catálogo local (${productosLocales.length.toLocaleString("es-MX")} productos guardados)…`);

    try {
      productosLocales = await sincronizarProductosActivosLocal();
      try { localStorage.setItem(CATALOGO_SYNC_KEY, String(Date.now())); } catch {}
    } catch (error) {
      // Si ya existe una bolsa local seguimos trabajando con ella. Sólo detenemos
      // la entrada cuando se trata de un dispositivo nuevo sin catálogo alguno.
      if (!productosLocales.length) throw error;
      console.warn("No se pudo refrescar catálogo; se conserva la copia local:", error);
    }
  }

  productosActivosBloqueo = productosLocales;
  productosActivosBloqueoCargados = productosLocales.length > 0;
  setProgress(24);
  setStatus(`Catálogo listo: ${productosLocales.length.toLocaleString("es-MX")} productos activos. Preparando inventario…`);
  return productosLocales.length;
}
...
function abrirCodigosBloqueadosAdmin() {
  ...
  const resultados = $("resultadosBusquedaBloqueados");
  if (resultados) { resultados.innerHTML = ""; resultados.classList.add("oculto"); }
...
async function cargarMovimientosHasta(fin, forzar = false, inicioVisible = FECHA_BASE_INVENTARIO) {
  let inicioConsulta = FECHA_BASE_INVENTARIO;
  let reemplazarRango = false;

  if (forzar) {
    // Refrescar ya no borra todo el histórico. Sólo vuelve a consultar la semana
    // visible y sustituye ese tramo en cache. Así detecta altas/cambios/bajas
    // recientes sin castigar Firebase con lecturas desde el punto cero.
    inicioConsulta = inicioVisible < FECHA_BASE_INVENTARIO ? FECHA_BASE_INVENTARIO : inicioVisible;
    reemplazarRango = true;
    configuracionTraszap = null;
  } else if (cacheMovimientos.inicializado) {
    if (fin <= cacheMovimientos.cargadoHasta) {
      if (cacheMovimientos.restauradoDeIndexedDB) {
        // Tras F5 usamos de inmediato el histórico local y sólo revalidamos el
        // último día visible una vez. Esto recoge movimientos nuevos del día
        // sin volver a bajar toda la semana ni todo el histórico.
        inicioConsulta = fin;
        reemplazarRango = true;
        cacheMovimientos.restauradoDeIndexedDB = false;
      } else {
        return;
      }
    } else {
  ...
  setStatus(
    reemplazarRango
      ? `Sincronizando cambios ${fechaCorta(inicioConsulta)} a ${fechaCorta(fin)}...`
      : `Descargando movimientos nuevos ${fechaCorta(inicioConsulta)} a ${fechaCorta(fin)}...`
  );
  ...
  if (reemplazarRango) {
    reemplazarRangoCache(cacheMovimientos.salidas, salidas.detalle, inicioConsulta, fin);
    reemplazarRangoCache(cacheMovimientos.entradas, entradas.detalle, inicioConsulta, fin);
    reemplazarRangoCache(cacheMovimientos.traszap, traszap.detalle, inicioConsulta, fin);
    reemplazarRangoCache(cacheMovimientos.ajustes, ajustes.detalle, inicioConsulta, fin);
  } else {
  ...
  if (!cacheMovimientos.cargadoHasta || fin > cacheMovimientos.cargadoHasta) {
    cacheMovimientos.cargadoHasta = fin;
  }
  cacheMovimientos.inicializado = true;
  cacheMovimientos.restauradoDeIndexedDB = false;
  await guardarCacheMovimientosIndexedDB();
}
...
async function cargarSalidasZapata(forzarRecarga = false) {
  try {
    ...
    setProgress(20);
    await cargarMovimientosHasta(rango.fin, forzarRecarga, rango.inicio);
    setProgress(65);
    ...
    setStatus(
      ...
      `Cache local hasta ${fechaCorta(cacheMovimientos.cargadoHasta)}.`
    );
...
function inicializarEventos() {
  ...
  $("nuevoCodigoBloqueado")?.addEventListener("input", pintarResultadosBusquedaBloqueados);
  $("nuevoCodigoBloqueado")?.addEventListener("keydown", event => {
    if (event.key === "Enter") {
      event.preventDefault();
      const primero = $("resultadosBusquedaBloqueados")?.querySelector("[data-cb-resultado]:not(:disabled)");
      if (primero) primero.click();
      else agregarCodigoBloqueado();
    }
  });
...
function aplicarEntradaGeneradaLocal(entrada) {
  ...
  if (detalleEntrada.length) {
    const docIdCache = detalleEntrada[0].docId;
    reemplazarDocumentoCache(cacheMovimientos.entradas, docIdCache, detalleEntrada);
    cacheMovimientos.inicializado = true;
    if (!cacheMovimientos.cargadoHasta || detalleEntrada[0].fecha > cacheMovimientos.cargadoHasta) {
      cacheMovimientos.cargadoHasta = detalleEntrada[0].fecha;
    }
    guardarCacheMovimientosIndexedDB();
  }
...
function ajusteGuardadoADetalleMovimientos(payload) {
  const docId = String(payload?.docId || payload?.folio || "").trim();
  const folio = String(payload?.folio || docId).trim();
  ...
  return partidas.map((p, idx) => {
    ...
    if (!docId || !fecha || !codigoKey) return null;
    const diferencia = Number(p.diferencia ?? p.cantidad ?? 0);
    return {
      tipo: "AJUINV",
      docId,
      partida: p.partida || codigoKey || idx + 1,
      folio,
      fecha,
      hora: String(p.hora_movimiento || "").trim(),
      codigo: codigoOriginal,
      codigoKey,
...
async function aplicarAjusteGuardadoLocal(payload) {
  const detalle = ajusteGuardadoADetalleMovimientos(payload);
  if (!detalle.length) return;

  const docId = detalle[0].docId;
  reemplazarDocumentoCache(cacheMovimientos.ajustes, docId, detalle);
  cacheMovimientos.inicializado = true;
  if (!cacheMovimientos.cargadoHasta || detalle[0].fecha > cacheMovimientos.cargadoHasta) {
    cacheMovimientos.cargadoHasta = detalle[0].fecha;
  }
  await guardarCacheMovimientosIndexedDB();

  // Reconstruye la tabla usando la cache ya actualizada; no consulta Firebase.
  await cargarSalidasZapata(false);
  setStatus("Ajuste guardado y aplicado en cache local. No fue necesario recargar el histórico de Firebase.");
}
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // En celular especialmente, la app NO deja entrar a sus funciones hasta que
    // catálogo, cache principal y configuraciones de trabajo estén preparados.
    // Así al cambiar de módulo no aparecen descargas que debieron ocurrir al inicio.
    await prepararCatalogoDesdeInicio();

    setProgress(28);
    setStatus("Revisando datos guardados del inventario…");
    const cacheRestaurada = await leerCacheMovimientosIndexedDB();
    if (cacheRestaurada) {
      setStatus(`Inventario local encontrado hasta ${fechaCorta(cacheMovimientos.cargadoHasta)}. Sincronizando sólo lo necesario…`);
    }
    ...
    setProgress(72);
    setStatus("Preparando listas y configuraciones…");
    await Promise.all([
      cargarListadosResumenProveedor(),
      cargarListasResumen(),
      cargarMenuInicioConfig()
    ]);

    setProgress(94);
    setStatus("Datos preparados. Abriendo aplicación…");
    ...
    iniciarModuloAjustesInventarioZapata({
      ...
      onAjusteGuardado: async (payload) => {
        await aplicarAjusteGuardadoLocal(payload);
      }
    });

    // El módulo de entradas también queda conectado después de que el catálogo
    // común ya está listo en su IndexedDB exclusivo.
    ocultarLoader(true);
  } catch (error) {
    console.error("Error durante preparación inicial:", error);
    cargaInicialEnProceso = false;
    setProgress(100);
    setStatus("No se pudo preparar la aplicación: " + (error?.message || error));
    // Dejamos visible el loader para que en un celular nuevo no se entre con datos
    // incompletos. Un usuario con datos locales conserva el fallback de las cargas.
  }
});