...
async function modificarEntradaUI(id) {
  ...
  for (let i = 0; i < articulos.length; i++) {
    ...
    if (Number(a.cantidad_factura || 0) !== 0) a.factor_conversion = Number(a.cantidad_factura) / cantidad;
  }
...
async function aplicarFactoresCatalogoArticulos() {
  ...
  for (const articulo of articulosPreparados) {
    ...
    articulo.cantidad_entrada = Number(articulo.cantidad_factura || 0) / factor;
        ...
        <div class="revision-partida-body">
          ${enlazada ? `
            ...
            <div class="revision-calculo">
              <div><small>FACTOR / CAJA</small><b>${numeroCorto(factor)}</b></div>
              <span class="revision-multiplica">×</span>
              <div><small>CANTIDAD FACTURA</small><b>${numeroCorto(cantidadFactura)}</b></div>
              <span class="revision-igual">=</span>
              <div class="revision-resultado"><small>ENTRADA</small><b>${numeroCorto(cantidadEntrada)}</b><em>piezas</em></div>
            </div>
          ` : `
...
async function buscarProductoUI() {
  ...
  // 1) Siempre buscar primero dentro del catálogo construido por la tabla.
  ...
  // 2) Solo cuando no exista coincidencia local, consultar la bolsa del catálogo general.
  const contenedor = document.getElementById("listaProductos");
  contenedor.innerHTML = "<p>Sin coincidencia en la tabla. Buscando en catálogo general...</p>";
  try {
    await asegurarProductosActivos();
    const generales = filtrarProductosCatalogo(productosActivos, texto);
    pintarProductos(generales, "general");
  } catch (error) {
...
function sincronizarCantidadDesdeFactor() {
  ...
  document.getElementById("cantidadEntradaProducto").value =
    numeroInput(factor > 0 ? cantidadFactura / factor : 0);
}
...
function sincronizarFactorDesdeCantidad() {
  ...
  if (cantidadFactura > 0) {
    document.getElementById("factorConversionProducto").value =
      numeroInput(cantidadEntrada > 0 ? cantidadFactura / cantidadEntrada : 1);
  }
}
