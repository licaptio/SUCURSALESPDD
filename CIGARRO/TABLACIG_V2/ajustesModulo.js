...
export function iniciarModuloAjustesInventarioCigarro(opciones = {}) {
  opcionesModulo = {
    ...opcionesModulo,
    ...opciones
  };
...
async function guardarAjuste() {
  ...
  const folio = `AJUINV-${fecha.replaceAll("-", "")}-${hora.replace(":", "")}`;
  ...
  try {
    ...
    let telegramOk = true;
    try {
      await enviarAjusteTelegram({ folio, ...partida });
    } catch (telegramError) {
      telegramOk = false;
    ...
    alert(telegramOk
      ? `Ajuste guardado y notificado. Diferencia: ${fmt(diferencia)}`
