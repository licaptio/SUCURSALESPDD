...
async function cargarInventarioInicial() {
  // Punto cero operativo solicitado: 01/06/2026.
  // No se lee ningún conteo anterior; todos los artículos comienzan en cero.
  inventarioInicialOriginal = {};
  setStatus("Punto cero cargado: inventario inicial en 0 al 01/06/2026.");
  setProgress(8);
...
async function consultarSalidas(inicio, fin) {
  ...
  // Las salidas guardan fecha como ISO completa; se filtra en memoria para
  // aceptar tanto YYYY-MM-DD como YYYY-MM-DDTHH:mm:ss.sssZ.
...
async function consultarEntradas(inicio, fin) {
  ...
  const snap = await getDocs(REF_ENTRADAS_ZAPATA);
  const detalle = [];
  const docsVistos = new Set();
  ...
  snap.forEach((documento) => {
    ...
    articulos.forEach((art, idx) => {
      ...
      detalle.push({
        ...
        entrega: String(data.entrega || data.proveedor || "").trim(),
        recibe: String(data.recibe || data.usuario || "").trim(),
        folioCincho: "",
        proveedor: String(data.proveedor || data.entrega || "").trim(),
        alias_pivot: String(data.proveedor || data.entrega || "").trim(),
        rfc_emisor: "",
        razon_social_emisor: "",
        codigo: codigoOriginal,
        codigoKey,
  ...
  if ($("selectorSemana")) {
    ...
    $("selectorSemana").addEventListener("change", async (event) => {
      if (!event.target.value || event.target.value < SEMANA_MINIMA) {
        ...
        alert("El punto cero inicia el 01/06/2026. No se permiten semanas anteriores.");
      }
