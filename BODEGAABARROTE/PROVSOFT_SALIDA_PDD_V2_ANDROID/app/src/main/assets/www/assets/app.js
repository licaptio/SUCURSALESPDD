const APP_BUILD_VERSION='V44-ANDROID-BLUETOOTH-ESC-POS-58MM';
...
const PRINTER_DEFAULTS={paperMm:58,charsPerLine:32,protocol:'ESC/POS',autoPrint:true,printLogo:true,printSignature:true,feedLines:4,partialCut:true,device:null};
...
async function printSalida58(salida,{test=false}={}){
  const cfg=printerConfigRead(),bytes=await buildEscPos58Bytes(salida,cfg),dataBase64=uint8ToBase64(bytes),bridge=getNativePrinterBridge();
...
async function printerConfigModal(){
  ...
  open(`<h2>Impresora térmica 58 mm</h2>
    ...
    <div style="padding:12px;border:1px solid #d7dce2;border-radius:12px;margin:6px 0 10px"><b id="printerDeviceName">${esc(deviceName)}</b><br><small>${cfg.device?.address?esc(cfg.device.address):bridge?'Puente Android detectado · falta seleccionar dispositivo':'PWA · selección Bluetooth disponible al instalar Android'}</small></div>
    <button id="printerSelect" class="secondary" style="width:100%;margin-bottom:14px">SELECCIONAR IMPRESORA</button>
    ...
    <label><input id="printerCut" type="checkbox" ${cfg.partialCut!==false?'checked':''}> Enviar comando de corte parcial</label>
    <p><small>Puente Android: <b>${bridge?'DETECTADO':'NO DISPONIBLE EN PWA'}</b></small></p>
    <div class="final-actions"><button id="printerPreview" class="secondary">VER TICKET DE PRUEBA</button><button id="printerDiag" class="secondary">DIAGNÓSTICO ESC/POS</button><button id="printerTest" class="primary">IMPRIMIR PRUEBA</button></div>
    <button id="printerSave" class="primary sticky-modal-btn">GUARDAR CONFIGURACIÓN</button>`);
  const saveCfg=()=>printerConfigWrite({charsPerLine:Number($('printerCols').value)||32,autoPrint:$('printerAuto').checked,printLogo:$('printerLogo').checked,printSignature:$('printerSignature').checked,partialCut:$('printerCut').checked,device:printerConfigRead().device||null});
  $('printerSelect').onclick=async()=>{saveCfg();const r=await selectNativePrinter();if(r.ok){$('printerDeviceName').textContent=printerDeviceLabel(r.device);toast('Impresora predeterminada guardada');return}if(!r.native)return alert('La selección de impresora Bluetooth se habilitará en la APK Android.
...
La PWA conserva toda la configuración y el motor ESC/POS.');if(r.reason==='SELECT_NOT_SUPPORTED')return alert('El puente Android fue detectado, pero todavía no incluye el selector de impresora.');alert('No se pudo seleccionar la impresora: '+(r.error||r.reason||'error desconocido'))};
  ...
  $('printerPreview').onclick=()=>{saveCfg();openTicket58Preview(printerTestSalida(),{title:'Vista previa de prueba 58 mm'})};
  $('printerDiag').onclick=async()=>{saveCfg();const d=await printerEscPosDiagnostic();open(`<h2>Diagnóstico ESC/POS 58 mm</h2>${printerDiagnosticHtml(d)}<button id="diagClose" class="primary sticky-modal-btn">CERRAR</button>`);$('diagClose').onclick=()=>{close();printerConfigModal()}};
  $('printerTest').onclick=async()=>{saveCfg();const r=await printSalida58(printerTestSalida(),{test:true});if(r.ok)return alert('Ticket de prueba enviado a la impresora.');if(!r.native)return alert('El ticket ESC/POS de 58 mm se generó correctamente.

En la PWA todavía no existe el puente Bluetooth Android.');alert('Android recibió la orden, pero la impresión falló: '+(r.error||r.reason||'error desconocido'))}
}
window.PROVSOFT_ESC_POS_58={buildBytes:buildEscPos58Bytes,buildPreview:buildEscPos58TextPreview,previewHtml:ticket58PreviewHtml,openPreview:openTicket58Preview,print:printSalida58,selectPrinter:selectNativePrinter,diagnostic:printerEscPosDiagnostic,getConfig:printerConfigRead,setConfig:printerConfigWrite,firmaRaster:firmaEscPosRasterBytes,printStatus:printStatusBase,reasonText:printerReasonText};
