...
const CACHE_VERSION_INVENTARIO = "cig-v14-cadena-semanal-selector-bloqueado";
...
// V13: sincronización económica en tiempo real.
// Entradas/CIGMAN: nuevos documentos por creado_en.
// Ajustes: nuevos y modificados por actualizado_en.
// Salidas históricas no tienen timestamp de sincronización; se vigila el conteo
// y solo se relee la colección completa cuando el conteo cambia.
let realtimeUnsubs = [];
let monitorSalidasInterval = null;
let realtimeQueue = Promise.resolve();
const SOLAPE_SYNC_MS = 5 * 60 * 1000;
const INTERVALO_VIGILANCIA_SALIDAS_MS = 60 * 1000;
...
async function refrescarSoloSalidasPorCambio() {
  const rango = obtenerRangoSemana();
  setStatus("Cambio en salidas detectado. Actualizando únicamente salidas...");
  const consulta = await consultarSalidas(FECHA_BASE_INVENTARIO, rango.fin);
  registrosDetalleSemana = consulta.detalle.filter(x => x.fecha >= rango.inicio && x.fecha <= rango.fin);
  registrosDetalleAcumuladoAnterior = rango.acumuladoAnteriorFin >= FECHA_BASE_INVENTARIO
    ? consulta.detalle.filter(x => x.fecha >= FECHA_BASE_INVENTARIO && x.fecha <= rango.acumuladoAnteriorFin)
    : [];
  await reconstruirPivotTrasCambioRealtime(FECHA_BASE_INVENTARIO,
    "Salidas actualizadas automáticamente. Pivot recalculado.");
}

async function revisarConteoSalidas({ forzar = false } = {}) {
  try {
    const metaKey = "conteo_salidas";
    const previo = await leerMetaSync(metaKey);
    const snapCount = await getCountFromServer(REF_SALIDAS_CIGARRO);
    const actual = Number(snapCount.data().count || 0);
    const anterior = previo == null ? null : Number(previo.valor);

    if (anterior == null) {
      await guardarMetaSync(metaKey, actual);
      return;
    }

    if (actual !== anterior) {
      await guardarMetaSync(metaKey, actual);
      await refrescarSoloSalidasPorCambio();
    } else if (forzar) {
      setStatus("Sin cambios nuevos en salidas. Pivot local vigente.");
    }
  } catch (err) {
    console.warn("No se pudo verificar conteo de salidas:", err);
  }
}
...
async function iniciarSincronizacionTiempoReal() {
  try {
    ...
    await revisarConteoSalidas();
    if (monitorSalidasInterval) clearInterval(monitorSalidasInterval);
    monitorSalidasInterval = setInterval(() => {
      if (document.visibilityState === "visible") revisarConteoSalidas();
    }, INTERVALO_VIGILANCIA_SALIDAS_MS);

    window.addEventListener("focus", () => revisarConteoSalidas());
    window.addEventListener("online", () => revisarConteoSalidas());
    setStatus("Pivot listo · sincronización automática activa con ahorro de lecturas.");
  } catch (err) {
