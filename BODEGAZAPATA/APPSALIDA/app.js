...
async function cargarCatalogoGuiadoDesdeFirestore(origen='manual'){
  const d=await RUTAS.CONFIG_DOC.get({source:'server'}).catch(async e=>{
    console.warn('No se pudo leer servidor, usando cache Firestore:',e);
    return RUTAS.CONFIG_DOC.get();
  });

  if(!d.exists)throw new Error('No existe configuracion/catalogo_guiado');

  guia=normalizeGuia(d.data());
  await setKV('guia',guia);
  $('estadoConexion').textContent=`Catálogo actualizado ${new Date().toLocaleTimeString('es-MX',{hour:'2-digit',minute:'2-digit'})}`;
  return guia;
}
...
async function sync(){
  ...
  $('estadoConexion').textContent=ok?'Catálogo guiado sincronizado':'Modo local';
}

function iniciarEscuchaConfiguracion(){
  if(unsubGuiaConfig)unsubGuiaConfig();

  unsubGuiaConfig=RUTAS.CONFIG_DOC.onSnapshot(async snap=>{
    ...
    $('estadoConexion').textContent=snap.metadata.fromCache
      ? `Catálogo local ${hora}`
      : `Catálogo actualizado ${hora}`;

    // Refresca la pantalla abierta cuando cambia catalogo_guiado.
    ...
    $('estadoConexion').textContent='Modo local';
  });
}

window.addEventListener('focus',()=>{
  cargarCatalogoGuiadoDesdeFirestore('focus').catch(e=>console.warn(e));
});

document.addEventListener('visibilitychange',()=>{
  if(!document.hidden)cargarCatalogoGuiadoDesdeFirestore('visible').catch(e=>console.warn(e));
});
...
window.actualizarCatalogoManual=async()=>{
  try{
    ...
    alert('No se pudo actualizar catálogo: '+e.message);
  }
};

function normalizeGuia(raw){
  const rawArts=Array.isArray(raw.articulos)?raw.articulos:[];

  // Estados actuales de departamentos/familias si el documento trae estructura guiada.
  const estadoDep=new Map();
  const estadoFam=new Map();

  if(Array.isArray(raw.departamentos)){
    raw.departamentos.forEach((d,i)=>{
      const depNombre=d.nombre||d.departamento||'SIN DEPARTAMENTO';
      const dk=slug(d.id||depNombre);
      estadoDep.set(dk,{
  ...
  // FUENTE PRINCIPAL: raw.articulos[] porque ahí se editan/actualizan los artículos.
  // Si no existe, se rescatan los artículos de la estructura departamentos/familias.
  let baseArts=rawArts;

  if(!baseArts.length && Array.isArray(raw.departamentos)){
    baseArts=[];
    raw.departamentos.forEach(d=>{
      (d.familias||[]).forEach(f=>{
        (f.articulos||[]).forEach(a=>{
          baseArts.push({
            ...a,
            departamento:d.nombre||d.departamento||'SIN DEPARTAMENTO',
            familia:f.nombre||f.familia||'SIN FAMILIA'
          });
        });
      });
    });
  ...
  baseArts.forEach((a,i)=>{
    const codigo=String(a.codigo||a.Codigo||a.CODIGO||'').trim();
    if(!codigo)return;

    // Evita duplicados por código. Se queda con el último ordenado si viniera repetido.
    const key=normCod(codigo);
    if(seenCodigos.has(key))return;
    seenCodigos.add(key);

    const dep=a.departamento||a.departamentoNombre||a.depto||'SIN DEPARTAMENTO';
    const fam=a.familia||a.familiaNombre||'SIN FAMILIA';
    const dk=slug(dep);
    const fk=slug(fam);
    const depState=estadoDep.get(dk)||{};
    const famState=estadoFam.get(`${dk}|${fk}`)||{};

    if(!map.has(dk)){
      map.set(dk,{
        id:dk,
        nombre:depState.nombre||dep,
        icono:depState.icono||'📦',
        activo:depState.activo!==false,
        orden:+(depState.orden||a.orden||i+1),
        familiasMap:new Map()
      });
    }
    ...
    if(!d.familiasMap.has(fk)){
      d.familiasMap.set(fk,{
        id:fk,
        nombre:famState.nombre||fam,
        icono:famState.icono||'📁',
        activo:famState.activo!==false,
        orden:+(famState.orden||a.orden||i+1),
        articulos:[]
      });
    }
  ...
  const departamentos=[...map.values()].map(d=>{
    const familias=[...d.familiasMap.values()]
      .map(f=>({...f,articulos:f.articulos.sort((a,b)=>(a.orden||0)-(b.orden||0))}))
  ...
  return{...raw,departamentos,articulos:baseArts};
}
