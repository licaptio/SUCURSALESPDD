...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Arranque de MENUINICIO.
  // En celular MENUINICIO es obligatorio como primera pantalla:
  // - con artículos configurados, abre la revisión;
  // - sin configuración, abre directamente el configurador.
  // En PC se conserva el comportamiento anterior.
  const esCelular = window.matchMedia("(max-width: 760px)").matches;
  const revisionAbierta = iniciarRevisionMenuInicio();
  if (esCelular && !revisionAbierta) {
    abrirConfigMenuInicio();
  }
