...
function cargarDatosEnModal(encabezado) {
  $("fechaEntrada").value = encabezado.fecha_entrada || fechaLocalISO();
  $("fechaMovimiento").value = encabezado.fecha_movimiento || fechaLocalISO();
  $("horaMovimiento").value = encabezado.hora_movimiento || horaLocal();
  $("usuarioMovimiento").value = encabezado.usuario || "GERARDO RIOS";
  $("motivoMovimiento").value = encabezado.motivo || "CONTEO FÍSICO";
  $("observacionesMovimiento").value = encabezado.observaciones || "";
}

function editarDatosDocumento() {
  if (!encabezadoActual) { toast("Primero abre o crea un movimiento."); return; }
  cargarDatosEnModal(encabezadoActual);
...
async function guardarDatosMovimiento() {
  ...
  const fechaAnterior = encabezadoActual?.fecha_movimiento || null;
  ...
  if (modoEdicion && fechaAnterior && fechaAnterior !== fechaMovimiento && partidas.length) {
    for (const p of partidas) {
      const teorico = await calcularTeorico(p.codigoKey, fechaMovimiento);
      p.teorico = Number(teorico || 0);
      p.diferencia = Number(p.fisico || 0) - p.teorico;
    }
    toast("Fecha modificada. Teóricos recalculados.");
  }
...
async function abrirMovimientoGrabado(folio) {
  try {
    ...
    pintarDocumento();
    cargarDatosEnModal(encabezadoActual);
    toast("Movimiento abierto en modo edición completa.");
    abrirModal("modalDatos");
  } catch (e) {
    console.error(e);
    toast(e.message);
...
function bindEventos() {
  ...
  $("btnSiguienteDatos").addEventListener("click", guardarDatosMovimiento);
  $("btnAgregarArticulo").addEventListener("click", abrirModalArticulo);
  $("btnEditarDatosDocumento").addEventListener("click", editarDatosDocumento);
