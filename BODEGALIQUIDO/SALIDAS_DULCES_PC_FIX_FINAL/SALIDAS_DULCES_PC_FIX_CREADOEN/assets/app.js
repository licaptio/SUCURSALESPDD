...
const FUENTES_SALIDAS = [
  { id: "DULCES", nombre: "Almacén Dulces", ref: collection(db, "almacenes", "Almacen_Dulces", "salidas") }
];
const POR_PAGINA = 30;
...
function extraerFecha(data) {
  const candidatos = [data.creadoEn, data.creado_en, data.timestamp, data.createdAt, data.fechaHora, data.fecha, data.fecha_salida];
...
function extraerHora(data) {
  ...
  const candidatos = [data.creadoEn, data.creado_en, data.timestamp, data.createdAt, data.fechaHora, data.fecha];
  for (const v of candidatos) {
    const d = timestampToDate(v);
...
function normalizarSalida(docSnap, fuente = null) {
  const data = docSnap.data() || {};
  // Las salidas actuales guardan las partidas en `productos`.
  // Se conserva compatibilidad con versiones anteriores que usaban `articulos`.
  const partidasOrigen = Array.isArray(data.productos)
    ? data.productos
    : (Array.isArray(data.articulos) ? data.articulos : []);

  const articulos = partidasOrigen.map((a, i) => ({
    partida: i + 1,
    codigo: texto(a?.codigoBarra ?? a?.codigo ?? a?.sku ?? a?.id),
    nombre: texto(a?.concepto ?? a?.descripcion ?? a?.nombre),
    cantidad: Number(a?.cantidad ?? a?.cant ?? a?.piezas ?? 0),
    unidad: texto(a?.unidad)
  }));

  const salida = {
    id: `${fuente?.id || "SALIDA"}::${docSnap.id}`,
    docId: docSnap.id,
    almacen: fuente?.nombre || "",
    almacenId: fuente?.id || "",
  ...
  salida.buscar = [
    salida.id, salida.docId, salida.almacen, salida.folio, salida.fecha, salida.hora, salida.destino,
...
async function cargarPagina(numeroPagina, cursorInicio, forzarFirestore = false) {
  ...
  try {
    const partes = [orderBy("creadoEn", "desc")];
    if (cursorInicio) partes.push(startAfter(cursorInicio));
    partes.push(limit(POR_PAGINA));

    // Leemos únicamente Almacen_Dulces/salidas y pedimos primero el creadoEn más reciente.
    const resultados = await Promise.all(FUENTES_SALIDAS.map(async (fuente) => {
      const snap = await getDocs(query(fuente.ref, orderBy("creadoEn", "desc"), limit(POR_PAGINA)));
      return { fuente, snap };
    }));
    const docsPagina = resultados.flatMap(({ fuente, snap }) =>
      snap.docs.map(doc => ({ doc, fuente }))
    );

    estado.salidas = docsPagina.map(({doc, fuente}) => normalizarSalida(doc, fuente)).sort((a,b) => {
    ...
    estado.haySiguiente = false; // Recargar trae las 30 salidas más recientes de Almacén Dulces.
    estado.pagina = numeroPagina;

    // Cursor para la siguiente página: último documento leído en esta consulta.
    delete estado.cursores[numeroPagina + 1];
...
function iniciarEscuchaUltimaSalida() {
  // Escucha mínima: solo la salida con creadoEn más reciente.
  // No escucha toda la colección ni todas las páginas.
  const qUltima = query(FUENTES_SALIDAS[0].ref, orderBy("creadoEn", "desc"), limit(1));
...
<div class="sheet">
  <div class="head">
    ...
    <div class="title"><h1>SALIDA DE ALMACÉN</h1><div class="folio">${escapeHtml(s.folio || "—")}</div><p>ALMACÉN LÍQUIDOS</p></div>
  </div>
  ...
  <div class="foot">Documento generado desde el historial de Almacén Dulces.</div>
</div>
<script>
