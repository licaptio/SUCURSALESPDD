...
function makePdf(salida){
  ...
  estimatedH += 22;
  const pageH=Math.max(90,Math.min(1000,estimatedH));
  const pdf=new jsPDF({orientation:'portrait',unit:'mm',format:[80,pageH],compress:true,putOnlyUsedFonts:true});
  let y=7;
...
<button id="clear" class="secondary">BORRAR FIRMA</button>
  ...
  $('save').onclick=async()=>{
    ...
    // V25: BLOQUEO REAL EN LA PRIMERA ACCION DEL BOTON.
    // No se abre popup, PDF, RawBT ni se inicia Firebase antes de pintar este bloqueo.
    $('save').disabled=true;
    $('clear').disabled=true;
    $('btnRegresarCarritoFirma').disabled=true;
    c.style.pointerEvents='none';
    down=false;
    showLoad('GENERANDO SALIDA…');
    if($('bootHint'))$('bootHint').textContent='No cierres la aplicación. Estamos procesando la salida.';

    // Obligamos al navegador a pintar el overlay ANTES de iniciar cualquier trabajo.
    ...
    try{
      ...
      // 1) Primero protegemos la salida localmente.
      if(!queueSalidaLocal(salida))throw new Error('No fue posible proteger la salida en almacenamiento local.');

      // 2) PDF TERMICO LOCAL. No depende de internet y se crea mientras la app sigue bloqueada.
      showLoad('GENERANDO PDF TÉRMICO…');
      if($('bootHint'))$('bootHint').textContent='Formato real de 80 mm para impresora térmica.';
      await new Promise(resolve=>requestAnimationFrame(resolve));
      pdfBlob=makePdf(salida);
      downloadBlob(pdfBlob,`${folio}.pdf`);

      // 3) Sincronización. Seguimos bloqueados hasta terminar el intento completo.
      showLoad('GRABANDO SALIDA…');
      if($('bootHint'))$('bootHint').textContent='Guardando en Firebase y enviando comprobante.';
      await new Promise(resolve=>requestAnimationFrame(resolve));
      await syncPendingSalidas();

      const siguePendiente=pendingSalidasRead().some(x=>x?.folio===folio);

      // 4) Solo aquí se libera la operación.
      ro.disconnect();
      close();
