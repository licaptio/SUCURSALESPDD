...
// V20: PDF visible para impresión + sincronización de salidas + actualización automática de catálogos cada 24 horas. El arranque ya NO depende de Firebase/CDN. Todo lo remoto se carga
...
function makePdf(salida){
  // PDF TERMICO V24: pensado para rollo de 80 mm, sin firma y casi sin margenes.
  // La altura se calcula con base en las partidas para evitar una hoja A4 enorme.
  const pageW=80, margin=4, contentW=pageW-(margin*2);
  const estimateLines=(txt,chars=33)=>Math.max(1,Math.ceil(String(txt||'').length/chars));
  let estimatedH=76;
  for(const x of salida.partidas||[]){
    estimatedH += 13 + (estimateLines(`${x.renglon}. ${x.descripcion}`,30)-1)*4.3;
  }
  estimatedH += 28;
  const pageH=Math.max(120,Math.min(1200,estimatedH));
  const pdf=new jsPDF({unit:'mm',format:[pageW,pageH],orientation:'portrait',compress:true});
  let y=7;
  const center=pageW/2;
  const sep=()=>{pdf.setDrawColor(60);pdf.setLineWidth(.25);pdf.line(margin,y,pageW-margin,y);y+=3.5};
  const ensure=(need=8)=>{if(y+need>pageH-7){pdf.addPage([pageW,pageH],'portrait');y=7}};

  pdf.setTextColor(0);pdf.setFont('helvetica','bold');pdf.setFontSize(13);
  pdf.text('SALIDA ABARROTES PDD',center,y,{align:'center'});y+=5;
  pdf.setFontSize(10);pdf.text('PROVSOFT',center,y,{align:'center'});y+=4;sep();

  pdf.setFont('helvetica','normal');pdf.setFontSize(8.8);
  const info=[
    `FOLIO: ${salida.folio}`,
    `FECHA: ${salida.fechaCapturaTxt||salida.fechaCaptura||''}`,
    `HORA: ${salida.horaLocal||''}`,
    `DESTINO: ${salida.destino||''}`,
    `ENTREGA: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`,
    `RECIBE: ${salida.recibe||''}`
  ];
  for(const t of info){
    const lines=pdf.splitTextToSize(t,contentW);ensure(lines.length*4+1);pdf.text(lines,margin,y);y+=lines.length*4;
  }
  y+=1;sep();
  pdf.setFont('helvetica','bold');pdf.setFontSize(9.5);pdf.text('PARTIDAS',margin,y);y+=4;sep();

  for(const x of salida.partidas||[]){
    const desc=pdf.splitTextToSize(`${x.renglon}. ${x.descripcion}`,contentW);
    ensure(desc.length*4.2+10);
    pdf.setFont('helvetica','bold');pdf.setFontSize(9.2);pdf.text(desc,margin,y);y+=desc.length*4.2;
    pdf.setFont('helvetica','normal');pdf.setFontSize(8.3);
    pdf.text(`COD: ${x.codigo}`,margin,y);y+=3.8;
    const cajas=x.cajasSalieron!=null&&x.cantidadPorCaja!=null?`${x.cajasSalieron} cj x ${x.cantidadPorCaja} = `:'';
    pdf.setFont('helvetica','bold');pdf.setFontSize(9.5);pdf.text(`${cajas}${x.cantidad} PZAS`,margin,y);y+=4.5;
    pdf.setDrawColor(175);pdf.setLineWidth(.15);pdf.line(margin,y,pageW-margin,y);y+=3;
  }

  ensure(28);sep();
  pdf.setFont('helvetica','bold');pdf.setFontSize(11);
  pdf.text(`PARTIDAS: ${salida.totalPartidas}`,margin,y);y+=5;
  pdf.text(`UNIDADES: ${salida.totalUnidades}`,margin,y);y+=5;sep();
  pdf.setFontSize(9.5);pdf.text(`ENTREGA: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`,margin,y,{maxWidth:contentW});y+=5;
  pdf.text(`RECIBE: ${salida.recibe||''}`,margin,y,{maxWidth:contentW});y+=6;
  pdf.setFontSize(8.5);pdf.text('*** SALIDA REGISTRADA ***',center,y,{align:'center'});
...
function buildRawBtBytes(salida){
  ...
  lf('FIRMA: ________________________');
  lf('');
...
<button id="clear" class="secondary">BORRAR FIRMA</button>
  ...
  $('save').onclick=()=>{
    ...
    // Bloqueo inmediato: desde este punto ya no se puede seguir firmando ni tocar botones.
    $('save').disabled=true;$('clear').disabled=true;$('btnRegresarCarritoFirma').disabled=true;
    c.style.pointerEvents='none';
    showLoad('GENERANDO SALIDA…');
    if($('bootHint'))$('bootHint').textContent='Preparando ticket PDF térmico para impresión';
    try{
      ...
      // 1) Abrimos una ventana VACÍA en el mismo toque del usuario.
      // Esto evita el bloqueo de ventanas emergentes cuando el PDF termine de generarse.
      const pdfWindow=openPdfWindowNow();

      // 2) Primero queda protegida LOCALMENTE. No depende de que Firebase responda.
      if(!queueSalidaLocal(salida)){
        try{pdfWindow?.close()}catch{}
        throw new Error('No fue posible proteger la salida en almacenamiento local.');
      }

      // 3) El PDF se genera LOCALMENTE de inmediato; no espera Firebase ni internet.
      // Mientras se construye, la pantalla permanece bloqueada con GENERANDO SALIDA.
      requestAnimationFrame(()=>setTimeout(async()=>{
        const ok=await openSalidaPdfForPrint(salida,pdfWindow);
        ro.disconnect();close();
        ...
        showModeGate();
        toast(ok?'Salida generada · PDF térmico listo para imprimir':'Salida guardada · no se pudo abrir el PDF');
        // Firebase, catálogo operativo y Telegram van después y no bloquean al operador.
        setTimeout(()=>syncPendingSalidas(),0);
      },60));
    }catch(e){
      console.error(e);hideLoad();
      if($('bootHint'))$('bootHint').textContent='Verificando catálogos disponibles en este equipo';
      c.style.pointerEvents='';$('save').disabled=false;$('clear').disabled=false;$('btnRegresarCarritoFirma').disabled=false;
