const APP_BUILD_VERSION='V57-ANDROID-SPP-STABLE';
...
// V43: agrega control formal de estados, errores y reintentos de impresión ESC/POS 58 mm. El arranque ya NO depende de Firebase/CDN. Todo lo remoto se carga
...
let collection,getDocs,doc,getDoc,setDoc,serverTimestamp,collectionGroup,query,where,orderBy,limit,onSnapshot; // V32: un solo listener centinela, nunca listener masivo
...
async function ensureOnlineStack(timeoutMs=12000){
  ...
  if(!onlineStackPromise){
    onlineStackPromise=(async()=>{
      ...
      ({collection,getDocs,doc,getDoc,setDoc,serverTimestamp,collectionGroup,query,where,orderBy,limit,onSnapshot}=fs);
...
const IS_NATIVE_ANDROID=!!window.ProvsoftAndroidPrinter;
...
if(!IS_NATIVE_ANDROID&&'serviceWorker' in navigator)window.addEventListener('load',async()=>{
...
function isStandalonePwa(){return IS_NATIVE_ANDROID||matchMedia('(display-mode: standalone)').matches||navigator.standalone===true||localStorage.getItem('abarrotesPddInstalled')==='1'}
...
function makePdf(salida){
  ...
  pageH += 29 + (salida.firmaRecibe?18:0); pageH=Math.max(90,Math.min(900,Math.ceil(pageH)));
  ...
  textLines(`ENTREGA: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`,7.7,true);textLines(`RECIBE: ${salida.recibe||''}`,7.7,true);
  if(salida.firmaRecibe){
    y+=1;pdf.setFont('helvetica','bold');pdf.setFontSize(7.2);pdf.text('FIRMA DE RECIBIDO',center,y,{align:'center'});y+=1.2;
    try{pdf.addImage(salida.firmaRecibe,'PNG',6,y,46,13,undefined,'FAST');y+=14.2}catch(e){console.warn('[PDF FIRMA] No se pudo insertar firma:',e);y+=2}
  }
  y+=1;pdf.setFont('helvetica','bold');pdf.setFontSize(7.5);pdf.text('*** SALIDA REGISTRADA ***',center,y,{align:'center'});
...
// ===== V38 IMPRESION TERMICA 58 MM ESC/POS + LOGO + FIRMA + COLA DE SINCRONIZACION =====
...
const SALIDAS_HISTORY_KEY='abarrotesPddSalidasHistorialV43';
const PRINTER_CONFIG_KEY='provsoftPrinter58ConfigV1';
const PRINTER_DEFAULTS={paperMm:58,charsPerLine:32,protocol:'ESC/POS',autoPrint:true,printLogo:true,printSignature:true,feedLines:4,partialCut:false,device:null};
// V38: Logo exacto de PROVEEDORA preparado para ticket térmico 58 mm.
...
function salidasHistoryRead(){try{const a=JSON.parse(localStorage.getItem(SALIDAS_HISTORY_KEY)||'[]');return Array.isArray(a)?a:[]}catch{return[]}}
function salidasHistoryWrite(a){try{localStorage.setItem(SALIDAS_HISTORY_KEY,JSON.stringify((a||[]).slice(0,30)));return true}catch(e){console.warn('[HISTORIAL SALIDAS]',e);return false}}
function rememberSalidaHistory(salida){if(!salida?.folio)return false;const a=salidasHistoryRead().filter(x=>x?.folio!==salida.folio);a.unshift(salida);return salidasHistoryWrite(a)}
function printStatusBase(salida){return {...(salida?.impresion||{}),estado:salida?.impresion?.estado||'PENDIENTE',intentos:Number(salida?.impresion?.intentos||0)}}
function printerReasonText(reason,error=''){
  const map={NO_NATIVE_BRIDGE:'Puente Android no disponible',NO_DEVICE:'No hay impresora seleccionada',DEVICE_NOT_FOUND:'Impresora no encontrada',BLUETOOTH_OFF:'Bluetooth apagado',BLUETOOTH_DISABLED:'Bluetooth apagado',PERMISSION_DENIED:'Permiso Bluetooth denegado',NOT_CONNECTED:'No fue posible conectar con la impresora',CONNECTION_FAILED:'Falló la conexión Bluetooth',WRITE_FAILED:'Falló el envío de datos a la impresora',PAPER_OUT:'Impresora sin papel',COVER_OPEN:'Tapa de impresora abierta',TIMEOUT:'La impresora no respondió a tiempo',PRINT_ERROR:'Error al imprimir'};
  return [map[reason]||reason||'Error de impresión',error].filter(Boolean).join(' · ');
}
function applyPrintStatusLocal(salida,status){
  if(!salida?.folio)return salida;
  salida.impresion={...printStatusBase(salida),...status};
  const hist=salidasHistoryRead().map(x=>x?.folio===salida.folio?{...x,impresion:{...printStatusBase(x),...status}}:x);salidasHistoryWrite(hist);
  const pend=pendingSalidasRead().map(x=>x?.folio===salida.folio?{...x,impresion:{...printStatusBase(x),...status}}:x);pendingSalidasWrite(pend);
  return salida;
}
async function persistPrintStatus(salida,status){
  applyPrintStatusLocal(salida,status);
  try{
    if(db&&salida?.folio){await setDoc(doc(db,...R.salidas,salida.folio),{impresion:salida.impresion},{merge:true})}
  }catch(e){console.warn('[IMPRESION ESTADO] Firebase pendiente:',e)}
  return salida.impresion;
}
function printerStatusLabel(salida){const st=printStatusBase(salida);if(st.estado==='IMPRESO')return'✅ IMPRESO';if(st.estado==='ERROR')return'⚠️ ERROR';if(st.estado==='NO_SOLICITADA')return'— NO SOLICITADA';return'🕓 PENDIENTE'}
...
async function firmaEscPosRasterBytes(dataUrl,{maxWidth=320,maxHeight=110,threshold=210}={}){
  if(!dataUrl||typeof dataUrl!=='string'||!dataUrl.startsWith('data:image/'))return new Uint8Array();
  try{
    const img=await new Promise((resolve,reject)=>{const im=new Image();im.onload=()=>resolve(im);im.onerror=reject;im.src=dataUrl});
    const src=document.createElement('canvas');src.width=Math.max(1,img.naturalWidth||img.width);src.height=Math.max(1,img.naturalHeight||img.height);
    const sx=src.getContext('2d',{willReadFrequently:true});sx.clearRect(0,0,src.width,src.height);sx.drawImage(img,0,0);
    const raw=sx.getImageData(0,0,src.width,src.height),d=raw.data;
    let minX=src.width,minY=src.height,maxX=-1,maxY=-1;
    for(let y=0;y<src.height;y++)for(let x=0;x<src.width;x++){const i=(y*src.width+x)*4,a=d[i+3],lum=(d[i]+d[i+1]+d[i+2])/3;if(a>18&&lum<245){if(x<minX)minX=x;if(x>maxX)maxX=x;if(y<minY)minY=y;if(y>maxY)maxY=y}}
    if(maxX<minX||maxY<minY)return new Uint8Array();
    const pad=6;minX=Math.max(0,minX-pad);minY=Math.max(0,minY-pad);maxX=Math.min(src.width-1,maxX+pad);maxY=Math.min(src.height-1,maxY+pad);
    const cropW=maxX-minX+1,cropH=maxY-minY+1,scale=Math.min(maxWidth/cropW,maxHeight/cropH,1.7);
    let w=Math.max(8,Math.round(cropW*scale));const h=Math.max(8,Math.round(cropH*scale));w=Math.ceil(w/8)*8;
    const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d',{willReadFrequently:true});
    ctx.fillStyle='#fff';ctx.fillRect(0,0,w,h);ctx.imageSmoothingEnabled=true;ctx.drawImage(src,minX,minY,cropW,cropH,0,0,w,h);
    const px=ctx.getImageData(0,0,w,h).data,rowBytes=w>>3,data=new Uint8Array(rowBytes*h);let p=0;
    for(let y=0;y<h;y++)for(let xb=0;xb<rowBytes;xb++){let b=0;for(let bit=0;bit<8;bit++){const x=xb*8+bit,i=(y*w+x)*4,a=px[i+3],lum=.299*px[i]+.587*px[i+1]+.114*px[i+2];if(a>18&&lum<threshold)b|=(0x80>>bit)}data[p++]=b}
    const header=new Uint8Array([29,118,48,0,rowBytes&255,(rowBytes>>8)&255,h&255,(h>>8)&255]);const out=new Uint8Array(header.length+data.length);out.set(header,0);out.set(data,header.length);return out
  }catch(e){console.warn('[ESC/POS FIRMA] No se pudo rasterizar la firma:',e);return new Uint8Array()}
}
async function buildEscPos58Bytes(salida,opts={}){
  const cfg={...printerConfigRead(),...opts,paperMm:58,protocol:'ESC/POS'}; const cols=Math.max(24,Math.min(42,Number(cfg.charsPerLine)||32));
  ...
  push(27,64);push(27,97,1);
  if(cfg.printLogo!==false){push(...PROVEEDORA_LOGO_ESC_POS_BYTES);lf('')}
  push(27,69,1);lf('SALIDA ABARROTES PDD');push(27,69,0);lf('PROVSOFT');rule();push(27,97,0);push(27,77,0);
  ...
  rule();push(27,69,1);lf(`PARTIDAS: ${salida.totalPartidas}`);lf(`UNIDADES: ${salida.totalUnidades}`);push(27,69,0);rule();ticketLines(`ENTREGA: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`,cols).forEach(lf);lf('');ticketLines(`RECIBE: ${salida.recibe||''}`,cols).forEach(lf);lf('');
  if(cfg.printSignature!==false&&salida.firmaRecibe){push(27,97,1);push(27,69,1);lf('FIRMA DE RECIBIDO');push(27,69,0);const firmaBytes=await firmaEscPosRasterBytes(salida.firmaRecibe);if(firmaBytes.length){push(...firmaBytes);lf('')}push(27,97,0)}
  push(27,97,1);push(27,69,1);lf('*** SALIDA REGISTRADA ***');push(27,69,0);push(27,97,0);
  for(let i=0;i<Math.max(2,Math.min(8,Number(cfg.feedLines)||4));i++)lf(''); return new Uint8Array(bytes)
}
function buildEscPos58TextPreview(salida){
  const cfg=printerConfigRead(),cols=cfg.charsPerLine||32,lines=[];const lf=t=>lines.push(asciiTicket(t||'')),rule=()=>lf('-'.repeat(cols));if(cfg.printLogo!==false)lf(centerTicket('[LOGO PROVEEDORA]',cols));lf(centerTicket('SALIDA ABARROTES PDD',cols));lf(centerTicket('PROVSOFT',cols));rule();ticketLines(`FOLIO: ${salida.folio}`,cols).forEach(lf);lf(`FECHA: ${salida.fechaCapturaTxt||salida.fechaCaptura||''}`);lf(`HORA: ${salida.horaLocal||''}`);ticketLines(`DESTINO: ${salida.destino||''}`,cols).forEach(lf);ticketLines(`ENTREGA: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`,cols).forEach(lf);ticketLines(`RECIBE: ${salida.recibe||''}`,cols).forEach(lf);rule();lf('PARTIDAS');rule();for(const x of salida.partidas||[]){ticketLines(`${x.renglon}. ${x.descripcion}`,cols).forEach(lf);ticketLines(`COD: ${x.codigo}`,cols).forEach(lf);const cajas=x.cajasSalieron!=null&&x.cantidadPorCaja!=null?`${x.cajasSalieron} cj x ${x.cantidadPorCaja} = `:'';ticketLines(`${cajas}${x.cantidad} PZAS`,cols).forEach(lf);lf('')}rule();lf(`PARTIDAS: ${salida.totalPartidas}`);lf(`UNIDADES: ${salida.totalUnidades}`);rule();ticketLines(`ENTREGA: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`,cols).forEach(lf);lf('');ticketLines(`RECIBE: ${salida.recibe||''}`,cols).forEach(lf);lf('');if(cfg.printSignature!==false&&salida.firmaRecibe){lf(centerTicket('[FIRMA DE RECIBIDO]',cols));lf('')}lf(centerTicket('*** SALIDA REGISTRADA ***',cols));return lines.join('\n')
}
function ticket58PreviewHtml(salida){
  const cfg=printerConfigRead(),cols=Math.max(24,Math.min(42,Number(cfg.charsPerLine)||32));
  const line=t=>`<div>${esc(asciiTicket(t||''))||'&nbsp;'}</div>`;
  const wrap=t=>ticketLines(t,cols).map(line).join('');
  const rule=()=>line('-'.repeat(cols));
  let h=`<div style="display:flex;justify-content:center;padding:8px 0 16px;background:#ececec;border-radius:14px;overflow:auto"><div style="width:384px;max-width:100%;box-sizing:border-box;background:#fff;padding:16px 18px 24px;color:#111;font:700 13px/1.25 'Courier New',monospace;box-shadow:0 8px 24px #0002"><div style="text-align:center">${cfg.printLogo!==false?'<img src="assets/logo.jfif" alt="PROVEEDORA" style="display:block;width:300px;max-width:92%;height:auto;margin:0 auto 8px;filter:grayscale(1) contrast(1.25)">':''}<div style="font-weight:900">SALIDA ABARROTES PDD</div><div>PROVSOFT</div></div>${rule()}`;
  h+=wrap(`FOLIO: ${salida.folio||'SE ASIGNA AL GUARDAR'}`);
  h+=line(`FECHA: ${salida.fechaCapturaTxt||salida.fechaCaptura||''}`)+line(`HORA: ${salida.horaLocal||''}`);
  h+=wrap(`DESTINO: ${salida.destino||''}`)+wrap(`ENTREGA: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`)+wrap(`RECIBE: ${salida.recibe||''}`)+rule();
  h+=`<div style="text-align:center;font-weight:900">PARTIDAS</div>${rule()}`;
  for(const x of salida.partidas||[]){h+=`<div style="font-weight:900">${wrap(`${x.renglon}. ${x.descripcion}`)}</div>`;h+=wrap(`COD: ${x.codigo}`);const cajas=x.cajasSalieron!=null&&x.cantidadPorCaja!=null?`${x.cajasSalieron} cj x ${x.cantidadPorCaja} = `:'';h+=`<div style="font-weight:900">${wrap(`${cajas}${x.cantidad} PZAS`)}</div>${line('')}`}
  h+=rule()+`<div style="font-weight:900">${line(`PARTIDAS: ${salida.totalPartidas}`)}${line(`UNIDADES: ${salida.totalUnidades}`)}</div>`+rule();
  h+=wrap(`ENTREGA: ${salida.entrega?.nombre||salida.entrega?.usuario||''}`)+line('')+wrap(`RECIBE: ${salida.recibe||''}`)+line('');
  if(cfg.printSignature!==false&&salida.firmaRecibe){h+=`<div style="text-align:center;font-weight:900;margin-top:6px">FIRMA DE RECIBIDO</div><div style="text-align:center"><img src="${esc(salida.firmaRecibe)}" alt="Firma" style="display:block;max-width:320px;width:84%;max-height:110px;object-fit:contain;margin:5px auto 7px;filter:grayscale(1) contrast(1.45)"></div>`}
  h+=`<div style="text-align:center;font-weight:900;margin-top:7px">*** SALIDA REGISTRADA ***</div></div></div>`;
  return h
}
function openTicket58Preview(salida,{title='Vista previa ticket 58 mm',onClose=null}={}){
  open(`<h2>${esc(title)}</h2><p class="review-note">Representación del ticket térmico de 58 mm. Usa el mismo contenido, ancho y saltos de línea del motor ESC/POS.</p>${ticket58PreviewHtml(salida)}<button id="ticketPreviewClose" class="secondary sticky-modal-btn">CERRAR VISTA PREVIA</button>`);
  $('ticketPreviewClose').onclick=()=>{close();if(typeof onClose==='function')onClose()}
}
function salidaPreviewFromState(firmaData=''){
  const now=new Date(),fechaCapturaTxt=(()=>{const [y,m,d]=String(S.fechaCaptura||'').split('-');return y&&m&&d?`${d}/${m}/${y}`:S.fechaCaptura||''})();
  return{folio:'SE ASIGNA AL GUARDAR',fechaCaptura:S.fechaCaptura,fechaCapturaTxt,horaLocal:now.toLocaleTimeString('es-MX'),destino:S.destino,entrega:{nombre:S.user?.nombre||S.user?.usuario||''},recibe:S.recibe,partidas:S.cart.map((x,i)=>({renglon:i+1,codigo:x.codigo,descripcion:x.descripcion,cantidad:x.cantidad,cantidadPorCaja:x.cantidadPorCaja??null,cajasSalieron:x.cajasSalieron??(Number(x.cantidadPorCaja)>0?Math.round((Number(x.cantidad)/Number(x.cantidadPorCaja)+Number.EPSILON)*1000)/1000:null)})),totalPartidas:S.cart.length,totalUnidades:S.cart.reduce((a,x)=>a+Number(x.cantidad||0),0),firmaRecibe:firmaData}
}
...
function printerDeviceLabel(device){if(!device)return'No configurada';return device.name||device.label||device.address||device.id||'Impresora seleccionada'}
async function selectNativePrinter(){
  const bridge=getNativePrinterBridge();
  if(!bridge)return{ok:false,native:false,reason:'NO_NATIVE_BRIDGE'};
  try{
    let result;
    if(bridge.type==='javascript-interface'){
      if(typeof bridge.api.selectPrinter!=='function')return{ok:false,native:true,reason:'SELECT_NOT_SUPPORTED'};
      result=bridge.api.selectPrinter();if(result&&typeof result.then==='function')result=await result;
      if(typeof result==='string'){try{result=JSON.parse(result)}catch{result={name:result}}}
    }else{
      if(typeof bridge.api.selectPrinter!=='function')return{ok:false,native:true,reason:'SELECT_NOT_SUPPORTED'};
      result=await bridge.api.selectPrinter();
    }
    const device=result?.device||result?.printer||result;
    if(!device||result?.ok===false)return{ok:false,native:true,reason:result?.reason||'NO_DEVICE'};
    const cfg=printerConfigWrite({device:{id:device.id||device.address||device.mac||'',address:device.address||device.mac||'',name:device.name||device.label||'Impresora Bluetooth'}});
    return{ok:true,native:true,device:cfg.device,result};
  }catch(error){return{ok:false,native:true,reason:'SELECT_ERROR',error:String(error?.message||error)}}
}
async function nativeBluetoothDiagnostic(){
  const bridge=getNativePrinterBridge();
  if(!bridge)return{ok:false,native:false,reason:'NO_NATIVE_BRIDGE'};
  try{
    let result;
    if(bridge.type==='javascript-interface'){
      if(typeof bridge.api.bluetoothDiagnostic!=='function')return{ok:false,native:true,reason:'BT_DIAG_NOT_SUPPORTED'};
      result=bridge.api.bluetoothDiagnostic();if(result&&typeof result.then==='function')result=await result;
      if(typeof result==='string'){try{result=JSON.parse(result)}catch{result={ok:false,reason:'BAD_DIAG_RESPONSE',message:result}}}
    }else{
      if(typeof bridge.api.bluetoothDiagnostic!=='function')return{ok:false,native:true,reason:'BT_DIAG_NOT_SUPPORTED'};
      result=await bridge.api.bluetoothDiagnostic();
    }
    return{...result,native:true,bridge:bridge.type};
  }catch(error){return{ok:false,native:true,reason:'BT_DIAGNOSTIC_ERROR',message:String(error?.message||error)}}
}
async function nativeSppConnectionDiagnostic(){
  const bridge=getNativePrinterBridge();
  if(!bridge)return{ok:false,native:false,code:'NO_ANDROID_BRIDGE',message:'Puente Android no disponible.'};
  try{
    if(bridge.type==='javascript-interface'){
      if(typeof bridge.api.startSppConnectionDiagnostic!=='function' || typeof bridge.api.getSppConnectionDiagnosticStatus!=='function'){
        return{ok:false,native:true,code:'SPP_ASYNC_NOT_SUPPORTED',message:'Esta versión Android no incluye diagnóstico SPP asíncrono.'};
      }
      let started=bridge.api.startSppConnectionDiagnostic();
      if(typeof started==='string'){try{started=JSON.parse(started)}catch{started={ok:false,message:started}}}
      if(started?.ok===false)return started;

      const deadline=Date.now()+18000;
      while(Date.now()<deadline){
        await new Promise(r=>setTimeout(r,500));
        let st=bridge.api.getSppConnectionDiagnosticStatus();
        if(typeof st==='string'){try{st=JSON.parse(st)}catch{st={ok:false,running:false,message:st}}}
        if(st?.ok===false)return st;
        if(st && st.running===false){
          if(st.result)return st.result;
          return{ok:false,native:true,code:'SPP_EMPTY_RESULT',message:'Android terminó la prueba pero no devolvió resultado.'};
        }
      }
      return{ok:false,native:true,code:'SPP_JS_TIMEOUT',message:'La prueba no respondió dentro de 18 segundos. La pantalla recuperó el control.'};
    }

    if(typeof bridge.api.startSppConnectionDiagnostic==='function' && typeof bridge.api.getSppConnectionDiagnosticStatus==='function'){
      await bridge.api.startSppConnectionDiagnostic();
      const deadline=Date.now()+18000;
      while(Date.now()<deadline){
        await new Promise(r=>setTimeout(r,500));
        const st=await bridge.api.getSppConnectionDiagnosticStatus();
        if(st?.running===false)return st?.result||{ok:false,code:'SPP_EMPTY_RESULT',message:'Sin resultado SPP.'};
      }
      return{ok:false,code:'SPP_JS_TIMEOUT',message:'La prueba no respondió dentro de 18 segundos.'};
    }
    return{ok:false,native:true,code:'SPP_ASYNC_NOT_SUPPORTED',message:'Puente Android sin API SPP asíncrona.'};
  }catch(e){return{ok:false,native:true,code:'SPP_DIAG_EXCEPTION',message:e?.message||String(e)}}
}
function sppDiagnosticHtml(d){
  const esc=s=>String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
  const device=d?.device||{};
  const stages=Array.isArray(d?.stages)?d.stages:[];
  const rows=stages.map(x=>`<div style="padding:8px 0;border-bottom:1px solid #eee"><b>${x.ok?'✅':'❌'} ${esc(x.name)}</b><div style="font-size:13px;color:#555">${esc(x.detail)} · ${esc(x.ms)} ms</div></div>`).join('');
  return `<div class="review-card"><p><b>Impresora:</b> ${esc(device.name||'—')}</p><p><b>MAC:</b> ${esc(device.address||device.id||'—')}</p><p><b>Resultado:</b> ${d?.ok?'✅ SPP CONECTADO':'❌ SPP NO CONECTÓ'}</p><p><b>Método:</b> ${esc(d?.connectMethod||'ninguno')}</p><p><b>Tiempo:</b> ${esc(d?.elapsedMs??'—')} ms</p>${d?.message?`<p>${esc(d.message)}</p>`:''}${rows?`<h3>Etapas</h3>${rows}`:''}${Array.isArray(d?.errors)&&d.errors.length?`<h3>Errores</h3><div style="font-size:13px;word-break:break-word">${d.errors.map(esc).join('<br>')}</div>`:''}</div>`;
}
function bluetoothDiagnosticHtml(d){
  if(!d?.ok)return `<div class="review-note"><b>Diagnóstico Bluetooth:</b> ERROR<br>${esc(d?.message||d?.reason||'Error desconocido')}</div>`;
  const dev=d.device||{},uuids=Array.isArray(d.uuids)?d.uuids:[],cached=Array.isArray(d.cachedUuids)?d.cachedUuids:[];
  const list=uuids.length?uuids.map(u=>`<div style="font-family:monospace;font-size:12px;padding:5px 0;border-bottom:1px solid #eee">${esc(u)}${String(u).toLowerCase()===String(d.expectedSppUuid||'').toLowerCase()?' <b style="color:#18864b">← SPP</b>':''}</div>`).join(''):'<div style="padding:8px 0"><b>No se anunciaron UUIDs.</b></div>';
  return `<div class="review-note"><b>Impresora:</b> ${esc(dev.name||'')}<br><b>MAC:</b> ${esc(dev.address||'')}<br><b>Emparejado:</b> ${esc(d.bondState||'')}<br><b>Tipo:</b> ${esc(d.deviceType||'')}<br><b>SPP anunciado:</b> <b>${d.sppAdvertised?'SÍ':'NO'}</b><br><b>SDP iniciado:</b> ${d.sdpStarted?'SÍ':'NO'}<br><b>Respuesta SDP:</b> ${d.sdpBroadcastReceived?'SÍ':'NO'}${d.fetchError?`<br><b>SDP:</b> ${esc(d.fetchError)}`:''}<br><b>Último método conectado:</b> ${esc(d.lastConnectMethod||'ninguno')}</div><h3 style="margin-top:14px">Servicios / UUID detectados</h3>${list}<p><small>UUID SPP esperado: ${esc(d.expectedSppUuid||'')}</small></p><p><small>UUIDs en caché antes de SDP: ${cached.length}</small></p>`;
}

function parseGsV0Raster(bytes){
  const b=bytes instanceof Uint8Array?bytes:new Uint8Array(bytes||[]);
  if(b.length<8||b[0]!==29||b[1]!==118||b[2]!==48)return null;
  const widthBytes=b[4]|(b[5]<<8),height=b[6]|(b[7]<<8),width=widthBytes*8;
  const need=8+widthBytes*height;if(widthBytes<=0||height<=0||b.length<need)return null;
  const payload=b.subarray(8,need);
  const isBlack=(x,y)=>{if(x<0||x>=width||y<0||y>=height)return false;const v=payload[y*widthBytes+(x>>3)];return !!(v&(0x80>>(x&7)))};
  return{widthBytes,width,height,isBlack};
}
function rasterToEscStar24(gsBytes){
  const r=parseGsV0Raster(gsBytes);if(!r)return new Uint8Array();
  const out=[],push=(...a)=>out.push(...a);const w=r.width;
  for(let y0=0;y0<r.height;y0+=24){
    push(27,42,33,w&255,(w>>8)&255);
    for(let x=0;x<w;x++)for(let slice=0;slice<3;slice++){
      let v=0;for(let bit=0;bit<8;bit++){const y=y0+slice*8+bit;if(r.isBlack(x,y))v|=(0x80>>bit)}push(v)
    }
    push(10)
  }
  return new Uint8Array(out)
}
function rasterToEscStar8(gsBytes){
  const r=parseGsV0Raster(gsBytes);if(!r)return new Uint8Array();
  const out=[],push=(...a)=>out.push(...a);const w=r.width;
  for(let y0=0;y0<r.height;y0+=8){
    push(27,42,1,w&255,(w>>8)&255);
    for(let x=0;x<w;x++){
      let v=0;for(let bit=0;bit<8;bit++){const y=y0+bit;if(r.isBlack(x,y))v|=(0x80>>bit)}push(v)
    }
    push(10)
  }
  return new Uint8Array(out)
}
function buildGraphicsCompatibilityBytes(){
  const bytes=[],push=(...a)=>bytes.push(...a),text=t=>{for(const ch of asciiTicket(t))push(ch.charCodeAt(0)&255)},lf=(t='')=>{text(t);push(10)};
  push(27,64);push(27,97,1);push(27,69,1);lf('PRUEBA GRAFICA V55');push(27,69,0);lf('Printer001 / 58 mm');lf('');
  lf('METODO A: GS v 0');push(...PROVEEDORA_LOGO_ESC_POS_BYTES);lf('FIN A');lf('');lf('-------------------------------');lf('');
  lf('METODO B: ESC * 24 DOT');push(...rasterToEscStar24(PROVEEDORA_LOGO_ESC_POS_BYTES));lf('FIN B');lf('');lf('-------------------------------');lf('');
  lf('METODO C: ESC * 8 DOT');push(...rasterToEscStar8(PROVEEDORA_LOGO_ESC_POS_BYTES));lf('FIN C');lf('');
  push(27,97,0);lf('Si uno imprime el logo bien,');lf('ese sera el modo definitivo.');for(let i=0;i<5;i++)lf('');
  return new Uint8Array(bytes)
}
async function printGraphicsCompatibilityTest(){
  const cfg=printerConfigRead(),bytes=buildGraphicsCompatibilityBytes(),bridge=getNativePrinterBridge(),dataBase64=uint8ToBase64(bytes);
  if(!bridge)return{ok:false,native:false,reason:'NO_NATIVE_BRIDGE'};
  const job={paperMm:58,protocol:'ESC/POS',test:true,graphicsCompat:true,folio:'GRAFICA-V55',device:cfg.device||null};
  try{
    let result;
    if(bridge.type==='javascript-interface'){result=bridge.api.printBase64(dataBase64,JSON.stringify(job));if(result&&typeof result.then==='function')result=await result;if(typeof result==='string'){try{result=JSON.parse(result)}catch{result={ok:true,message:result}}}}
    else result=await bridge.api.print({dataBase64,...job});
    const ok=result?.ok!==false;return{ok,native:true,result,reason:ok?null:(result?.code||result?.reason||'PRINT_ERROR'),error:ok?null:(result?.message||result?.error||'')};
  }catch(error){return{ok:false,native:true,reason:'PRINT_ERROR',error:String(error?.message||error)}}
}

async function printSalida58(salida,{test=false}={}){
  const baseCfg=printerConfigRead(),cfg=test?{...baseCfg,partialCut:false}:baseCfg,bytes=await buildEscPos58Bytes(salida,cfg),dataBase64=uint8ToBase64(bytes),bridge=getNativePrinterBridge();
  if(!bridge)return{ok:false,native:false,reason:'NO_NATIVE_BRIDGE',bytes:dataBase64,preview:buildEscPos58TextPreview(salida)};
  const job={paperMm:58,protocol:'ESC/POS',test,folio:salida.folio||'PRUEBA',device:cfg.device||null};
  try{
    let result;
    if(bridge.type==='javascript-interface'){result=bridge.api.printBase64(dataBase64,JSON.stringify(job));if(result&&typeof result.then==='function')result=await result; if(typeof result==='string'){try{result=JSON.parse(result)}catch{result={ok:true,message:result}}}}
    else result=await bridge.api.print({dataBase64,...job});
    const ok=result?.ok!==false;
    return{ok,native:true,bridge:bridge.type,result,reason:ok?null:(result?.reason||result?.code||'PRINT_ERROR'),error:ok?null:(result?.error||result?.message||'')};
  }catch(error){console.error('[IMPRESORA 58MM]',error);return{ok:false,native:true,bridge:bridge.type,reason:'PRINT_ERROR',error:String(error?.message||error)}}
}
...
async function printerEscPosDiagnostic(){
  const cfg=printerConfigRead(),salida=printerTestSalida(),bytes=await buildEscPos58Bytes(salida,cfg);
  const hasSeq=seq=>{outer:for(let i=0;i<=bytes.length-seq.length;i++){for(let j=0;j<seq.length;j++)if(bytes[i+j]!==seq[j])continue outer;return true}return false};
  const checks=[
    ['Inicialización ESC/POS',hasSeq([27,64])],
    ['Perfil de papel 58 mm',Number(cfg.paperMm)===58],
    ['Columnas válidas', [30,32,42].includes(Number(cfg.charsPerLine))],
    ['Logo raster GS v 0',cfg.printLogo===false||hasSeq([29,118,48,0])],
    ['Texto de prueba',asciiTicket(buildEscPos58TextPreview(salida)).includes('PRUEBA-58MM')],
    ['Avance de papel',Number(cfg.feedLines)>=2],
    ['Corte parcial GS V',cfg.partialCut===false||hasSeq([29,86,66,0])],
    ['Paquete no vacío',bytes.length>100]
  ];
  const ok=checks.every(x=>x[1]),bridge=getNativePrinterBridge();
  return {ok,bytes:bytes.length,base64Chars:uint8ToBase64(bytes).length,checks,bridge:bridge?.type||null,device:printerDeviceLabel(cfg.device),config:cfg};
}
function printerDiagnosticHtml(d){
  const rows=d.checks.map(([name,ok])=>`<div style="display:flex;justify-content:space-between;gap:12px;padding:9px 0;border-bottom:1px solid #e5e7eb"><span>${esc(name)}</span><b>${ok?'OK':'REVISAR'}</b></div>`).join('');
  return `<div class="review-note"><b>Resultado:</b> ${d.ok?'PAQUETE ESC/POS CORRECTO':'HAY PUNTOS POR REVISAR'}<br><b>Tamaño:</b> ${d.bytes.toLocaleString('es-MX')} bytes · Base64 ${d.base64Chars.toLocaleString('es-MX')} caracteres<br><b>Impresora:</b> ${esc(d.device)}<br><b>Puente Android:</b> ${d.bridge?esc(d.bridge):'NO DISPONIBLE (normal en PWA)'}</div><div style="margin-top:10px">${rows}</div><p><small>Este diagnóstico valida la construcción del ticket. La conexión Bluetooth y la respuesta física de la impresora se validarán en Android.</small></p>`
}
async function printerConfigModal(){
  const cfg=printerConfigRead(),bridge=getNativePrinterBridge(),deviceName=printerDeviceLabel(cfg.device);
  open(`<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px"><h2 style="margin:0">Impresora térmica 58 mm</h2><button id="printerClose" type="button" aria-label="Cerrar configuración de impresora" style="border:0;background:transparent;font-size:32px;line-height:1;padding:2px 8px;cursor:pointer;color:#444">&times;</button></div>
    <div class="review-note"><b>Perfil:</b> ESC/POS · 58 mm. Esta configuración queda guardada en este dispositivo. Android aportará la conexión Bluetooth cuando instalemos la APK.</div>
    <label>Impresora predeterminada</label>
    <div id="printerDeviceBox" style="padding:12px;border:2px solid ${cfg.device?'#18864b':'#d7dce2'};border-radius:12px;margin:6px 0 10px;background:${cfg.device?'#eefaf3':'#fff'}">
      <div style="font-size:12px;font-weight:900;color:${cfg.device?'#18864b':'#667085'}">${cfg.device?'✓ IMPRESORA SELECCIONADA':'SIN IMPRESORA SELECCIONADA'}</div>
      <b id="printerDeviceName" style="display:block;font-size:18px;margin-top:3px">${esc(deviceName)}</b>
      <small id="printerDeviceAddress">${cfg.device?.address?esc(cfg.device.address):bridge?'Puente Android detectado · toca Seleccionar impresora':'PWA · selección Bluetooth disponible al instalar Android'}</small>
    </div>
    <button id="printerSelect" class="secondary" style="width:100%;margin-bottom:14px">${cfg.device?'CAMBIAR IMPRESORA':'SELECCIONAR IMPRESORA'}</button>
    <label>Ancho de papel</label><input class="field" value="58 mm" disabled>
    <label>Protocolo</label><input class="field" value="ESC/POS" disabled>
    <label>Caracteres por línea</label><select id="printerCols" class="field"><option value="32" ${Number(cfg.charsPerLine)===32?'selected':''}>32 (recomendado)</option><option value="30" ${Number(cfg.charsPerLine)===30?'selected':''}>30</option><option value="42" ${Number(cfg.charsPerLine)===42?'selected':''}>42 (fuente compacta)</option></select>
    <label><input id="printerAuto" type="checkbox" ${cfg.autoPrint!==false?'checked':''}> Imprimir automáticamente al guardar salida</label>
    <label><input id="printerLogo" type="checkbox" ${cfg.printLogo!==false?'checked':''}> Imprimir logo de PROVEEDORA</label>
    <label><input id="printerSignature" type="checkbox" ${cfg.printSignature!==false?'checked':''}> Imprimir firma de recibido</label>
    <div class="review-note" style="margin-top:12px"><b>Perfil confirmado:</b> Bluetooth SPP · ESC/POS 58 mm · imágenes GS v 0 · corte manual.</div>
    <p><small>Puente Android: <b>${bridge?'DETECTADO':'NO DISPONIBLE EN PWA'}</b></small></p>
    <button id="printerSave" class="primary sticky-modal-btn">GUARDAR CONFIGURACIÓN</button>`);
  const saveCfg=()=>printerConfigWrite({charsPerLine:Number($('printerCols').value)||32,autoPrint:$('printerAuto').checked,printLogo:$('printerLogo').checked,printSignature:$('printerSignature').checked,partialCut:false,device:printerConfigRead().device||null});
  $('printerClose').onclick=()=>{saveCfg();close()};
  $('printerSelect').onclick=async()=>{saveCfg();const r=await selectNativePrinter();if(r.ok){const box=$('printerDeviceBox');$('printerDeviceName').textContent=printerDeviceLabel(r.device);$('printerDeviceAddress').textContent=r.device?.address||r.device?.id||'';box.style.borderColor='#18864b';box.style.background='#eefaf3';box.firstElementChild.textContent='✓ IMPRESORA SELECCIONADA';box.firstElementChild.style.color='#18864b';$('printerSelect').textContent='CAMBIAR IMPRESORA';alert('Impresora seleccionada:\n\n'+printerDeviceLabel(r.device)+(r.device?.address?'\n'+r.device.address:''));return}if(!r.native)return alert('La selección de impresora Bluetooth se habilitará en la APK Android.\n\nLa PWA conserva toda la configuración y el motor ESC/POS.');if(r.reason==='SELECT_NOT_SUPPORTED')return alert('El puente Android fue detectado, pero todavía no incluye el selector de impresora.');alert('No se pudo seleccionar la impresora: '+(r.error||r.reason||'error desconocido'))};
  $('printerSave').onclick=()=>{saveCfg();close();toast('Configuración 58 mm guardada')};
}
window.PROVSOFT_ESC_POS_58={buildBytes:buildEscPos58Bytes,buildPreview:buildEscPos58TextPreview,previewHtml:ticket58PreviewHtml,openPreview:openTicket58Preview,print:printSalida58,selectPrinter:selectNativePrinter,getConfig:printerConfigRead,setConfig:printerConfigWrite,firmaRaster:firmaEscPosRasterBytes,printStatus:printStatusBase,reasonText:printerReasonText};
...
async function loadRecentSalidasForReprint(){
  const byFolio=new Map();
  for(const x of [...pendingSalidasRead(),...salidasHistoryRead()])if(x?.folio)byFolio.set(x.folio,x);
  try{
    await ensureOnlineStack(12000);
    const q=query(collection(db,...R.salidas),orderBy('creadoEnLocal','desc'),limit(30));
    const snap=await getDocs(q);snap.forEach(d=>{const x=d.data()||{};if(x.folio||d.id)byFolio.set(x.folio||d.id,{folio:d.id,...x})});
  }catch(e){console.warn('[REIMPRESION] usando historial local:',e)}
  return [...byFolio.values()].sort((a,b)=>String(b.creadoEnLocal||b.fechaCaptura||'').localeCompare(String(a.creadoEnLocal||a.fechaCaptura||''))).slice(0,30)
}
async function reprintSalida58(salida){
  const prev=printStatusBase(salida),ahora=new Date().toISOString();
  const r=await printSalida58(salida);
  if(r?.ok){
    await persistPrintStatus(salida,{estado:'IMPRESO',intentos:prev.intentos+1,ultimaIntentoLocal:ahora,ultimaImpresionLocal:ahora,errorCodigo:null,errorMensaje:null,bridge:r.bridge||null,dispositivo:printerConfigRead().device||null});
    alert('Ticket enviado a la impresora 58 mm.');return true;
  }
  if(!r?.native){
    await persistPrintStatus(salida,{estado:'PENDIENTE',intentos:prev.intentos,ultimaIntentoLocal:prev.ultimaIntentoLocal||null,errorCodigo:'NO_NATIVE_BRIDGE',errorMensaje:'Pendiente de imprimir desde Android',bridge:null,dispositivo:printerConfigRead().device||null});
    openTicket58Preview(salida,{title:'Reimpresión · '+salida.folio,onClose:openReprintSalidas});return false;
  }
  const reason=r.reason||'PRINT_ERROR',msg=printerReasonText(reason,r.error||'');
  await persistPrintStatus(salida,{estado:'ERROR',intentos:prev.intentos+1,ultimaIntentoLocal:ahora,errorCodigo:reason,errorMensaje:msg,bridge:r.bridge||null,dispositivo:printerConfigRead().device||null});
  alert(`No fue posible imprimir.\n\n${msg}\n\nLa salida sigue guardada. Puedes reintentar sin volver a generar la salida.`);return false;
}
async function openReprintSalidas(){
  open('<h2>🧾 Reimpresión de salidas</h2><p class="review-note">Cargando las últimas salidas guardadas…</p>');
  const rows=await loadRecentSalidasForReprint();
  if(!rows.length){card.innerHTML='<h2>🧾 Reimpresión de salidas</h2><div class="empty">No hay salidas disponibles en este equipo ni en Firebase.</div><button id="rpClose" class="secondary sticky-modal-btn">CERRAR</button>';$('rpClose').onclick=close;return}
  card.innerHTML=`<h2>🧾 Reimpresión de salidas</h2><p class="review-note">Selecciona una salida. Reimprimir no modifica inventario ni vuelve a guardar la salida.</p><div style="display:grid;gap:10px">${rows.map((x,i)=>{const ps=printStatusBase(x),err=ps.errorMensaje?`<br><small style="font-weight:800">${esc(ps.errorMensaje)}</small>`:'';return `<div style="border:1px solid #ddd;border-radius:12px;padding:12px"><b>${esc(x.folio)}</b><br><small>${esc(x.fechaCapturaTxt||x.fechaCaptura||'')} · ${esc(x.horaLocal||'')} · ${esc(x.destino||'')}</small><br><small>${esc(x.recibe||'')} · ${Number(x.totalPartidas||0)} partidas · ${Number(x.totalUnidades||0)} unidades</small><br><small><b>${printerStatusLabel(x)}</b> · intentos ${ps.intentos||0}</small>${err}<div style="display:flex;gap:8px;margin-top:9px"><button class="secondary rpPrev" data-i="${i}" style="flex:1">VISTA PREVIA</button><button class="primary rpPrint" data-i="${i}" style="flex:1">${ps.estado==='IMPRESO'?'REIMPRIMIR':'REINTENTAR'}</button></div></div>`}).join('')}</div><button id="rpClose" class="secondary sticky-modal-btn">CERRAR</button>`;
  card.querySelectorAll('.rpPrev').forEach(b=>b.onclick=()=>{const x=rows[Number(b.dataset.i)];openTicket58Preview(x,{title:'Salida '+x.folio,onClose:openReprintSalidas})});
  card.querySelectorAll('.rpPrint').forEach(b=>b.onclick=async()=>{const x=rows[Number(b.dataset.i)];b.disabled=true;try{await reprintSalida58(x)}finally{b.disabled=false}});
...
async function selectProduct(p){
  ...
  const abrirModalCajasSalida=(boxQty,boxesValue='')=>{
    ...
    $('add').onclick=()=>{
      ...
      // V61: sin aviso cuando la salida supera el inventario inicial.
      if(ex){
...
function signAndSave(){
  ...
  open(`<div class="signature-screen">
    ...
    <div class="signature-actions">
      ...
      <button id="ticketPreviewBeforeSave" class="secondary">👁 VISTA PREVIA 58 MM</button>
  ...
  $('ticketPreviewBeforeSave').onclick=()=>{
    if(!has)return alert('Firma primero para ver el ticket completo.');
    const firmaData=c.toDataURL('image/png'),salidaPrev=salidaPreviewFromState(firmaData);
    const layer=document.createElement('div');layer.id='ticket58PreviewLayer';
    Object.assign(layer.style,{position:'fixed',inset:'0',zIndex:'2147483000',background:'rgba(0,0,0,.72)',overflow:'auto',padding:'18px',boxSizing:'border-box'});
    layer.innerHTML=`<div style="max-width:520px;margin:0 auto;background:#fff;border-radius:20px;padding:18px;box-sizing:border-box"><h2 style="margin-top:0">Vista previa ticket 58 mm</h2><p class="review-note">Incluye el logo y la firma que acabas de capturar. El folio definitivo se asigna al guardar.</p>${ticket58PreviewHtml(salidaPrev)}<button id="ticket58PreviewBack" class="primary" style="width:100%;margin-top:14px">REGRESAR A LA FIRMA</button></div>`;
    document.body.appendChild(layer);$('ticket58PreviewBack').onclick=()=>layer.remove();
  };
  $('save').onclick=async()=>{
    ...
    try{
      ...
      if(printerCfg.autoPrint!==false){
        ...
        const ps=printStatusBase(salida),ahoraIntento=new Date().toISOString();
        if(printResult?.ok){
          applyPrintStatusLocal(salida,{estado:'IMPRESO',intentos:ps.intentos+1,ultimaIntentoLocal:ahoraIntento,ultimaImpresionLocal:ahoraIntento,errorCodigo:null,errorMensaje:null,bridge:printResult.bridge||null,dispositivo:printerCfg.device||null});
        }else if(printResult?.native){
          const reason=printResult.reason||'PRINT_ERROR';
          applyPrintStatusLocal(salida,{estado:'ERROR',intentos:ps.intentos+1,ultimaIntentoLocal:ahoraIntento,errorCodigo:reason,errorMensaje:printerReasonText(reason,printResult.error||''),bridge:printResult.bridge||null,dispositivo:printerCfg.device||null});
        }else{
          applyPrintStatusLocal(salida,{estado:'PENDIENTE',intentos:ps.intentos,errorCodigo:'NO_NATIVE_BRIDGE',errorMensaje:'Pendiente de imprimir desde Android',bridge:null,dispositivo:printerCfg.device||null});
        }
        await pintarBloqueoSalida();
      }else{
        applyPrintStatusLocal(salida,{estado:'NO_SOLICITADA',intentos:0,errorCodigo:null,errorMensaje:null,dispositivo:printerCfg.device||null});
      }
      ...
      showModeGate();
      const psFinal=printStatusBase(salida);
      const estadoImpresion=psFinal.estado==='IMPRESO'?'\nTicket 58 mm IMPRESO':psFinal.estado==='ERROR'?`\nImpresión 58 mm FALLÓ: ${psFinal.errorMensaje||'revisar impresora'}`:psFinal.estado==='NO_SOLICITADA'?'\nAutoimpresión desactivada':'\nTicket 58 mm PENDIENTE de impresión';
...
$('reprintBtn').onclick=()=>{menu.classList.add('hidden');openReprintSalidas()};
