...
function pintarFacturas() {
  ...
  facturasPendientes.forEach((f, index) => {
    ...
    const articulosFactura = prepararArticulosEntrada(f, equivalencias);
    const totalConceptos = articulosFactura.length;
    const conceptosEnlazados = articulosFactura.filter(a => a.equivalencia_encontrada).length;
    const conceptosFaltantes = Math.max(0, totalConceptos - conceptosEnlazados);
    const porcentajeEnlace = totalConceptos > 0
      ? Math.round((conceptosEnlazados / totalConceptos) * 100)
      : 0;
    const listaParaProcesar = totalConceptos > 0 && conceptosFaltantes === 0;

    div.classList.toggle("card-lista-procesar", listaParaProcesar);

    div.innerHTML = `
      <h3>${escapeHtml(f.razon_social_emisor || "Proveedor sin nombre")}</h3>
      <p><span class="badge ${listaParaProcesar ? "badge-lista" : "badge-pendiente"}">${listaParaProcesar ? "LISTA PARA PROCESAR" : "PENDIENTE"}</span></p>
      ...
      <div class="progreso-enlaces ${listaParaProcesar ? "completo" : ""}">
        <div class="progreso-enlaces-cabecera">
          <strong>Enlazados: ${conceptosEnlazados} de ${totalConceptos}</strong>
          <span>${porcentajeEnlace}%</span>
        </div>
        <div class="progreso-enlaces-barra" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${porcentajeEnlace}">
          <span style="width:${porcentajeEnlace}%"></span>
        </div>
        <div class="progreso-enlaces-pie">
          ${listaParaProcesar
            ? "100% enlazada · lista para procesar"
            : `Faltan por enlazar: <b>${conceptosFaltantes}</b>`}
        </div>
      </div>

      <div class="acciones-card">
        <button data-index="${index}" class="btnAutorizar ${listaParaProcesar ? "btnProcesarLista" : ""}">
          ${listaParaProcesar ? "Procesar entrada" : "Revisar / autorizar"}
        </button>
      </div>
    `;
...
async function asegurarProductosActivos() {
  ...
  // El catálogo se prepara desde el arranque general de la app.
  // Aquí sólo se abre la copia local para NO volver a tocar Firebase al cambiar de función.
  productosActivos = await cargarProductosActivosLocal();
  productosActivosCargados = true;

  // Respaldo únicamente para un navegador nuevo o si IndexedDB fue borrado
  // después de que la app ya había iniciado.
...
function cerrarModal() {
  ...
  document.getElementById("panelEntradasZapata")?.classList.remove("revisando-factura");
}
...
function asegurarPantallaRevisionFactura() {
  ...
  pantalla.innerHTML = `
    ...
    <div class="revision-backbar">
      <button id="btnCerrarRevisionFactura" type="button">← Regresar a facturas</button>
    </div>
...
function mostrarPantallaRevisionFactura() {
  ...
  document.getElementById("panelEntradasZapata")?.classList.add("revisando-factura");
