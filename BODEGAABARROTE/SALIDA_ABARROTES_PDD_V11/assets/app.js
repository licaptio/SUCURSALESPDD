...
function askInstallBefore(action){
  diag('askInstallBefore','inicio');
  if(isStandalonePwa()){
    diag('PWA','standalone -> acción directa');
    action();
    return;
  }

  // El selector inicial usa z-index alto; el modal de instalación debe quedar encima.
  // La navegación nunca depende de que el prompt nativo de instalación resuelva.
  open(`<h2>Instalar Abarrotes PDD</h2><p>¿Deseas instalar la aplicación en este dispositivo antes de continuar?</p><div class="actions"><button id="installCancel" class="secondary">CANCELAR</button><button id="installAccept" class="primary">ACEPTAR</button></div>`);
  ...
  const continueOnce=()=>{
    ...
    continued=true;
    close();
    diag('askInstallBefore','continuar');
    action();
  };

  $('installCancel').onclick=()=>{
    diag('PWA','instalación cancelada');
    continueOnce();
  };

  $('installAccept').onclick=()=>{
    const p=deferredInstallPrompt;
    diag('PWA',p?'prompt disponible':'prompt no disponible');
    // Continuar de inmediato. Algunos Chrome/WebView de Android pueden dejar
    // userChoice pendiente y antes bloqueaban la Entrada/Salida.
    continueOnce();
    if(p){
      try{
        p.prompt();
        Promise.resolve(p.userChoice).catch(()=>{}).finally(()=>{deferredInstallPrompt=null});
      }catch(e){
        deferredInstallPrompt=null;
        console.warn('[PWA] No se pudo mostrar prompt de instalación:',e);
      }
    }else{
      setTimeout(()=>toast('La instalación no está disponible en este momento'),0);
    }
  };
}
...
async function loadFixedUser(){
  try{
    ...
    console.error('[USUARIO FIJO]',e);
    throw new Error('No fue posible cargar el usuario fijo JUAN / Abarrotes.');
  }
...
$('modeEntrada').onclick=()=>{diag('BOTÓN ENTRADA','onclick');askInstallBefore(enterEntrada)};
$('modeSalida').onclick=()=>{diag('BOTÓN SALIDA','onclick');askInstallBefore(enterSalida)};
...
(async()=>{
  ...
  try{
    ...
    setBootStatus('Aplicación lista.');await new Promise(r=>setTimeout(r,300));hideLoad();showModeGate();
  }catch(e){console.error(e);setBootStatus(e.message||'No fue posible iniciar la aplicación.');}
})();
