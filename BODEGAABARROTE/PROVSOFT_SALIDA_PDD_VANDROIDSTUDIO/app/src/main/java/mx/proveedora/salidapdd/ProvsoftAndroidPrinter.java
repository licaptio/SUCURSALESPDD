...
public class ProvsoftAndroidPrinter {
    ...
    public String printBase64(String dataBase64, String jobJson) {
        ...
        try {
            ...
            // V56: envío definitivo para Printer001. Texto pausado + imágenes GS v 0 atómicas.
    ...
    private BluetoothSocket connectSocket(BluetoothDevice device) throws Exception {
        // V49: conexión centrada en SPP real. La impresora ya confirmó por SDP
        // el UUID 00001101-0000-1000-8000-00805F9B34FB.
        List<String> errors = new ArrayList<>();
        ...
        // Para impresoras térmicas emparejadas suele funcionar mejor insecure primero.
        socket = tryConnect("SPP_INSECURE", () -> device.createInsecureRfcommSocketToServiceRecord(SPP_UUID), 7000, errors);
        if (socket != null) return socket;

        socket = tryConnect("SPP_SECURE", () -> device.createRfcommSocketToServiceRecord(SPP_UUID), 7000, errors);
        if (socket != null) return socket;

        // Fallback clave V49: RFCOMM INSEGURO directo. V48 solo probaba
        // createRfcommSocket(channel), que es la variante segura.
        for (int channel = 1; channel <= 3; channel++) {
            final int ch = channel;
            socket = tryConnect("RFCOMM_INSECURE_CH_" + ch, () -> {
                Method m = device.getClass().getMethod("createInsecureRfcommSocket", int.class);
                return (BluetoothSocket) m.invoke(device, ch);
            }, 5500, errors);
            if (socket != null) return socket;
        }
        ...
        for (int channel = 1; channel <= 3; channel++) {
            final int ch = channel;
            socket = tryConnect("RFCOMM_SECURE_CH_" + ch, () -> {
        ...
        throw new Exception("No fue posible abrir SPP. Intentos: " + String.join(" | ", errors));
    }
