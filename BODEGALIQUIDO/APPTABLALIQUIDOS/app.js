...
function imprimirListaConteo(listado) {
  ...
  const ventana = window.open("", "_blank", "width=480,height=800");
  if (!ventana) {
...
<html lang="es"><head><meta charset="utf-8"><title>Lista de conteo - ${escapeHtml(listado.nombre)}</title>
<style>
  @page { size: 88mm auto; margin: 2mm; }
  * { box-sizing: border-box; }
  html, body { width: 84mm; margin: 0; padding: 0; }
  body { font-family: Arial, Helvetica, sans-serif; color:#000; font-size:10px; }
  h1 { font-size:14px; line-height:1.15; text-align:center; margin:0 0 3mm; font-weight:800; }
  .meta { border-top:1px solid #000; border-bottom:1px solid #000; padding:1.5mm 0; margin-bottom:2mm; font-size:9px; line-height:1.45; }
  .meta b { font-weight:800; }
  table { width:100%; border-collapse:collapse; table-layout:fixed; }
  th, td { border-bottom:1px solid #000; padding:1.4mm .8mm; vertical-align:middle; }
  th { font-size:8px; text-align:left; border-top:1px solid #000; }
  th.codigo, td.codigo { width:22mm; font-weight:700; word-break:break-word; }
  th.concepto, td.concepto { width:46mm; word-break:break-word; }
  th.cantidad, td.cantidad { width:16mm; text-align:center; }
  td.cantidad { height:9mm; border-left:1px solid #000; }
  .pie { text-align:center; font-size:8px; margin-top:2.5mm; padding-top:1.5mm; border-top:1px dashed #000; }
  @media print {
    html, body { width:84mm; }
    body { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  }
</style></head><body>
<h1>${escapeHtml(listado.nombre)}</h1>
<div class="meta">
  <div><b>FECHA INVENTARIO:</b> ${escapeHtml(fechaInventario)}</div>
  <div><b>HORA:</b> ${escapeHtml(horaInventario)}</div>
</div>
<table>
  <thead><tr><th class="codigo">CÓDIGO</th><th class="concepto">CONCEPTO</th><th class="cantidad">CANT.</th></tr></thead>
  <tbody>${rows.map((r) => `<tr><td class="codigo">${escapeHtml(r.codigo)}</td><td class="concepto">${escapeHtml(r.nombre)}</td><td class="cantidad"></td></tr>`).join("")}</tbody>
</table>
<div class="pie">${rows.length} ARTÍCULOS</div>
...
${
  ...
  tfoot.innerHTML = `
    <tr>
      ...
      <td class="${Number(totalInviniSemana || 0) < 0 ? "inventario-negativo" : ""}">${fmtNum(totalInviniSemana)}</td>
      ...
      <td class="${Number(totalExistenciaFinal || 0) < 0 ? "inventario-negativo" : ""}">${fmtNum(totalExistenciaFinal)}</td>
    </tr>
  `;
}
