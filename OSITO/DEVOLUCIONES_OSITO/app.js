...
const LS_BORRADOR = "provsoft_devolucion_osito_borrador";
const DB_NAME = "provsoft_catalogo_devoluciones_osito";
const STORE_PRODUCTOS = "productos";
const LS_CATALOGO_FECHA = "provsoft_devolucion_osito_catalogo_fecha";
const LS_PENDIENTES = "provsoft_devolucion_osito_pendientes_firebase";
...
async function iniciarApp(){
  ...
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_OSITO;
...
async function subirDocumentoFirebase(documento){
  ...
  await setDoc(
    docRef(db, APP_CONFIG.COLECCION_TIENDAS, APP_CONFIG.TIENDA_OSITO, APP_CONFIG.SUBCOLECCION_DEVOLUCIONES, documento.folio),
    docOnline,
    { merge:true }
  );
}
      ...
      try{
        ...
        try{
          ...
          await enviarPDFTelegram(pdfBlob,{
            ...
            tienda: registro.documento.tiendaOrigen || APP_CONFIG.TIENDA_OSITO,
...
function generarFolio(){
  const limpio = APP_CONFIG.TIENDA_OSITO;
  const f = new Date();
...
function recuperarBorrador(){
  ...
  // OSITO es fija: nunca recuperar una tienda heredada de otra app.
  document.getElementById("tienda").value = APP_CONFIG.TIENDA_OSITO;
  document.getElementById("encargado").value = b.encargado || "";

  // Solo conservar folios válidos de DEVOLUCIONES OSITO.
  const folioRecuperado = String(b.folio || "").trim();
  document.getElementById("folio").value = folioRecuperado.startsWith("DEV-OSITO-")
    ? folioRecuperado
    : generarFolio();
...
function construirDocumentoDevolucion(){
  const t = calcularTotales();
  const tienda = APP_CONFIG.TIENDA_OSITO;
  const encargado = document.getElementById("encargado").value.trim();
...
async function generarSolicitud(){
  ...
  try{
    ...
    if(telegramEnviado){
      alert("Solicitud de devolución OSITO generada y PDF enviado a Telegram.");
    }else{
...
function generarPDF(documentoDevolucion){
  ...
  pdf.text("Documento de alta de devolución desde OSITO a MATRIZ",105,y,{align:"center"});
