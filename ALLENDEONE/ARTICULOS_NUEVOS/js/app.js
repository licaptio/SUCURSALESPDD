...
import { crearIndiceFotos, buscarMetaFoto, extraerTodasLasUrls } from "./relaciones.js";
...
async function cargarAplicacion(forzarActualizacion = false) {
  mostrarPantallaCarga("Cargando catálogo de productos…", "Consultando los 30 artículos activos más recientes", 8);
  ...
  try {
    ...
    actualizarCarga("Relacionando fotografías…", "Buscando todas las fotos de estos 30 artículos", 38);
...
function prepararRelaciones(productos, catalogo) {
  ...
  for (const producto of productos) {
    ...
    const urls = resultado ? extraerTodasLasUrls(resultado.meta) : [];
...
async function cargarImagenesNecesarias(relaciones) {
  const urlsUnicas = [...new Set(relaciones.flatMap(item => item.urls || []).filter(Boolean))];
  ...
  if (!total) {
    for (const relacion of relaciones) estado.fotosPorCodigo.set(relacion.codigo, []);
  ...
  for (const relacion of relaciones) {
    const fotos = [];
    for (const url of relacion.urls || []) {
      const blob = blobsPorUrl.get(url);
      fotos.push(blob ? crearUrlObjeto(blob) : url);
    }
    estado.fotosPorCodigo.set(relacion.codigo, fotos);
    if (fotos.length) relacionadas += 1;
  }
