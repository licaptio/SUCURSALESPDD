...
async function cargarMovimientosHasta(fin, forzar = false) {
  ...
  setStatus(`Descargando salidas Abarrotes ${fechaCorta(inicioConsulta)} a ${fechaCorta(fin)}...`);

  // Por ahora el pivot principal sólo conecta SALIDAS.
  // Entradas y ajustes se conectarán cuando existan sus colecciones definitivas.
  const salidas = await consultarSalidas(inicioConsulta, fin);
...
async function cargarSalidasZapata(forzarRecarga = false) {
  try {
    ...
    setStatus(
      ...
      `Entradas: pendientes de conectar. ` +
      `Salidas: ${registrosDetalleSemana.length}. Ajustes: pendientes de conectar. ` +
