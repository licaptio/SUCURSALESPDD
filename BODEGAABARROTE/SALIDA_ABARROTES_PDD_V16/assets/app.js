...
<button id="clear" class="secondary">BORRAR FIRMA</button>
  ...
  $('save').onclick=()=>{
    ...
    try{
      ...
      // 3) La pantalla queda libre. El PDF se genera y se abre para imprimir;
      // Firebase/Telegram continúan por separado y no bloquean al operador.
      ro.disconnect();close();
      S.cart=[];S.last=[];render();S.recibe='';S.recibeEmpleado=null;S.destino='';S.fechaCaptura='';showModeGate();
      toast('Salida guardada · preparando PDF para imprimir');
      openSalidaPdfForPrint(salida,pdfWindow).then(ok=>{if(!ok)toast('Salida guardada · no se pudo abrir el PDF')});
