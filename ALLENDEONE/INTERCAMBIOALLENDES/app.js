...
const DB_NAME = 'PROVSOFT_INTERCAMBIOS';
const DB_VERSION = 1;
const STORE_PRODUCTOS = 'catalogo_allende1';
const LS_BORRADOR = 'borrador_intercambio_a1';
const LS_AUTORIZACIONES_NOTIFICADAS = 'intercambios_autorizados_notificados_a1';
...
window.generarSolicitud = async function(){
  ...
  const documento = {
    folio:f,
    tienda_solicita:'ALLENDE 1',
    tienda_contesta:'ALLENDE 2',
  ...
  try{
    ...
    const referenciaIntercambio = doc(db,'TIENDAS','ALLENDE 1','INTERCAMBIOS',f);
...
function generarPDFIntercambio(docu,tipo='SOLICITUD'){
 ...
 pdf.setFontSize(10); pdf.setFont('helvetica','normal'); pdf.text(`${docu.tienda_solicita||'ALLENDE 1'} → ${docu.tienda_contesta||'ALLENDE 2'}`,105,y,{align:'center'}); y+=12;
 const campos=[['Folio:',docu.folio||''],['Solicita:',docu.tienda_solicita||'ALLENDE 1'],['Contesta:',docu.tienda_contesta||'ALLENDE 2'],['Estado:',docu.estado||''],['Solicitado por:',docu.solicitado_por||''],['Autorizado por:',docu.autorizado_por||docu.contestado_por||''],['Fecha:',new Date().toLocaleString('es-MX')]];
...
function imprimir(docu){
  ...
  win.document.write(`
    ...
    <body>
      ...
      <b>Solicita:</b> ALLENDE 1<br>
      <b>Contesta:</b> ALLENDE 2<br>
