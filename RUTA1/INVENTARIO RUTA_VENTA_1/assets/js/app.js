...
const REF_INVENTARIO_RUTA1 = refDesdeRuta(APP_CONFIG.colecciones.inventarios);
const REF_ENTRADAS_RUTA1 = refDesdeRuta(APP_CONFIG.colecciones.entradas);
const REF_VENTAS_RUTAV2 = refDesdeRuta(APP_CONFIG.colecciones.ventas);
const REF_AJUSTES_RUTA1 = refDesdeRuta(APP_CONFIG.colecciones.ajustes);
const REF_CONTEOS_RUTA1 = refDesdeRuta(APP_CONFIG.colecciones.conteos);
...
async function cargarInventarioInicial() {
  ...
  aplicarLimiteSelectorSemana();
  setStatus(`Cargando inventario inicial Ruta 1 del ${fechaCorta(FECHA_BASE_INVENTARIO)}...`);
...
async function cargarAjustesRuta1UnaVez() {
  if (cacheBaseCargado && cacheAjustesTodos.length) return cacheAjustesTodos;

  setStatus("Cargando ajustes Ruta 1...");
  setProgress(72);

  const snap = await getDocs(REF_AJUSTES_RUTA1);
  const ajustes = [];
...
function recalcularTeoricoLote(codigoKey, fecha, hora, excluirIndice = -1) {
  let teorico = calcularExistenciaTeoricaHasta(codigoKey, fecha, hora);
  loteAjustesPendientes.forEach((item, i) => {
    if (i !== excluirIndice && item.codigoKey === codigoKey) teorico += Number(item.diferencia || 0);
  });
  return teorico;
}

function pintarLoteAjustes() {
  const tbody = $("tbodyLoteAjustes");
  if (!tbody) return;
  $("resumenLoteAjustes").textContent = `${loteAjustesPendientes.length} partida(s) pendiente(s)`;
  tbody.innerHTML = loteAjustesPendientes.length ? loteAjustesPendientes.map((x, i) => `
    <tr>
      <td>${escapeHtml(x.codigo)}</td>
      <td>${escapeHtml(x.nombre)}</td>
      <td>${fmtNum(x.teorico)}</td>
      <td>${fmtNum(x.fisico)}</td>
      <td class="${Number(x.diferencia) > 0 ? 'positivo' : Number(x.diferencia) < 0 ? 'negativo' : ''}">${fmtNum(x.diferencia)}</td>
      <td>
        <button type="button" class="btn-mini" data-editar-ajuste="${i}">Editar</button>
        <button type="button" class="btn-mini danger" data-eliminar-ajuste="${i}">Eliminar</button>
      </td>
    </tr>`).join('') : '<tr><td colspan="6" class="sin-datos">No hay partidas en el lote.</td></tr>';

  tbody.querySelectorAll('[data-eliminar-ajuste]').forEach(btn => btn.addEventListener('click', () => {
    loteAjustesPendientes.splice(Number(btn.dataset.eliminarAjuste), 1);
    pintarLoteAjustes();
  }));
  tbody.querySelectorAll('[data-editar-ajuste]').forEach(btn => btn.addEventListener('click', () => {
    const i = Number(btn.dataset.editarAjuste);
    const item = loteAjustesPendientes[i];
    if (!item) return;
    const nuevo = prompt(`Físico para ${item.codigo} · ${item.nombre}`, String(item.fisico));
    if (nuevo === null) return;
    const fisico = Number(nuevo);
    if (Number.isNaN(fisico) || fisico < 0) return alert('Captura un físico válido.');
    const teorico = recalcularTeoricoLote(item.codigoKey, item.fecha, item.hora, i);
    item.teorico = teorico;
    item.fisico = fisico;
    item.diferencia = fisico - teorico;
    item.ajuste = item.diferencia;
    item.tipo = item.diferencia >= 0 ? 'ENTRADA_AJUSTE' : 'SALIDA_AJUSTE';
    pintarLoteAjustes();
  }));
}
...
async function guardarAjuste() {
  ...
  if (!producto) return alert("El código no existe en el inventario o movimientos de Ruta 1.");
  ...
  const existente = loteAjustesPendientes.findIndex(x => x.codigoKey === producto.codigoKey);
  const teorico = recalcularTeoricoLote(producto.codigoKey, fecha, hora, existente);
  const fisico = Number(fisicoTexto);
  const diferencia = fisico - teorico;
  if (diferencia === 0 && !confirm("El físico ya coincide con el teórico. ¿Agregar ajuste en cero al lote?")) return;

  const partida = {
    fecha, hora, fechaHora: `${fecha} ${hora}:00`,
    codigo: producto.codigo || codigo, codigoKey: producto.codigoKey,
    nombre: producto.nombre || "", teorico, fisico, diferencia,
    ajuste: diferencia, tipo: diferencia >= 0 ? "ENTRADA_AJUSTE" : "SALIDA_AJUSTE",
    rutaId: "RUTA1"
  };

  if (existente >= 0) {
    if (!confirm(`El código ${partida.codigo} ya está en el lote. ¿Reemplazar su físico por ${fmtNum(fisico)}?`)) return;
    loteAjustesPendientes[existente] = partida;
  } else {
    loteAjustesPendientes.push(partida);
  }
  pintarLoteAjustes();
  limpiarFormularioAjuste();
}

async function enviarLoteAjustes() {
  if (!loteAjustesPendientes.length) return alert('No hay partidas pendientes en el lote.');
  const resumen = loteAjustesPendientes.map(x => `${x.codigo} · ${x.nombre}: ${fmtNum(x.teorico)} → ${fmtNum(x.fisico)} (${fmtNum(x.diferencia)})`).join('\n');
  if (!confirm(`Se guardarán ${loteAjustesPendientes.length} ajustes en un solo lote:\n\n${resumen}\n\n¿Continuar?`)) return;

  const btn = $("btnEnviarLoteAjustes");
  btn.disabled = true;
  btn.textContent = 'Guardando lote...';
  const ahora = new Date();
  const fecha = hoyISO() < FECHA_INICIO_MINIMA ? FECHA_INICIO_MINIMA : hoyISO();
  const hora = ahora.toTimeString().slice(0,5);
  const folioLote = `AJU-RUTA1-${fecha.replaceAll('-', '')}-${hora.replace(':','')}-${Date.now().toString().slice(-4)}`;

  try {
    const batch = writeBatch(db);
    loteAjustesPendientes.forEach((x, indice) => {
      const refNuevo = doc(REF_AJUSTES_RUTA1);
      batch.set(refNuevo, { ...x, lote_folio: folioLote, lote_indice: indice + 1, lote_total: loteAjustesPendientes.length, creadoEn: serverTimestamp() });
    });
    await batch.commit();

    try {
      await enviarLoteAjustesTelegram({ folio: folioLote, fecha, hora, partidas: loteAjustesPendientes });
    } catch (telegramError) {
      console.error('Lote guardado, pero falló Telegram:', telegramError);
      alert('El lote se guardó correctamente, pero falló la notificación de Telegram: ' + telegramError.message);
    }

    const totalGuardado = loteAjustesPendientes.length;
    loteAjustesPendientes = [];
    pintarLoteAjustes();
    ...
    alert(`Lote ${folioLote} guardado correctamente con ${totalGuardado} partida(s).`);
  } catch (error) {
    console.error(error);
    alert('No se pudo guardar el lote de ajustes: ' + error.message);
  } finally {
    btn.disabled = false;
...
async function asegurarDatosBaseCargados() {
  ...
  cargaBasePromise = (async () => {
    ...
    await cargarAjustesRuta1UnaVez();
...
async function cargarListasConteo() {
  const snap = await getDocs(query(REF_CONTEOS_RUTA1, orderBy("nombre", "asc")));
...
async function guardarListaConteo() {
  ...
  btn.disabled = true;
  try {
    if (listaConteoEditandoId) await setDoc(doc(REF_CONTEOS_RUTA1, listaConteoEditandoId), data, { merge:true });
    else {
      data.fecha_creacion = serverTimestamp();
      await addDoc(REF_CONTEOS_RUTA1, data);
    }
...
async function eliminarListaConteo(id) {
  ...
  try {
    await deleteDoc(doc(REF_CONTEOS_RUTA1, id));
...
function generarPdfListaConteo(id) {
  ...
  pdf.text("RUTA VENTA 1", pageW/2, 16, { align:"center" });
  ...
  pdf.save(`${String(lista.nombre || "conteo_ruta1").replace(/[^a-z0-9_-]+/gi,"_")}.pdf`);
}
...
function inicializarEventos() {
  ...
  $("btnEnviarLoteAjustes").addEventListener("click", enviarLoteAjustes);
  $("btnVaciarLoteAjustes").addEventListener("click", () => {
    if (!loteAjustesPendientes.length || confirm("¿Vaciar todas las partidas pendientes del lote?")) { loteAjustesPendientes = []; pintarLoteAjustes(); }
  });
